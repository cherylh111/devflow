...
Hooks are the automation layer that connects a platform to DevFlow. When the user wants to change "when context is injected," "how shell commands inherit a session," or "which files are read before an agent starts," hooks are usually the edit point.
...
| Hook | Purpose |
| --- | --- |
| session-start | Injects a DevFlow overview when a session starts, clears, or compacts. |
...
3. Read the hook script and identify inputs, outputs, and called `.devflow/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.devflow/workflow.md`.
...
If the hook ultimately calls `.devflow/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.devflow/`; they should not depend on DevFlow upstream source paths.
