...
The DevFlow task system is stored entirely under `.devflow/tasks/` in the user project. Each task is a directory containing requirements, context, research, state, and relationship information.
...
```bash
python ./.devflow/scripts/task.py create "<child title>" --slug <child-slug> --parent <parent-dir>
```

Link or unlink existing tasks with:

```bash
python ./.devflow/scripts/task.py add-subtask <parent-dir> <child-dir>
python ./.devflow/scripts/task.py remove-subtask <parent-dir> <child-dir>
```

`children` on the parent is a historical list. When a child is archived, DevFlow keeps that child name in the parent so progress like `[2/3 done]` remains meaningful after completed children move to `archive/`.
...
The user sees a "current task," but DevFlow stores active task state per session.
...
```bash
python ./.devflow/scripts/task.py create "<title>" --slug <slug>
python ./.devflow/scripts/task.py start <task>
python ./.devflow/scripts/task.py current --source
python ./.devflow/scripts/task.py add-context <task> implement <file> <reason>
python ./.devflow/scripts/task.py add-context <task> implement knowledge:<id> <reason>
python ./.devflow/scripts/task.py validate <task>
python ./.devflow/scripts/task.py finish
python ./.devflow/scripts/task.py archive <task>
```
...
| Need | Edit location |
| --- | --- |
| Change the default task template | `.devflow/scripts/common/task_store.py` and task creation instructions. |
| Change status semantics | `.devflow/workflow.md`, workflow-state hook logic, and task usage conventions. |
| Add task lifecycle actions | `hooks.after_*` in `.devflow/config.yaml`. |
| Change context rules | Planning artifact guidance in `.devflow/workflow.md` and related platform agent/hook instructions. |
| Change archive policy | `.devflow/scripts/common/task_store.py` / `task_utils.py`. |

These are local files in the user project. Do not default to editing DevFlow CLI source code unless the user wants to contribute upstream.
