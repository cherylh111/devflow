...
Filter out paths under `.devflow/workspace/` and `.devflow/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
- **Any remaining path looks like current-task work** — bail out with:
  > "Working tree has uncommitted code changes from this task: `<list>`. Return to workflow Phase 3.4 to commit them before running ``finish-work` (DevFlow command)`."
...
```bash
python ./.devflow/scripts/task.py archive <task-name>
```
