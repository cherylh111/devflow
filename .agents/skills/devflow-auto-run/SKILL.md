---
name: devflow-auto-run
description: "Run the approved ready-task queue through the normal DevFlow workflow without requiring manual phase commands between steps. Use only after the user approves the queue and stop on unresolved decisions, failed checks, rejected commit plans, or user stop requests."
---

# Auto Run Task Queue

Run the trusted-team fast path: process ready DevFlow tasks one after another without asking the user to manually invoke ``continue` (DevFlow command)` between phases.
...
python ./.devflow/scripts/task.py list --mine
```
...
Ask once for approval to auto-run that queue. After approval, do not ask for another ``continue` (DevFlow command)` between phases. Keep the existing Phase 3.4 commit-plan confirmation unless the user explicitly approved a concrete commit plan in advance.
...
```bash
python ./.devflow/scripts/task.py start <task-dir-or-name>
```
...
```bash
python ./.devflow/scripts/get_context.py --mode phase --step 2.2 --platform codex
python ./.devflow/scripts/get_context.py --mode phase --step 3.1 --platform codex
```
...
```bash
python ./.devflow/scripts/get_context.py --mode phase --step 3.3 --platform codex
```

Capture reusable knowledge with `devflow-update-spec`, `python ./.devflow/scripts/knowledge.py learn`, or local knowledge entries when warranted. If there is nothing to record, state that conclusion briefly.
...
```bash
python ./.devflow/scripts/get_context.py --mode phase --step 3.4 --platform codex
```
...
```bash
python ./.devflow/scripts/get_context.py --mode record
```

Then follow ``finish-work` (DevFlow command)`:
