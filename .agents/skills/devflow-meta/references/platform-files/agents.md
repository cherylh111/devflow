...
| `devflow-implement` | Implement against `prd.md`, optional `design.md` / `implement.md`, `implement.jsonl`, and related spec/research/knowledge. |
...
Main-session workflow platforms such as Kilo, Antigravity, and Windsurf may not have DevFlow sub-agent files. They usually rely on workflows/skills to guide the main session.
...
2. **Specify the read order**. Agents must know to start from the active task, read jsonl/spec/knowledge context, then read `prd.md`, `design.md` if present, and `implement.md` if present.
