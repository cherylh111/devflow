---
name: devflow-update-spec
description: "Captures executable contracts and coding conventions into .devflow/spec/ documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions."
---
...
```
Development Flow:
  Learn something → `update-spec` (DevFlow command) → Knowledge captured
       ↑                                  ↓
  `break-loop` (DevFlow command) ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- ``break-loop` (DevFlow command)` - Analyzes bugs deeply, often reveals spec updates needed
- ``update-spec` (DevFlow command)` - Actually makes the updates
- ``finish-work` (DevFlow command)` - Reminds you to check if specs need updates
