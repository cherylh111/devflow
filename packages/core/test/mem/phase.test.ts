...
describe("collectCodexTurnsAndEvents", () => {
  ...
  it("recognizes task.py boundary from `argv[]` joined with spaces", () => {
        ...
        payload: {
          ...
          arguments: JSON.stringify({
            argv: [
              ...
              ".devflow/scripts/task.py",
              "start",
              ".devflow/tasks/05-08-codex-task",
            ],
          }),
        },
      },
    ...
    expect(events[1]).toMatchObject({
      action: "start",
      taskDir: ".devflow/tasks/05-08-codex-task",
      turnIndex: 2,
    });
    ...
    const s = buildSession([
      ...
      },
      {
        ...
        payload: {
          ...
          name: "exec_command",
          arguments: JSON.stringify({
            cmd: "python3 .devflow/scripts/task.py create --slug str-cmd",
          }),
        },
      },
    ]);
...
describe("collectPiTurnsAndEvents", () => {
  ...
  it("captures task.py boundaries from assistant toolCall blocks and bashExecution messages", () => {
    const s = buildSession([
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating" },
            {
              ...
              arguments: {
                command:
                  "python3 .devflow/scripts/task.py create --slug pi-task",
              },
            },
          ],
        },
      ...
      },
      {
        ...
        message: {
          role: "bashExecution",
          command:
            "python3 .devflow/scripts/task.py start .devflow/tasks/06-18-pi-task",
          output: "",
        },
      },
      {
    ...
    expect(events[1]).toMatchObject({
      action: "start",
      taskDir: ".devflow/tasks/06-18-pi-task",
      turnIndex: 3,
    });
