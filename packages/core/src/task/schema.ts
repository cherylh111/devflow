/**
 * Canonical task.json shape — single source of truth for DevFlow tasks.
 *
 * The runtime Python writer is `.devflow/scripts/common/task_store.py`
 ...
 * task.json shape.
 */
export interface DevFlowTaskRecord {
  id: string;
  name: string;
  title: string;
  description: string;
...
export const TASK_RECORD_FIELD_ORDER = [
  ...
  "notes",
  "meta",
] as const satisfies readonly (keyof DevFlowTaskRecord)[];
...
/**
 * Lightweight runtime schema for {@link DevFlowTaskRecord}. Zero-dep on
...
export const taskRecordSchema = {
  parse(input: unknown): DevFlowTaskRecord {
    return parseTaskRecord(input);
  },
  safeParse(
    input: unknown,
  ):
    | { success: true; data: DevFlowTaskRecord }
...
function parseTaskRecord(input: unknown): DevFlowTaskRecord {
...
/**
 * Produce a fully-populated canonical-shape {@link DevFlowTaskRecord}.
 *
...
export function emptyTaskRecord(
  overrides: Partial<DevFlowTaskRecord> = {},
): DevFlowTaskRecord {
  const today = new Date().toISOString().split("T")[0] ?? "";
  const base: DevFlowTaskRecord = {
    id: "",
    name: "",
    title: "",
    description: "",
