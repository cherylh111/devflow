import type { DevFlowTaskRecord } from "./schema.js";

/**
 * Coarse-grained DevFlow task phase derived from task status.
 *
 * Phase is a projection of {@link DevFlowTaskRecord.status} only. There is
...
export function inferTaskPhase(
  recordOrStatus: DevFlowTaskRecord | string | null | undefined,
): DevFlowTaskPhase {
  const status =
