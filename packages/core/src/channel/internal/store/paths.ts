...
/** Top-level DevFlow channels directory. */
export function channelRoot(): string {
  const env = process.env.DEVFLOW_CHANNEL_ROOT;
  if (env && env.length > 0) return path.resolve(env);
  return path.join(os.homedir(), ".devflow", "channels");
}
 ...
 * `DEVFLOW_CHANNEL_PROJECT` env first, then falls back to deriving from
 * `process.cwd()`.
 */
export function currentProjectKey(): string {
  const env = process.env.DEVFLOW_CHANNEL_PROJECT;
...
export function resolveExistingChannelRef(
  ...
  if (opts.scope) {
    ...
    process.env.DEVFLOW_CHANNEL_PROJECT = project;
  ...
  if (globalExists) {
    process.env.DEVFLOW_CHANNEL_PROJECT = GLOBAL_PROJECT_KEY;
    return {
      name,
      scope: "global",
  ...
  if (fs.existsSync(eventsPath(name, current))) {
    process.env.DEVFLOW_CHANNEL_PROJECT = current;
    return {
      name,
      scope: "project",
      project: current,
  ...
  if (projectMatches.length === 1) {
    process.env.DEVFLOW_CHANNEL_PROJECT = projectMatches[0];
    return {
      name,
      scope: "project",
