...
def should_skip_injection() -> bool:
    """Check if any platform's non-interactive flag is set, or if DevFlow
    hooks are explicitly disabled via DEVFLOW_HOOKS=0 / DEVFLOW_DISABLE_HOOKS=1.
    """
...
def _resolve_context_key(devflow_dir: Path, input_data: dict) -> str | None:
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    """Expose DevFlow session identity to later Claude Code Bash commands.
    ...
    try:
        with open(env_file, "a", encoding="utf-8") as handle:
            handle.write(f"export DEVFLOW_CONTEXT_ID={shlex.quote(context_key)}\n")
    except OSError:
        pass


def _resolve_active_task(devflow_dir: Path, input_data: dict):
    scripts_dir = devflow_dir / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
...
def _get_task_status(devflow_dir: Path, input_data: dict) -> str:
    """Return compact active-task status, artifact presence, and next action."""
    active = _resolve_active_task(devflow_dir, input_data)
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            f"Next-Action: Run `python3 ./.devflow/scripts/task.py finish` to clear the stale pointer, "
...
def _load_devflow_config(devflow_dir: Path, input_data: dict) -> tuple:
    """Load DevFlow config for session-start decisions.
...
def _collect_spec_index_paths(devflow_dir: Path, allowed_pkgs: set | None) -> list[str]:
    paths: list[str] = []
...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `python3 ./.devflow/scripts/task.py list --mine` only if needed."
            )
        except Exception:
            pass
...
def main():
    ...
    context_key = _resolve_context_key(devflow_dir, hook_input)
    _persist_context_key_for_bash(context_key)

    # Load config for scope filtering and legacy detection
    is_mono, packages, scope_config, task_pkg, default_pkg = _load_devflow_config(
        devflow_dir,
        hook_input,
    )
    ...
    spec_index_paths = _collect_spec_index_paths(devflow_dir, allowed_pkgs)
...
""")
    ...
    legacy_warning = _check_legacy_spec(devflow_dir, is_mono, packages)
    if legacy_warning:
    ...
    output.write(_build_workflow_overview(devflow_dir / "workflow.md"))
    output.write("\n</devflow-workflow>\n\n")
    ...
    adr_path = project_dir / "docs" / "adr"

    if vocab_path.is_file():
        output.write(
            "## Domain vocabulary\n"
            f"Load project-specific terms: `cat {vocab_path.relative_to(project_dir)}`\n\n"
        )

    if adr_path.is_dir() and any(adr_path.glob("*.md")):
        output.write(
            "## Architecture Decision Records\n"
            f"Browse decisions: `ls {adr_path.relative_to(project_dir)}/*.md`\n"
        )
