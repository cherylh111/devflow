"""
Safe git-add helpers for DevFlow-owned paths.

Why this module exists
----------------------
A real user incident: a project's `.gitignore` listed `.devflow/` (company-wide
...
`git add -f .devflow/` — which fan-out-included every ignored subtree
(`.devflow/.backup-*/`, `.devflow/worktrees/`, `.devflow/.template-hashes.json`,
`.devflow/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.devflow/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.devflow/` in `.gitignore` is treated as user
  intent ("keep .devflow/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .devflow/` ...``
...
# Paths under .devflow/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.devflow/` tree.
DEVFLOW_IGNORED_SUBPATHS = (
    ".devflow/.backup-*",
    ".devflow/worktrees/",
...
def safe_archive_paths_to_add(
    ...
    `.devflow/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .devflow/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .devflow/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    print(
        "[WARN] DevFlow manages these specific paths and they should be tracked:",
        file=sys.stderr,
    )
    if paths:
        for p in paths:
        ...
        print(
            "[WARN]   .devflow/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .devflow/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .devflow/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.devflow/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in DEVFLOW_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .devflow/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .devflow/config.yaml:",
        file=sys.stderr,
    )
    print(
