...
Use `/devflow:auto-run` when multiple tasks have already been discussed and are ready to execute. The runner follows the same required steps as the manual flow - implement, check, knowledge review, Phase 3.4 commit, `/devflow:finish-work`, then the next ready task - without asking the user to invoke `/devflow:continue` between phases.

Auto-run is not approval to skip gates. It must stop when a task has unresolved requirements, checks cannot be made green, dirty files cannot be classified, the Phase 3.4 commit plan is rejected, or the user interrupts.
...
<!-- Per-turn breadcrumb: shown while status='in_progress'.
     ...
     commit, including Phase 3.3 knowledge update and Phase 3.4 commit. -->
...
- 3.3 Knowledge update `[required · once]`
...
Review whether this task produced reusable knowledge worth recording:
- Newly discovered patterns or conventions
- Pitfalls you hit
- New technical decisions
- Lightweight lessons that should be searchable later

Use the right persistence path:
- Hard implementation/review rules: load `devflow-update-spec` and update the relevant `.devflow/spec/` guide, or run `devflow spec add --layer <layer> --file <file.md> --title "<title>" --body "<rule>"`.
- Lightweight lessons or gotchas: load `devflow-learn` or run `devflow knowledge learn "<insight>" --keywords <csv> --task current`.
- Longer reusable recipes, references, decisions, templates, assets, or session handoffs: run `devflow knowhow add --type <type> --title "<title>" --body "<markdown>"`.
- Local knowledge edits: use `devflow wiki create/update/append/remove-entry/delete` for deliberate structured knowledge changes.
- Knowledge graph links: run `devflow wiki connect --dry-run --report` before `devflow wiki connect --fix --report` applies `related` metadata between structured entries; add `--learn` when the connection pass should become a reusable learning.
- Knowledge graph cleanup: run `devflow wiki cleanup --report` to inspect issues; `devflow wiki cleanup --fix --report` only removes broken structured `related` metadata.
- Knowledge synthesis: run `devflow wiki digest <topic> --report` to persist theme/gap summaries into the active task's `research/` directory; add `--learn` when the synthesis itself should become a reusable learning.
- Knowledge gap routing: add `devflow wiki digest <topic> --create-issues --report` when digest gaps should become searchable `issue` entries.

Even if the conclusion is "nothing to update", walk through the judgment.
...
After the above, remind the user they can run `/finish-work` to wrap up (archive the task, record the session), or `/devflow:auto-run` when they want the AI to continue through the next already-ready task without manual `/devflow:continue` calls between phases.
