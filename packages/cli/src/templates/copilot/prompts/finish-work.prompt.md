---
description: "DevFlow Copilot prompt: Finish Work — survey + archive task + record session journal"
---
...
Filter out paths under `.devflow/workspace/` and `.devflow/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this prompt's own work.
