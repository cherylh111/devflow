...
# -*- coding: utf-8 -*-
"""
Copilot Session Start Hook - Emit DevFlow session-start context.
...
and Copilot versions, which is outside DevFlow's control. UserPromptSubmit
...
def _resolve_active_task(devflow_dir: Path, hook_input: dict):
    ...
    return resolve_active_task(devflow_dir.parent, hook_input, platform="copilot")
...
def _get_task_status(devflow_dir: Path, hook_input: dict) -> str:
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            "Next: Task directory not found. Run: python3 ./.devflow/scripts/task.py finish"
        )
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\n"
            f"Next: Archive with `python3 ./.devflow/scripts/task.py archive {task_dir.name}` "
            "or start a new task."
        )
