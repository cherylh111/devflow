/* global process */
/**
 * DevFlow Workflow State Injection Plugin
 *
 * Per-turn UserPromptSubmit equivalent for OpenCode.
 *
 * On every chat.message, if a DevFlow task is active, inject a short
...
import { DevFlowContext, debugLog, isDevFlowSubagent } from "../lib/devflow-context.js"
...
function loadBreadcrumbs(directory) {
  const workflowPath = join(directory, ".devflow", "workflow.md")
...
export default async ({ directory }) => {
  ...
  return {
      ...
      "chat.message": async (input, output) => {
        try {
          // Skip DevFlow sub-agent turns — the per-turn breadcrumb is for the
          // main session only; sub-agent context comes from the parent's
          // tool.execute.before injection.
          if (isDevFlowSubagent(input)) {
            debugLog("workflow-state", "Skipping devflow subagent turn:", input?.agent)
            return
          }
