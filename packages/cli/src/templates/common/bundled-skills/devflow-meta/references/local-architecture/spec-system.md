...
`.devflow/spec/` is the user's project-specific engineering spec library. DevFlow is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.devflow/config.yaml` can declare packages:
...
When the AI learns a new rule during implementation or debugging, it should update `.devflow/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need | Edit location |
| --- | --- |
| Add a new spec layer | `.devflow/spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping | `packages` / `default_package` / `spec_scope` in `.devflow/config.yaml`. |
...
| Change when specs should be updated | Phase 3.3 in `.devflow/workflow.md` and the `devflow-update-spec` skill. |

## Boundaries

`.devflow/spec/` is the user's project specification, not a permanent copy of DevFlow built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating DevFlow default templates as immutable documents.
