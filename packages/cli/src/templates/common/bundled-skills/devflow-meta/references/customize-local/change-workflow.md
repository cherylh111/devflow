...
When the user wants to change DevFlow phases, next-action hints, whether to create tasks, whether to use sub-agents, or when to check/wrap up, edit `.devflow/workflow.md` first.
...
1. Find the relevant section in `.devflow/workflow.md`.
...
4. Workflow-state changes only need an edit to the `[workflow-state:STATUS]` block in `.devflow/workflow.md`. The hook is parser-only — it reads whatever you put in the block. Keep the opening and closing tags' STATUS strings identical (`[workflow-state:foo]…[/workflow-state:foo]`); mismatched STATUS pairs are silently dropped.
5. Make the AI reread `.devflow/workflow.md`; do not keep using rules from the old conversation.
...
If the user wants only one platform to avoid sub-agents, first confirm whether that platform has a separate group in the workflow. Then change Phase 2 routing for that platform group instead of deleting all `devflow-implement` / `devflow-check` instructions across platforms.

## `/devflow:continue` Route Table

`/devflow:continue` resumes a task by deciding which phase step to load next. The decision combines `task.json.status` with the presence of artifacts inside the task directory. The mapping is fixed in the command itself; forks that add custom statuses must extend both the workflow.md tag block and this table.
...
When you add a custom status (e.g. `in-review`), add a `[workflow-state:in-review]` block in `.devflow/workflow.md` for the per-turn breadcrumb AND extend this route table — usually by editing the `/devflow:continue` command file (`.{platform}/commands/devflow/continue.md` or equivalent) to add a row that decides where to resume from. Without the route entry, `/devflow:continue` will fall through to a default branch and the user will not land on the step you intended.

## Notes

`.devflow/workflow.md` is the local project workflow, not an immutable template. The user can adapt it to team habits. After editing it, platform entry files may still contain old descriptions, so inspect them too.
