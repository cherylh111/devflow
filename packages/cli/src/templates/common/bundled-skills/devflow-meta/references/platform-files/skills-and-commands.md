...
Skills and commands are textual entry points for user interaction with DevFlow. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the DevFlow flow when the user expresses a certain intent.
...
DevFlow workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `devflow-meta` use layered references.
...
They should not store task state; task state belongs in `.devflow/tasks/` and `.devflow/.runtime/`.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `devflow-meta`.
...
4. **Put project-specific capabilities in local skills**. Do not put team-private flows into public `devflow-meta`.

If the user only wants local AI to know one more project rule, usually create a project-local skill or update `.devflow/spec/` instead of changing a DevFlow built-in workflow skill.
