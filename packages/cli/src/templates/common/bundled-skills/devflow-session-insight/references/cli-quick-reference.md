# `devflow mem` CLI Reference

Full flag reference for the five subcommands. Pin this as the authoritative source: `devflow mem help` prints the same content at runtime, so anything here that drifts is a bug.
...
| `context <session-id>` | Drill into one session: top-N hit turns plus surrounding context. Pair with `--grep` for keyword anchoring. |
...
| `--phase brainstorm\|implement\|all` | extract | Slice session by DevFlow task boundaries. `brainstorm` = `[task.py create, task.py start)`. `implement` = `[task.py start, task.py finish)` window. Default `all`. |
...
| `--task <task-dir>` | list | Narrow to sessions whose context-key resolved to a given task directory (uses `.devflow/.runtime/sessions/*.json`). |

## Common one-liners

```bash
# What past sessions discussed "deadlock" anywhere on this machine?
devflow mem search "deadlock" --global --limit 20

# Inside a specific session, surface the top 5 turns that mention "lock contention"
# plus 2 turns of surrounding context.
devflow mem context 5842592d --grep "lock contention" --turns 5 --around 2
...
# List every project this machine has DevFlow sessions for, with counts.
devflow mem projects
```
...
Run `devflow mem help` in the user's shell. The runtime help is authoritative and will be ahead of this reference during fast-moving beta releases.
