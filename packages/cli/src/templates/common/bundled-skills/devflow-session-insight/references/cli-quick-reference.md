...
| `--phase brainstorm\|implement\|all`          | extract           | Slice session by DevFlow task boundaries. `brainstorm` = `[task.py create, task.py start)`. `implement` = turns outside brainstorm windows. Default `all`. |
