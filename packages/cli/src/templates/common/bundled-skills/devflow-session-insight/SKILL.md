...
This skill teaches an AI **how to call `devflow mem`** — the project's cross-session memory feedstock — and **when reaching for it is the right move**.
...
- You are in a sub-agent (`devflow-implement` / `devflow-check`) whose dispatch prompt already includes the curated `implement.jsonl` / `check.jsonl` context. Adding `mem` on top usually just clutters.
...
- **Update `.devflow/spec/`** if the finding is a project-wide convention or gotcha that would help future tasks. Run the `devflow-update-spec` skill for that — `session-insight` ends at the discovery.
...
DevFlow does not prescribe a single destination. Forcing every recall into a fixed file makes the file grow into noise. Let the situation decide.
...
devflow mem search "<keyword>"

# Dump dialogue from one session, optionally filtered by phase or keyword.
devflow mem extract <session-id> --phase brainstorm
devflow mem extract <session-id> --grep "<keyword>"
...
- This skill does not replace `devflow-update-spec` (which is the right tool for promoting a finding into project-wide guidance) or the platform-native task / spec workflow.
