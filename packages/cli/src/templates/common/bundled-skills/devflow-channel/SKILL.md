---
name: devflow-channel
description: Use DevFlow channel for live multi-agent collaboration, spawned workers, cross-agent review, progress inspection, forum channels, and channel log debugging.
---

# devflow-channel

`devflow channel` is the local multi-agent collaboration runtime. Reach for it when agents need to talk through a durable event log, when a worker should be spawned as a peer process, when an in-flight worker needs interrupt / debugging, or when feedback should be recorded on a durable `--type forum` channel.
...
```bash
devflow --version
devflow channel --help
devflow channel list --all
devflow channel list --scope global --all
```

If the user names a channel or thread, inspect it before asking for background:

```bash
devflow channel forum <board> --scope global
devflow channel thread <board> <thread> --scope global
devflow channel context list <board> --scope global --thread <thread>
```
...
- Use `--context-file` / `--context-raw` and `devflow channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- **Dispatcher wait pattern**: use `--kind done` / `--kind turn_finished` (devflow-emitted system events), NOT a user `--tag` as the completion signal. CLI help lists `phase_done` / `question` as `--tag` examples but only `interrupt` is a reserved tag with hardcoded devflow behavior; the others are opaque user labels. Relying on a worker to run `send --tag <my_signal>` is unreliable — LLM workers commonly write the tag string into prose instead of running the actual CLI command. See `references/command-reference.md` "tag vs kind".
- Forum channels are event-sourced. Do not parse `events.jsonl` first; use `forum`, `thread`, `messages --thread`, and `context list`.
- `@enpd/devflow-core` owns reusable channel/thread state, event append, seq allocation, context/title projection, reducers, and task helpers. The CLI owns flags, terminal rendering, prompts, worker lifecycle, and process exits.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `devflow mem` (the `devflow-session-insight` skill) for session/history search.
