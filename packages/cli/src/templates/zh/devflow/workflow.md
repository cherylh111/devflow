...
长时间工作时，在有意义的实现、检查和收尾边界更新 `task.py progress set <task> ...`，让 `/devflow:continue` 能更快恢复。
[/workflow-state:in_progress]
...
- `progress.json` 中的进度提示只是辅助恢复状态；不能替代 `task.json.status`、PRD/design/implement 产物或验证证据。
