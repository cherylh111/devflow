---
name: record-session
description: "在人类测试并提交后，将已完成工作进展记录到 .devflow/workspace/ journal 文件。捕获 session summaries、commit hashes，并更新 developer index 文件，供未来会话使用。用于编码会话完成、人类已提交代码后，或需要为未来 AI 会话持久化 session knowledge 时。"
---

[!] **前置条件**：此 skill 只能在人类已经测试并提交代码之后使用。
