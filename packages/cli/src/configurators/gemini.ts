 ...
 * - commands/devflow/ — start + finish-work as TOML slash commands
 * - .agents/skills/devflow-{name}/SKILL.md — 5 auto-triggered shared skills
...
export async function configureGemini(cwd: string): Promise<void> {
  ...
  const commandsDir = path.join(configRoot, "commands", "devflow");
