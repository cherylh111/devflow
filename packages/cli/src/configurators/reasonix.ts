 ...
 *
 * Workflow templates are surfaced as skills with `devflow-` prefix (invocable
 * via `/skill devflow-start`, `/skill devflow-continue`, etc.).
 * Subagent skills (devflow-implement, devflow-check) use `runAs: subagent`
...
/**
 * Collect all Reasonix template files for `devflow update` diff tracking.
...
export function collectReasonixTemplates(): Map<string, string> {
  ...
  // Subagent skills (devflow-implement, devflow-check) — written with
