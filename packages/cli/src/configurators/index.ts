...
import { getAllAgents as getKiroAgents, getIdeHooks as getKiroIdeHooks } from "../templates/kiro/index.js";
import {
...
const PLATFORM_FUNCTIONS: Record<AITool, PlatformFunctions> = {
  ...
  devin: {
    ...
    collectTemplates: () =>
      collectBothTemplates(
        ...
        (n) => `.devin/workflows/devflow-${n}.md`,
        ".devin/skills",
      ),
  },
  qoder: {
  ...
  trae: {
    ...
    collectTemplates: () => {
      ...
      files.set(`.trae/${settings.targetPath}`, resolvePlaceholders(settings.content));
      return files;
    },
  },
};
