...
export async function configureCodex(cwd: string): Promise<void> {
  ...
  // Additionally write `devflow-start` to .agents/skills/ — Codex-specific.
  // The SessionStart hook was removed in 0.5.5 (de-recursion); inject-workflow-state.py
  // injects a `<devflow-bootstrap>` block on no_task turns instructing the AI to
  // invoke `$devflow-start` to load workflow context. Without this skill, that
  ...
  // (configurators/index.ts) — both share `resolveCodexDevFlowStartSkill`.
  const devflowStart = resolveCodexDevFlowStartSkill(
    AI_TOOLS.codex.templateContext,
  );
  if (devflowStart) {
    const devflowStartDir = path.join(sharedSkillsRoot, devflowStart.name);
    ensureDir(devflowStartDir);
    await writeFile(
      path.join(devflowStartDir, "SKILL.md"),
  ...
  // load DevFlow context themselves via the prelude injected here.
  ...
  // it the workflow breadcrumb won't auto-inject (the devflow-bootstrap
  // fallback in inject-workflow-state.py covers this case). Documented in
  // spec/cli/backend/platform-integration.md.
  if (!process.env.VITEST && !process.env.DEVFLOW_QUIET) {
    process.stderr.write(
      "⚠️  Codex hooks require `features.hooks = true` in your " +
        "~/.codex/config.toml (Codex 0.129+; older versions: `codex_hooks = true`). " +
        "On Codex 0.129+ also run `/hooks` once to approve the DevFlow " +
        "UserPromptSubmit hook. Without these the DevFlow workflow breadcrumb " +
        "won't auto-inject. See DevFlow docs for details.\n",
    );
  }
