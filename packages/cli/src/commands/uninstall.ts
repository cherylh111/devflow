/**
 * `devflow uninstall` — remove every file written by `devflow init` / `update`
 * from the current project, plus the `.devflow/` directory itself.
 *
 * The single source of truth for "what devflow wrote" is
 * `.devflow/.template-hashes.json`. Files outside that manifest are never
 ...
 *      package.json) — passed through a scrubber that strips just the devflow
 ...
 * (per the PRD: "全删"). The `.devflow/` tree is removed unconditionally.
 */
...
interface StructuredFileSpec {
   ...
   * devflow-managed `command` strings.
   */
...
interface UninstallPlan {
  ...
  /** Whether `.devflow/` directory itself will be removed. */
  removeDevFlowDir: boolean;
}
...
/**
 * Execute the plan: write modifications, unlink deletions, remove `.devflow/`,
...
function executePlan(
  ...
  // 3. Drop `.devflow/` entirely.
  let deletedDirs = 0;
  if (plan.removeDevFlowDir) {
    const devflowDir = path.join(cwd, DIR_NAMES.WORKFLOW);
  ...
  // has no purpose. `.devflow` is already gone (step 3), so we skip it.
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  // Pre-check 1: must have a `.devflow/` directory.
  if (!fs.existsSync(devflowDir)) {
    console.log(
      chalk.gray(
        localized(
  ...
  // platform files are devflow-owned vs user-owned.
  ...
  // manifest only to delete the whole .devflow/ dir anyway — but the
  ...
  if (!options.yes) {
    ...
    // honor the same UX as `devflow update` (which also fails closed in
    // that case).
