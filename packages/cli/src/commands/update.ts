...
const DEVFLOW_BLOCK_START = "<!-- DEVFLOW:START -->";
const DEVFLOW_BLOCK_END = "<!-- DEVFLOW:END -->";
const LEGACY_UNTRACKED_AGENTS_MD_BLOCK_HASHES = new Set<string>([
  ...
  // This hash is the pristine DevFlow-managed block before the Subagents
...
function getDevFlowManagedBlock(content: string): string | null {
  const start = content.indexOf(DEVFLOW_BLOCK_START);
  if (start === -1) {
    return null;
  }

  const end = content.indexOf(DEVFLOW_BLOCK_END, start);
  if (end === -1) {
    return null;
  }

  return content.slice(start, end + DEVFLOW_BLOCK_END.length);
}

function replaceDevFlowManagedBlock(
  existingContent: string,
  templateContent: string,
): string | null {
  const existingStart = existingContent.indexOf(DEVFLOW_BLOCK_START);
  if (existingStart === -1) {
    return null;
  }

  const existingEnd = existingContent.indexOf(DEVFLOW_BLOCK_END, existingStart);
  if (existingEnd === -1) {
    return null;
  }

  const templateBlock = getDevFlowManagedBlock(templateContent);
  if (!templateBlock) {
    return null;
  }
  ...
  return (
    ...
    templateBlock +
    existingContent.slice(existingEnd + DEVFLOW_BLOCK_END.length)
  );
}
...
function buildAgentsMdTemplate(cwd: string): string {
  ...
  // Existing file already has DEVFLOW:START/END markers — replace just the
  // managed block, preserving everything outside it.
  const replaced = replaceDevFlowManagedBlock(existingContent, agentsMdContent);
  if (replaced !== null) {
    return replaced;
  }

  // Existing file has no managed-block markers (pre-0.5.0-beta.18 project, or
  // user hand-wrote AGENTS.md without ever running through DevFlow). Append
  // the template's managed block at the end so user content is preserved
  // instead of clobbered.
  const templateBlock = getDevFlowManagedBlock(agentsMdContent);
  if (!templateBlock) {
    return agentsMdContent;
  }
...
function isKnownUntrackedTemplate(
  ...
  const managedBlock = getDevFlowManagedBlock(existingContent);
  if (!managedBlock) {
    return false;
  }
...
/**
 * Load update.skip paths from .devflow/config.yaml
 *
 ...
 *
 * Old DevFlow versions used `.agents/skills/` as codex's configDir.
 * New versions use `.codex/` for Codex-specific config and `.agents/skills/`
 * as a shared layer.
 *
 * Detection: DevFlow-tracked hashes contain `.agents/skills/` entries
...
function classifyMigrations(
  ...
  for (const item of migrations) {
    ...
    // rename/rename-dir are allowed to target protected paths (e.g., 0.2.0 renames into .devflow/workspace)
    if (
      item.to &&
...
async function promptMigrationAction(
  ...
  const whyFlagged = item.reason
    ...
    : chalk.gray(
        `  Why prompted: file content doesn't match the DevFlow template hash\n` +
 ...
 * Recursively removes empty parent directories up to .devflow root
 */
/** @internal Exported for testing only */
export function cleanupEmptyDirs(cwd: string, dirPath: string): void {
  ...
  // Safety: never delete managed root directories themselves (e.g., .claude, .devflow)
...
async function executeMigrations(
  ...
  for (const item of classified.confirm) {
    ...
    // This is in addition to the full project snapshot at .devflow/.backup-*/;
    ...
    if (item.type === "rename" && item.to) {
      ...
      if (action === "backup-rename") {
        ...
        // inline without digging through .devflow/.backup-*/.
...
export async function update(options: UpdateOptions): Promise<void> {
  ...
  // Detect legacy Codex (has .agents/skills/ tracked by DevFlow but no .codex/)
  ...
  // buggy versions of `devflow init` over-hashed (e.g. .codex/sessions/*).
  ...
  if (hasMigrations) {
    ...
    if (
      ...
      if (gateMetadata.breaking && gateMetadata.recommendMigrate) {
        ...
        console.log("");
        console.log(chalk.yellow(`  Run: devflow update --migrate`));
        console.log("");
        console.log(
          chalk.gray(
  ...
  if (options.migrate && classifiedMigrations) {
    ...
    // traces-*.md files are in .devflow/workspace/{developer}/ with variable developer names
