/**
 * Remote template fetcher for DevFlow CLI
 *
 * Fetches spec templates from the official marketplace:
 * https://github.com/enpd/marketplace
 */
...
export const TEMPLATE_INDEX_URL =
  "https://raw.githubusercontent.com/enpd/marketplace/main/index.json";

const TEMPLATE_REPO = "gh:enpd/marketplace";
...
async function cloneRegistryRef(
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "devflow-registry-"),
  );
  try {
    try {
      await runGit([
      ...
      await runGit(["-C", dir, "config", "core.eol", "lf"]);
    } catch (error) {
