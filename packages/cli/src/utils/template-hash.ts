...
/**
 * Patterns to exclude from hash tracking (only applied to the .devflow/ walk).
 */
const EXCLUDE_FROM_HASH = [
  ...
  ".devflow/spec/", // User-customized spec files
...
export interface InitializeHashesOptions {
  /**
   * POSIX-style relative paths devflow actually wrote during the init run
 ...
 *
 * `.devflow/` is still walked recursively (with `EXCLUDE_FROM_HASH`) because
 * uninstall removes `.devflow/` wholesale via `rm -rf` regardless of manifest
 * content — accuracy there doesn't affect data-loss, only `devflow update`
...
export function initializeHashes(
  ...
  if (trackedPaths) {
    for (const relativePath of trackedPaths) {
      // `.devflow/` paths are handled by the walk below — don't double-track.
      if (relativePath.startsWith(".devflow/") || relativePath === ".devflow") {
        continue;
      }
  ...
  // .devflow/ workflow tree: still walked recursively. Accuracy here is for
  // `devflow update`'s 3-way merge of workflow.md / config.yaml / scripts;
  // uninstall removes .devflow/ wholesale so it does not matter for the
  // data-loss bug this contract addresses.
  const files = collectFiles(cwd, ".devflow");
