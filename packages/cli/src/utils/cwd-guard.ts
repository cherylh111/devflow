 ...
 *
 * Running `devflow init` / `devflow uninstall` in `$HOME` is catastrophic:
...
/**
 * Error message printed by both `devflow init` and `devflow uninstall` when
 * the homedir guard trips.
 */
export function homedirGuardMessage(commandName: "init" | "uninstall"): string {
  return (
    `✗ Refusing to run \`devflow ${commandName}\` in your home directory.\n\n` +
    `DevFlow manages platform config dirs like .claude/, .codex/, .opencode/, which\n` +
    ...
    `Run devflow from your project directory instead. If you really want to run in\n` +
    `$HOME, set DEVFLOW_ALLOW_HOMEDIR=1.`
  );
}
...
export function homedirBypassEnabled(): boolean {
  return process.env.DEVFLOW_ALLOW_HOMEDIR === "1";
}
