 ...
 *
 * Centralizes how `devflow init --workflow` and `devflow workflow` discover and
...
import { workflowMdTemplate } from "../templates/devflow/index.js";
import {
  TIMEOUTS,
  TEMPLATE_INDEX_URL,
 ...
 *
 * Treated as DevFlow-managed for hash-tracking: when this id is selected by
 * `init --workflow` or `devflow workflow`, `.devflow/workflow.md` stays in
 ...
 *   `bundled:devflow/workflow.md` for the native virtual entry.
 */
...
function nativeListingEntry(): WorkflowTemplateListing {
  return {
    ...
    type: "workflow",
    name: "Native DevFlow Workflow",
    description:
      "Default DevFlow Plan / Execute / Finish workflow bundled with the CLI",
    path: "bundled:devflow/workflow.md",
    source: "bundled",
  };
}
...
async function fetchWorkflowFileGit(
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "devflow-workflow-"),
  );
