 ...
 * `devflow channel spawn --agent <name>`, which loads `.devflow/agents/<name>.md`
 ...
 * runtime. We surface that mismatch eagerly (at `devflow init --workflow` /
 * `devflow workflow --template` time) so users can run `devflow update` before
 * the first spawn.
 *
 ...
 *
 *   1. `--agent <name>` flag on a `devflow channel spawn ...` command
 *   2. `.devflow/agents/<name>.md` literal path reference
 *
...
const AGENT_PATH_RE = new RegExp(
  `\\.devflow/agents/([${SAFE_AGENT_NAME_CHARS}]+)\\.md`,
  "g",
);

/**
 * Extract the set of `.devflow/agents/<name>.md` agent names that the given
 ...
 * exist under `<cwd>/.devflow/agents/`.
 */
