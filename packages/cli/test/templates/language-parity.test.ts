...
describe("Chinese template overlays", () => {
  ...
  it("preserves workflow start-gate guidance", () => {
    const workflow = readText(join(zhRoot, "devflow", "workflow.md"));
    expect(workflow).toContain("implement.jsonl");
    expect(workflow).toContain("check.jsonl");
    expect(workflow).toContain("sub-agent-dispatch platforms");
    expect(workflow).toContain("Runtime consumers tolerate missing or seed-only manifests");
