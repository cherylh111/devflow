...
describe("codex getConfigTemplate", () => {
  ...
  // `data did not match any variant of untagged enum FeatureToml`. DevFlow
...
// =============================================================================
//
// devflow-implement / devflow-check agent toml MUST contain a hard recursion
// guard that tells the sub-agent it is already the dispatched agent and must
// not spawn another devflow-implement / devflow-check sub-agent. Without this,
// SessionStart's "dispatch devflow-implement" guidance leaks into sub-agent
...
describe("codex sub-agent recursion guard (issue #234)", () => {
  for (const name of ["devflow-implement", "devflow-check"] as const) {
    it(`${name}.toml developer_instructions forbids spawning devflow-implement / devflow-check`, () => {
...
describe("codex session-start.py compact SessionStart context", () => {
  ...
  it("uses compact task artifact guidance instead of sub-agent dispatch prose", () => {
    ...
    expect(content).toContain("DevFlow compact SessionStart context");
