...
import { DevFlowContext } from "../src/templates/opencode/lib/devflow-context.js";
...
describe("regression: add_session.py runtime branch context (issue-106)", () => {
  ...
  function writeDevFlowScripts(): void {
    const scriptsDir = path.join(tmpDir, ".devflow", "scripts");
...
`;
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", "workspace", "test-dev", "index.md"),
      indexContent,
      "utf-8",
    );
  }
  ...
  function setupSessionRepo(options?: {
    ...
    fs.mkdirSync(path.join(tmpDir, ".devflow", "workspace", "test-dev"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-22T00:00:00\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", "workspace", "test-dev", "journal-1.md"),
    ...
    if (options?.taskBranch || options?.taskBaseBranch) {
      const taskDir = path.join(tmpDir, ".devflow", "tasks", "issue-106");
      fs.mkdirSync(taskDir, { recursive: true });
      fs.mkdirSync(
        path.join(tmpDir, ".devflow", ".runtime", "sessions"),
        { recursive: true },
      );
      fs.writeFileSync(
        path.join(
        ...
        JSON.stringify(
          {
            current_task: ".devflow/tasks/issue-106",
            platform: "test",
          },
          null,
          2,
...
describe("regression: task directory paths (0.2.14, 0.2.15, beta.13)", () => {
  it("[0.2.15] PATHS.TASKS is .devflow/tasks (not .devflow/workspace/*/tasks)", () => {
    expect(PATHS.TASKS).toBe(".devflow/tasks");
    expect(PATHS.TASKS).not.toContain("workspace");
  });

  it("[0.2.14] Claude agent templates do not contain hardcoded .devflow/workspace/*/tasks/ paths", () => {
    const agents = getClaudeAgents();
    for (const agent of agents) {
      expect(agent.content).not.toMatch(/\.devflow\/workspace\/[^/]+\/tasks\//);
    }
  });
...
describe("regression: resolve_task_dir path handling", () => {
  it("[beta.12] resolve_task_dir handles .devflow prefix", () => {
    // The function should recognize .devflow-prefixed paths as relative paths
    expect(commonTaskUtils).toContain('.startswith(".devflow")');
  });
...
describe("regression: update only configured platforms (beta.16)", () => {
  ...
  // `devflow update` silently ignored .opencode/, so CLI-side changes to
  ...
  it("[beta.8] collectPlatformTemplates returns Map for opencode (plugins + agents + lib + package.json + commands + skills)", () => {
    ...
    // fix was a plugin-shape change that couldn't be delivered via `devflow update`.
    ...
    expect(result.has(".opencode/agents/devflow-implement.md")).toBe(true);
    expect(result.has(".opencode/lib/devflow-context.js")).toBe(true);
...
describe("regression: shell to Python migration (beta.0)", () => {
  it("[beta.0] no .sh scripts remain in devflow templates", () => {
    const scripts = getAllScripts();
  ...
  it("[beta.3] getAllScripts covers every .py file in templates/devflow/scripts/", () => {
...
describe("regression: agent-session DevFlow update hint", () => {
  let tmpDir: string;
  const pythonCmd = process.platform === "win32" ? "python" : "python3";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-update-hint-"));
    const scriptsDir = path.join(tmpDir, ".devflow", "scripts");
    for (const [relativePath, content] of getAllScripts()) {
      const absPath = path.join(scriptsDir, relativePath);
      fs.mkdirSync(path.dirname(absPath), { recursive: true });
    ...
    fs.mkdirSync(path.join(tmpDir, ".devflow", "tasks"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", ".developer"),
  ...
  function runContextWithDevFlowOutput(
    currentVersion: string,
    devflowVersionOutput: string | null,
  ): string {
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", ".version"),
      `${currentVersion}\n`,
      "utf-8",
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "sys.path.insert(0, str(Path.cwd() / '.devflow' / 'scripts'))",
        "from common import session_context",
        "output = os.environ.get('DEVFLOW_VERSION_OUTPUT')",
        "session_context._fetch_devflow_version_output = lambda: None if output == '__NONE__' else output",
    ...
    return execSync(`${pythonCmd} ${JSON.stringify(runnerPath)}`, {
      ...
      env: {
        ...process.env,
        DEVFLOW_VERSION_OUTPUT: devflowVersionOutput ?? "__NONE__",
        DEVFLOW_CONTEXT_ID: "test-update-session",
      },
    });
  }
  ...
  it("shows a concise update hint when devflow --version reports a newer version", () => {
    const output = runContextWithDevFlowOutput(
      "0.5.0",
      "DevFlow update available: 0.5.0 → 0.5.9\nRun: devflow update\n0.5.9",
    );

    expect(output).toContain("DevFlow update available: 0.5.0 -> 0.5.9");
    expect(output).toContain("run devflow upgrade");
    expect(output).toContain("SESSION CONTEXT");
  });

  it("does not show a hint when installed version is equal or newer", () => {
    expect(runContextWithDevFlowOutput("0.5.9", "0.5.9")).not.toContain(
      "DevFlow update available",
    );
    fs.rmSync(path.join(tmpDir, ".devflow", ".runtime"), {
      recursive: true,
      force: true,
    });
    expect(runContextWithDevFlowOutput("0.6.0", "0.5.9")).not.toContain(
      "DevFlow update available",
    );
  });

  it("silently skips the hint when devflow --version fails or version parsing fails", () => {
    expect(runContextWithDevFlowOutput("0.5.0", null)).not.toContain(
      "DevFlow update available",
    );
    fs.rmSync(path.join(tmpDir, ".devflow", ".runtime"), {
      recursive: true,
    ...
    expect(runContextWithDevFlowOutput("not-a-version", "0.5.9")).not.toContain(
      "DevFlow update available",
    );
  });
  ...
  it("does not burn the once-per-session marker when version lookup fails", () => {
    ...
    const output = runContextWithDevFlowOutput("0.5.0", "0.5.9");

    expect(output).toContain("DevFlow update available: 0.5.0 -> 0.5.9");
  });

  it("uses the final devflow --version token when no update line is present", () => {
    const output = runContextWithDevFlowOutput("0.5.0", "0.5.9");
  ...
  it("only attempts the default text update hint once per session", () => {
    const first = runContextWithDevFlowOutput("0.5.0", "0.5.9");
    const second = runContextWithDevFlowOutput("0.5.0", "0.5.9");

    expect(first).toContain("DevFlow update available: 0.5.0 -> 0.5.9");
    expect(second).not.toContain("DevFlow update available");
    expect(
      fs.existsSync(
        path.join(
          tmpDir,
  ...
  it("keeps the update hint out of JSON, record, packages, and phase paths", () => {
    ...
    for (const functionName of [
      ...
      expect(
        ...
        `${functionName} should not check DevFlow updates`,
...
describe("regression: issue #252 polyrepo Git context", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-polyrepo-git-"));
    const scriptsDir = path.join(tmpDir, ".devflow", "scripts");
    for (const [relativePath, content] of getAllScripts()) {
      const absPath = path.join(scriptsDir, relativePath);
      fs.mkdirSync(path.dirname(absPath), { recursive: true });
  ...
  function writeConfigYaml(content: string): void {
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", "config.yaml"),
      content,
      "utf-8",
    );
  }
...
describe("regression: current-task path normalization", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-current-task-"));
  });
  ...
  function writeLegacyCurrentTask(taskRef: string): void {
    writeProjectFile(path.join(".devflow", ".current-task"), `${taskRef}\n`);
  }

  function writeSessionContext(contextKey: string, taskRef: string): void {
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", `${contextKey}.json`),
      JSON.stringify(
        {
  ...
  function setupTaskRepo(): void {
    writeDevFlowScripts();
    writeProjectFile(
      path.join(".devflow", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-27T00:00:00\n",
    );
    writeProjectFile(path.join(".devflow", "workflow.md"), "# Workflow\n");
    writeProjectFile(
      path.join(".devflow", "spec", "guides", "index.md"),
      "# Guides\n",
    );
    writeProjectFile(
      path.join(".devflow", "tasks", "issue-106", "task.json"),
      JSON.stringify(
        {
          title: "Issue 106 task",
          status: "in_progress",
    ...
    writeProjectFile(
      path.join(".devflow", "tasks", "issue-106", "implement.jsonl"),
  ...
  it("[session-current-task] task.py start without context key enters degraded mode (returns 0, no pointer)", () => {
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".devflow\\\\tasks\\\\issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv(),
    ...
    expect(output).toContain("DEVFLOW_CONTEXT_ID");

    // No active-task pointer written
    expect(
      fs.existsSync(path.join(tmpDir, ".devflow", ".current-task")),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".devflow", ".runtime")),
    ).toBe(false);
  ...
  it("[session-current-task] task.py start writes session runtime state when DEVFLOW_CONTEXT_ID is set", () => {
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".devflow\\\\tasks\\\\issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "session-a" }),
      },
    );
    ...
    expect(context.current_task).toBe(".devflow/tasks/issue-106");
    expect(
      fs.existsSync(path.join(tmpDir, ".devflow", ".current-task")),
    ).toBe(false);
  });
  ...
  it("[session-current-task] task.py finish deletes the session runtime context", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".devflow/tasks/issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "session-finish" }),
      },
    );
  ...
  it("[workflow-state-r7] task.py create auto-sets session pointer when DEVFLOW_CONTEXT_ID is set (planning breadcrumb reachable)", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} create "r7-auto-active" --slug r7-auto --assignee test-dev`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "r7-session" }),
      },
    );

    // Resolve the new task directory (MM-DD-r7-auto)
    const taskDir = fs
      .readdirSync(path.join(tmpDir, ".devflow", "tasks"))
    ...
    expect(context.current_task).toBe(`.devflow/tasks/${taskDir}`);
  });
  ...
  it("[workflow-state-r7] task.py create degrades silently without session identity (no .runtime side effect)", () => {
    ...
    const sessionsDir = path.join(tmpDir, ".devflow", ".runtime", "sessions");
  ...
  it("[workflow-state-r7] task.py create then task.py start is idempotent (pointer + status flip)", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} create "r7-idem" --slug r7-idem --assignee test-dev`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "r7-idem-session" }),
      },
    );
    ...
    expect(taskDir).toBeDefined();
    const relTaskDir = path.posix.join(".devflow", "tasks", taskDir as string);
  ...
  it("[session-current-task] task.py archive deletes runtime sessions pointing at the archived task", () => {
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "session-a.json"),
      JSON.stringify({ current_task: ".devflow/tasks/issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "session-b.json"),
      JSON.stringify({ current_task: "issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "session-other.json"),
      JSON.stringify({ current_task: ".devflow/tasks/other-task" }, null, 2),
    );
  ...
  it("[task-lifecycle] task.py create refuses an archived task dir-name collision", () => {
    ...
    const env = sessionEnv({ DEVFLOW_CONTEXT_ID: "archive-collision" });
    ...
    const tasksDir = path.join(tmpDir, ".devflow", "tasks");
    ...
    expect(result.stderr).toContain(".devflow/tasks/archive/");
  ...
  it("[task-input-contract] task.py archive accepts task name, relative path, and absolute path", () => {
    ...
    for (const name of taskNames) {
      writeProjectFile(
        path.join(".devflow", "tasks", name, "task.json"),
        JSON.stringify(
          {
            title: `Task ${name}`,
            status: "in_progress",
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} archive ${JSON.stringify(`.devflow/tasks/${taskNames[1]}`)} --no-commit`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv(),
    ...
    const absPath = path.join(tmpDir, ".devflow", "tasks", taskNames[2]);
    execSync(
    ...
    for (const name of taskNames) {
      expect(
        fs.existsSync(path.join(tmpDir, ".devflow", "tasks", name)),
        `task ${name} should no longer exist in active tasks/`,
      ).toBe(false);
    }

    // All three should appear under archive/<YYYY-MM>/.
    const archiveRoot = path.join(tmpDir, ".devflow", "tasks", "archive");
  ...
  it("[session-current-task] task.py finish ignores legacy .current-task when no session task is set", () => {
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} finish`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "session-fallback" }),
      },
    );
  ...
  it("[session-current-task] stale session task does not fall back to legacy .current-task", () => {
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "session-b.json"),
      JSON.stringify(
        { current_task: ".devflow/tasks/missing-task", platform: "test" },
        null,
        2,
      ),
    );
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} current --source`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ DEVFLOW_CONTEXT_ID: "session-b" }),
      },
    );

    expect(output).toContain("Current task: .devflow/tasks/missing-task");
  ...
  it("[session-current-task] Python session-start hooks resolve session backslash refs without stale pointer", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".devflow\\tasks\\issue-106");
    writeSessionContext("codex_session-a", ".devflow\\tasks\\issue-106");
  ...
  it("[session-current-task] Claude SessionStart persists DEVFLOW_CONTEXT_ID for Bash commands", () => {
    setupTaskRepo();
    ...
    expect(fs.readFileSync(envFile, "utf-8")).toContain(
      "export DEVFLOW_CONTEXT_ID=claude_bash-start-a",
    );
  });
  ...
  it("[session-current-task] Cursor beforeShellExecution bridges conversation_id into task.py shell commands", () => {
    ...
    const hookOutput = runPython(
      ...
      JSON.stringify({
        ...
        generation_id: "gen-a",
        cwd: tmpDir,
        command: `${pythonCmd} ./.devflow/scripts/task.py start .devflow/tasks/issue-106 && ${pythonCmd} ./.devflow/scripts/task.py current --source`,
    ...
    expect(currentOutput).toContain("Current task: .devflow/tasks/issue-106");
  ...
  it("[session-current-task] Cursor preToolUse injects context for custom Task subagents", () => {
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "cursor_parent-a.json"),
      JSON.stringify(
        {
          current_task: ".devflow/tasks/issue-106",
          current_run: null,
    ...
    expect(prompt).toContain(
      "=== .devflow/tasks/issue-106/prd.md (Requirements) ===",
    );
    expect(prompt).toContain("TOKEN_CURSOR_HOOK_TEST");
    expect(parsed.hookSpecificOutput?.updatedInput?.prompt).toBe(prompt);
  });

  it("[session-current-task] Cursor generic subagents do not receive DevFlow jsonl injection", () => {
    setupTaskRepo();
    writeProjectFile(path.join(".git", "HEAD"), "ref: refs/heads/main\n");
    const injectSubagentContextScript = getSharedHookScripts().find(
      (hook) => hook.name === "inject-subagent-context.py",
  ...
  it("[session-current-task] Cursor hook uses conversation_id when transcript_path is null", () => {
    ...
    writeProjectFile(
      path.join(".devflow", "tasks", "cursor-task", "task.json"),
      JSON.stringify(
        {
          id: "cursor-task",
          title: "Cursor task",
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "cursor_cursor-a.json"),
      JSON.stringify(
        {
          current_task: ".devflow/tasks/cursor-task",
          platform: "cursor",
        },
        null,
        2,
  ...
  it("[session-current-task] OpenCode resolver ignores legacy .current-task and uses plugin sessionID", () => {
    ...
    writeLegacyCurrentTask(".devflow/tasks/issue-106");
    writeProjectFile(
      path.join(".devflow", "tasks", "opencode-task", "task.json"),
      JSON.stringify(
        {
          title: "OpenCode task",
          status: "in_progress",
    ...
    writeProjectFile(
      ...
      JSON.stringify(
        {
          current_task: ".devflow/tasks/opencode-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    const ctx = new DevFlowContext(tmpDir);
    ...
    expect(none.taskPath).toBe(".devflow/tasks/opencode-task");
    ...
    expect(active.taskPath).toBe(".devflow/tasks/opencode-task");
  ...
  it("[session-current-task] OpenCode resolver prefers OPENCODE_RUN_ID over plugin sessionID", () => {
    setupTaskRepo();
    writeProjectFile(
      path.join(".devflow", "tasks", "opencode-run-task", "task.json"),
      JSON.stringify(
        {
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "opencode_run-a.json"),
      JSON.stringify(
        {
          current_task: ".devflow/tasks/opencode-run-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "opencode_oc-a.json"),
      JSON.stringify(
        {
          current_task: ".devflow/tasks/issue-106",
          platform: "opencode",
    ...
    try {
      const active = new DevFlowContext(tmpDir).getActiveTask({
        sessionID: "oc-a",
      });

      expect(active.source).toBe("session:opencode_run-a");
      expect(active.taskPath).toBe(".devflow/tasks/opencode-run-task");
      expect(active.stale).toBe(false);
    } finally {
  ...
  it("[session-start-proof] shared and Codex contexts include one-shot first-reply notice without changing payload shape", () => {
    ...
    for (const payload of [sharedPayload, codexPayload]) {
      ...
      expect(ctx).toMatch(/first visible assistant reply|First visible reply|DevFlow SessionStart 已注入/);
  ...
  it("[#240] Codex SessionStart output uses compact context without generic sub-agent notice", () => {
    ...
    expect(ctx).toContain("DevFlow compact SessionStart context");
  ...
  it("[#248] Copilot template does not assert Copilot ignores SessionStart hook output", () => {
    ...
    // injection mechanism. The previous DevFlow hook hardcoded a misleading
    // "currently ignores" claim in both the docstring and the runtime
    // systemMessage. Both must stay removed; DevFlow should not re-introduce
    ...
    expect(content).not.toContain("DevFlow context injected");
  ...
  it("[workflow-v2] shared session-start summarizes in-progress context without auto-dispatch approval", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".devflow/tasks/issue-106");
  ...
  it("[devflow-hooks-env] runtime: shared hooks emit no additionalContext when DEVFLOW_HOOKS=0", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".devflow/tasks/issue-106");
    ...
    // With DEVFLOW_HOOKS=0: shared hooks short-circuit with empty stdout
    ...
    // DEVFLOW_DISABLE_HOOKS=1 has the same effect
    const gatedAlt = runPython(
      path.join(".claude", "hooks", "session-start.py"),
      stdinPayload,
      { DEVFLOW_DISABLE_HOOKS: "1" },
    );
  ...
  it("[session-current-task] OpenCode context layer normalizes backslash refs for downstream plugins", () => {
    setupTaskRepo();
    writeSessionContext("opencode_oc-a", ".devflow\\tasks\\issue-106");

    const ctx = new DevFlowContext(tmpDir) as DevFlowContext & {
    ...
    expect(ctx.resolveTaskDir(".devflow\\tasks\\issue-106")).toBe(
      path.join(tmpDir, ".devflow", "tasks", "issue-106"),
    );
  });
  ...
  it("[session-fallback] single session file — fallback returns its task with session-fallback source", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_parent", ".devflow/tasks/issue-106");

    const { output, status } = runTaskCurrent();
    expect(status).toBe(0);
    expect(output).toContain("Current task: .devflow/tasks/issue-106");
  ...
  it("[session-fallback] multiple session files — refuses to guess, returns none", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_a", ".devflow/tasks/issue-106");
    writeProjectFile(
      path.join(".devflow", "tasks", "other-task", "task.json"),
      JSON.stringify({ title: "other", status: "in_progress" }, null, 2),
    );
    writeSessionContext("codex_session_b", ".devflow/tasks/other-task");
  ...
  it("[session-fallback] explicit context-key match takes precedence over fallback", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_explicit", ".devflow/tasks/issue-106");

    const { output, status } = runTaskCurrent({
      DEVFLOW_CONTEXT_ID: "codex_session_explicit",
    });
    expect(status).toBe(0);
    expect(output).toContain("Current task: .devflow/tasks/issue-106");
  ...
  function writeWorkflowStateHook(): void {
    writeProjectFile(
      path.join(".devflow", "hooks", "inject-workflow-state.py"),
  ...
  function writeWorkflowMd(body: string): void {
    writeProjectFile(path.join(".devflow", "workflow.md"), body);
  }
  ...
  it("[workflow-state] missing/empty workflow.md degrades to generic line (post-R5: no fallback dict)", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".devflow/tasks/issue-106");
    writeWorkflowStateHook();
    ...
    expect(parsed.hookSpecificOutput.additionalContext).not.toContain(
      "devflow-implement → devflow-check",
    );
  });
  ...
  it("[workflow-state] CWD drift: hook finds .devflow/ when invoked from subdirectory", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".devflow/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] no_task breadcrumb emitted when no session active task exists", () => {
    writeDevFlowScripts();
    writeProjectFile(path.join(".devflow", ".developer"), "name=test\n");
    ...
    // routing to devflow-brainstorm content surfaces.
    writeProjectFile(
      path.join(".devflow", "workflow.md"),
      "[workflow-state:no_task]\n" +
        "No active task. Load `devflow-brainstorm` skill to start.\n" +
        "[/workflow-state:no_task]\n",
    );
    writeLegacyCurrentTask(".devflow/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] silent exit 0 when not a DevFlow project (no .devflow/ dir)", () => {
    // No .devflow/ at all — hook should silently exit
    writeWorkflowStateHook();
    fs.rmSync(path.join(tmpDir, ".devflow"), { recursive: true, force: true });
    fs.mkdirSync(path.join(tmpDir, ".devflow", "hooks"), { recursive: true });
    fs.copyFileSync(
      path.join(
        __dirname,
        "..",
      ...
      path.join(tmpDir, ".devflow", "hooks", "inject-workflow-state.py"),
    );
    // Now .devflow/ exists only as a parent for the hook script — need to move
    // the hook out of .devflow/ so root-finding fails. Use a fully separate dir.
    const nonDevFlowDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "non-devflow-"),
    );
    try {
      const hookPath = path.join(nonDevFlowDir, "hook.py");
      fs.copyFileSync(
        path.join(
          __dirname,
          "..",
      ...
      const result = execSync(`${pythonCmd} ${JSON.stringify(hookPath)}`, {
        cwd: nonDevFlowDir,
        input: JSON.stringify({ cwd: nonDevFlowDir }),
        encoding: "utf-8",
      });
      expect(result.trim()).toBe("");
    } finally {
      fs.rmSync(nonDevFlowDir, { recursive: true, force: true });
    }
  });
  ...
  it("[init-context-removal] task.py init-context is deprecated with clear pointer to planning artifacts", () => {
    ...
    try {
      execSync(
        `${pythonCmd} ${JSON.stringify(taskScriptPath)} init-context .devflow/tasks/issue-106 fullstack`,
        { cwd: tmpDir, encoding: "utf-8" },
      );
    } catch (err) {
      threw = true;
...
`;
  ...
  it("[issue-237] all implement/check agent templates contain recursion guards", () => {
    ...
    const agentFiles = [
      "claude/agents/devflow-implement.md",
      "claude/agents/devflow-check.md",
      "codebuddy/agents/devflow-implement.md",
      "codebuddy/agents/devflow-check.md",
      "codex/agents/devflow-implement.toml",
      "codex/agents/devflow-check.toml",
      "cursor/agents/devflow-implement.md",
      "cursor/agents/devflow-check.md",
      "gemini/agents/devflow-implement.md",
      "gemini/agents/devflow-check.md",
      "kiro/agents/devflow-implement.json",
      "kiro/agents/devflow-check.json",
      "opencode/agents/devflow-implement.md",
      "opencode/agents/devflow-check.md",
      "pi/agents/devflow-implement.md",
      "pi/agents/devflow-check.md",
      "qoder/agents/devflow-implement.md",
      "qoder/agents/devflow-check.md",
    ];
    ...
    for (const relativePath of agentFiles) {
      ...
      if (relativePath.includes("implement")) {
        expect(content, `${relativePath} should forbid nested implement`).toContain(
          "spawn another `devflow-implement`",
        );
  ...
  it("[issue-241-followup] codex sub-agent toml files disable collab tools at protocol level", () => {
    ...
    const codexAgentFiles = [
      "codex/agents/devflow-implement.toml",
      "codex/agents/devflow-check.toml",
      "codex/agents/devflow-research.toml",
    ];
  ...
  it("[workflow-v2] --mode phase --platform codex (sub-agent mode) filters out generic before-dev routing", () => {
    ...
    // explicitly so the legacy spawn-devflow-implement block surfaces.
    ...
    expect(output).toContain("devflow-implement");
    expect(output).not.toContain(
      "| About to write code / start implementing | devflow-before-dev |",
    );
  ...
  it("[workflow-v2] step 2.1 for codex (sub-agent mode) describes self-loaded agent context, not hook injection", () => {
    ...
    expect(output).not.toContain("Load the `devflow-before-dev` skill");
  });
  ...
  it("[workflow-v2] --mode phase --platform kilo keeps devflow-before-dev routing (agent-less path)", () => {
    // Symmetric to the codex filter test: agent-less platforms MUST still
    // see `devflow-before-dev` because they write code in the main session.
    writeDevFlowScripts();
    writeProjectFile(path.join(".devflow", ".developer"), "name=test\n");
    writeProjectFile(
      path.join(".devflow", "workflow.md"),
    ...
    expect(output).not.toContain("Dispatch the `devflow-implement` sub-agent");
  });

  // ------------------------------------------------------------
  // session-start.py <devflow-workflow> + <guidelines> compact context
  // ------------------------------------------------------------

  it("[workflow-v2] session-start.py <devflow-workflow> block contains compact Phase Index", () => {
    writeDevFlowScripts();
    writeProjectFile(path.join(".devflow", ".developer"), "name=test\n");
    writeProjectFile(
      path.join(".devflow", "workflow.md"),
    ...
    const workflowMatch = /<devflow-workflow>([\s\S]*?)<\/devflow-workflow>/.exec(ctx);
  ...
  it("[workflow-v2] session-start.py <guidelines> block lists context order and spec paths", () => {
    ...
    writeProjectFile(
      path.join(".devflow", "spec", "cli", "backend", "index.md"),
    ...
    expect(guidelinesBlock).toContain(".devflow/spec/cli/backend/index.md");
  ...
  it("[workflow-v2] inject-subagent-context.py does NOT write current_phase when implement spawns", () => {
    ...
    writeDevFlowScripts();
    writeProjectFile(path.join(".devflow", ".developer"), "name=test\n");
    writeProjectFile(path.join(".devflow", "workflow.md"), "# Minimal\n");
    // Session active task WITHOUT current_phase field (post-migration state)
    writeProjectFile(
      path.join(".devflow", ".runtime", "sessions", "claude_phase-a.json"),
      JSON.stringify(
        {
          current_task: ".devflow/tasks/issue-106",
          platform: "claude",
    ...
    const taskJson = JSON.parse(
      fs.readFileSync(
        path.join(tmpDir, ".devflow", "tasks", "issue-106", "task.json"),
        "utf-8",
      ),
  ...
  function writeConfigYaml(content: string): void {
    writeProjectFile(path.join(".devflow", "config.yaml"), content);
  }
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb defaults to inline dispatch when config absent", () => {
    ...
    writeProjectFile(
      path.join(".devflow", "workflow.md"),
      "[workflow-state:in_progress]\n" +
        "DISPATCH the devflow-implement / devflow-check sub-agents.\n" +
        "[/workflow-state:in_progress]\n" +
        "[workflow-state:in_progress-inline]\n" +
        "MAIN SESSION edits code via devflow-before-dev directly.\n" +
        "[/workflow-state:in_progress-inline]\n",
    );
    ...
    expect(ctx).not.toContain("DISPATCH the devflow-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to plain status when codex.dispatch_mode=sub-agent", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("DISPATCH the devflow-implement");
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to inline tag when codex.dispatch_mode=inline", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("MAIN SESSION edits code");
    expect(ctx).toContain("devflow-before-dev");
    expect(ctx).not.toContain("DISPATCH the devflow-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] inline `#` comment after value is stripped (config.yaml uncomment leaves trailing hint)", () => {
    ...
    writeDevFlowScripts();
    writeProjectFile(
      path.join(".devflow", "hooks", "devflow_config.py"),
      expectTemplateContent(
        getAllScripts().get("common/devflow_config.py") ?? "",
        "devflow_config",
      ),
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "from pathlib import Path",
        `hook_path = Path(${JSON.stringify(
          path.join(tmpDir, ".devflow", "hooks", "devflow_config.py"),
        )})`,
  ...
  it("[issue-codex-dispatch-mode] resolve_effective_platform namespaces codex into codex-sub-agent / codex-inline", () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import sys, json",
        `sys.path.insert(0, ${JSON.stringify(path.join(tmpDir, ".devflow", "scripts"))})`,
  ...
  it("[issue-codex-dispatch-mode] codex hook injects <codex-mode> banner reflecting dispatch_mode", () => {
    ...
    writeProjectFile(
      path.join(".devflow", "workflow.md"),
      "[workflow-state:in_progress]\nDISPATCH the devflow-implement.\n[/workflow-state:in_progress]\n[workflow-state:in_progress-inline]\nMAIN SESSION inline edit.\n[/workflow-state:in_progress-inline]\n",
    );
    ...
    expect(subAgentRun.hookSpecificOutput.additionalContext).toContain(
      "<codex-mode>sub-agent: implement/check work defaults to DevFlow sub-agents; the main session still coordinates, clarifies, updates specs, commits, and finishes.</codex-mode>",
    );
  });
...
describe("regression: cli_adapter platform support (beta.9, beta.13, beta.16)", () => {
  ...
  it("[pi] cli_adapter.py supports pi platform", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".pi/prompts/devflow-{name}.md"',
    );
  });
  ...
  it("[droid] cli_adapter.py treats droid as commands-only (no CLI run/resume yet)", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".factory/commands/devflow/{name}.md"',
    );
  });

  it("[droid] cli_adapter.py has explicit droid branches in all key methods", () => {
    expect(commonCliAdapter).toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "droid":[\s\S]*?\.factory\/commands\/devflow\//,
    );
    expect(commonCliAdapter).toMatch(
  ...
  // get_devflow_command_path were missing the `devflow-` prefix that
  // 0.5.0-beta.0 introduced via 60+ rename manifest entries. Without the
  // prefix, any caller that built skill paths via get_devflow_command_path
  // (add-context, check agent prelude, etc.) would produce paths that don't
  // resolve to any real skill file.
  it("[migrate-flow-bugs] get_devflow_command_path codex branch uses devflow- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "codex":[\s\S]*?return f"\.agents\/skills\/devflow-\{name\}\/SKILL\.md"/,
    );
    expect(commonCliAdapter).not.toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "codex":[\s\S]*?return f"\.agents\/skills\/\{name\}\/SKILL\.md"/,
    );
  });

  it("[migrate-flow-bugs] get_devflow_command_path kiro branch uses devflow- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "kiro":[\s\S]*?return f"\.kiro\/skills\/devflow-\{name\}\/SKILL\.md"/,
    );
    expect(commonCliAdapter).not.toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "kiro":[\s\S]*?return f"\.kiro\/skills\/\{name\}\/SKILL\.md"/,
    );
  });
  ...
  it("[migrate-flow-bugs] detect_platform has codex shared-skills fallback guarded by no-other-platform-dir check", () => {
    // Fallback fires when .agents/skills/devflow-* exists AND no other
    ...
    expect(commonCliAdapter).toMatch(/entry\.name\.startswith\("devflow-"\)/);
  });
  ...
  it("[migrate-flow-bugs] 0.5.0-beta.0 manifest has migrationGuide + aiInstructions (back-filled)", () => {
    ...
    expect(manifest.migrationGuide).toMatch(/devflow-/); // skill renames
  ...
  it("[copilot] cli_adapter.py has explicit copilot branches in all key methods", () => {
    ...
    expect(commonCliAdapter).toMatch(
      /def get_devflow_command_path[\s\S]*?elif self\.platform == "copilot":[\s\S]*?\.github\/prompts\//,
    );
    expect(commonCliAdapter).toMatch(
...
describe("regression: migration manifest consistency", () => {
  ...
  it("[#57] shell archive migrations use rename type with correct from/to paths", () => {
    ...
    for (const m of shellArchives) {
      ...
      expect(m.from).toMatch(/\.devflow\/scripts\/.*\.sh$/);
      expect(m.to).toMatch(/\.devflow\/scripts-shell-archive\/.*\.sh$/);
  ...
  it("[0.2.14] command namespace migration renames exist", () => {
    ...
    // Should include commands moved to devflow/ subdirectory
  ...
  describe("[0.5.0-beta.0] command→skill rename coverage", () => {
    ...
    const PLATFORMS: { id: string; from: PathFn; to: PathFn }[] = [
      {
        id: "claude",
        from: (n) => `.claude/commands/devflow/${n}.md`,
        to: (n) => `.claude/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "cursor",
        from: (n) => `.cursor/commands/devflow-${n}.md`,
        to: (n) => `.cursor/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "opencode",
        from: (n) => `.opencode/commands/devflow/${n}.md`,
        to: (n) => `.opencode/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "codebuddy",
        from: (n) => `.codebuddy/commands/devflow/${n}.md`,
        to: (n) => `.codebuddy/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "droid",
        from: (n) => `.factory/commands/devflow/${n}.md`,
        to: (n) => `.factory/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "gemini",
        from: (n) => `.gemini/commands/devflow/${n}.toml`,
        to: (n) => `.gemini/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "copilot",
        from: (n) => `.github/prompts/${n}.prompt.md`,
        to: (n) => `.github/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "kilo",
        from: (n) => `.kilocode/workflows/${n}.md`,
        to: (n) => `.kilocode/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "antigravity",
      ...
      },
      {
        ...
        to: (n) => `.kiro/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "qoder",
        from: (n) => `.qoder/skills/${n}/SKILL.md`,
        to: (n) => `.qoder/skills/devflow-${n}/SKILL.md`,
      },
      {
        id: "shared",
        from: (n) => `.agents/skills/${n}/SKILL.md`,
        to: (n) => `.agents/skills/devflow-${n}/SKILL.md`,
      },
    ];
    ...
    it("has at least 65 rename entries for command→skill (13 platforms × 5 commands)", () => {
      ...
      // relocation under devflow- namespace on skill-only platforms).
    ...
    it("breaking + recommendMigrate flags are both set (drives gate)", () => {
      ...
      // by running `devflow update` without `--migrate`.
...
describe("regression: collectTemplates paths match init directory structure (0.3.1)", () => {
  it("[0.3.1] all platforms with commands use consistent devflow/ subdirectory", () => {
    ...
    for (const id of platformsWithCommands) {
      ...
      for (const key of commandKeys) {
        expect(
          key,
          `${id} command path should include devflow/ subdirectory: ${key}`,
        ).toContain("/commands/devflow/");
      }
    }
  });
  ...
  it("[copilot] collectTemplates tracks hooks and VS Code discovery config", () => {
    ...
    expect(keys).toContain(".github/hooks/devflow.json");
  });
});
...
describe("regression: parse_simple_yaml uses _unquote not greedy strip (0.3.8)", () => {
  ...
  it("config.py uses _unquote for key-value, not .strip('\"')", () => {
    ...
    // mirrors devflow_config.py so YAML `key: false  # comment` parses
...
describe("regression: class-2 platforms use pull-based sub-agent context", () => {
  ...
  const class2 = [
    {
      ...
      nonPreludeAgents: [".qoder/agents/devflow-research.md"],
    },
    {
      id: "gemini" as const,
      ...
      nonPreludeAgents: [".gemini/agents/devflow-research.md"],
    },
    {
      id: "codex" as const,
      ...
      nonPreludeAgents: [".codex/agents/devflow-research.toml"],
    },
    {
      id: "copilot" as const,
      hooksDir: ".github/copilot/hooks",
      preludeAgents: [
        ".github/agents/devflow-implement.agent.md",
        ".github/agents/devflow-check.agent.md",
      ],
      nonPreludeAgents: [".github/agents/devflow-research.agent.md"],
    },
  ];
  ...
  for (const { id, hooksDir, preludeAgents, nonPreludeAgents } of class2) {
    describe(`[${id}]`, () => {
      ...
      beforeEach(async () => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `devflow-c2-${id}-`));
        setWriteMode("force");
      ...
      it("research definition does NOT contain pull-based prelude", () => {
        // research is orthogonal: it searches .devflow/spec/ and doesn't
        ...
        for (const file of nonPreludeAgents) {
          const content = fs.readFileSync(path.join(tmpDir, file), "utf-8");
          expect(content).not.toContain("Required: Load DevFlow Context First");
        }
      });
...
describe("regression: copilot agents use YAML tools frontmatter", () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-copilot-tools-"));
    setWriteMode("force");
  ...
  it("collectPlatformTemplates matches written Copilot agent output", () => {
    ...
    const generated = fs.readFileSync(
      path.join(tmpDir, ".github/agents/devflow-check.agent.md"),
      "utf-8",
    );
    expect(templates?.get(".github/agents/devflow-check.agent.md")).toBe(
      generated,
    );
  });
});
...
describe("regression: pi uses TypeScript extension assets instead of Python hooks", () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-pi-"));
    setWriteMode("force");
  ...
  it("installs a subagent-capable extension and pull-based agent context", () => {
    ...
    for (const agent of ["devflow-implement.md", "devflow-check.md"]) {
      const content = fs.readFileSync(
...
describe("regression: research agent persists findings to task dir", () => {
  ...
  // platforms accept (Zod expects an array or omission). DevFlow omits the
  ...
  it("[packages/cli/src/templates/gemini/agents/devflow-research.md] omits tools line + has persist instruction", () => {
    const rel = "packages/cli/src/templates/gemini/agents/devflow-research.md";
    const content = fs.readFileSync(path.join(repoRoot, rel), "utf-8");
    const fm = content.split("---\n")[1] ?? "";
  ...
  it("codex research.toml uses workspace-write sandbox and persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/codex/agents/devflow-research.toml",
      ),
      "utf-8",
    );
  ...
  it("kiro research.json includes write tool and persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/kiro/agents/devflow-research.json",
      ),
      "utf-8",
    );
  ...
  it("opencode research.md grants write/edit permission and has persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/opencode/agents/devflow-research.md",
      ),
      "utf-8",
    );
    const fm = content.split("---\n")[1] ?? "";
...
describe("regression: templates/markdown/spec contains only .md.txt files (0.5.0-beta.9)", () => {
  ...
  // .devflow/spec/cli/backend/directory-structure.md "Don't: Leak dogfood
...
describe("regression: Gemini CLI 0.40.x template compatibility (#224)", () => {
  ...
  it("[#224] gemini agent .md files do NOT carry a comma-separated tools line", () => {
    ...
    // "tools: Expected array, received string". DevFlow omits the line so
  ...
  it("[#224] configurePlatform('gemini') writes shared skills to .agents/skills, NOT .gemini/skills", async () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "devflow-gemini-issue224-"),
    );
    try {
      setWriteMode("force");
  ...
  it("[devflow-hooks-env] all hook templates honor DEVFLOW_HOOKS=0 / DEVFLOW_DISABLE_HOOKS=1", () => {
    // All shipped hook scripts must early-return when the operator sets
    // DEVFLOW_HOOKS=0 (or DEVFLOW_DISABLE_HOOKS=1), so subprocess wrappers
    // and casual-chat scenarios can disable DevFlow injection without
    ...
    for (const name of sharedHookTargets) {
      ...
      expect(script).toContain('os.environ.get("DEVFLOW_HOOKS") == "0"');
      expect(script).toContain('os.environ.get("DEVFLOW_DISABLE_HOOKS") == "1"');
    }
    ...
    for (const [label, hooks] of [
      ...
      expect(sessionStart).toContain('os.environ.get("DEVFLOW_HOOKS") == "0"');
      expect(sessionStart).toContain('os.environ.get("DEVFLOW_DISABLE_HOOKS") == "1"');
    }
    ...
    for (const name of jsPlugins) {
      ...
      expect(content).toContain('process.env.DEVFLOW_HOOKS === "0"');
      expect(content).toContain('process.env.DEVFLOW_DISABLE_HOOKS === "1"');
    }
  });
...
describe("regression: session-start.py f-string Python <=3.11 compat (0.5.2)", () => {
  ...
  // of an f-string. DevFlow 0.5.0/0.5.1 shipped session-start hooks with
...
describe("regression: sub-agent context injection fallback (0.5.3)", () => {
  ...
  //
  // The fix: hook output now begins with a `<!-- devflow-hook-injected -->`
  // marker, and every class-1 devflow-implement / devflow-check definition
  // file carries a DevFlow Context Loading Protocol section telling the
  // sub-agent to load context itself when the marker is absent.
  const HOOK_INJECTED_MARKER = "<!-- devflow-hook-injected -->";
  ...
  it("inject-subagent-context.py emits the marker for implement / check / finish", () => {
    ...
    const matches = src.match(/<!--\s*devflow-hook-injected\s*-->/g) ?? [];
  ...
  const CLASS1_MD_AGENT_FILES: { platform: string; rel: string; agent: "implement" | "check" }[] = [
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/devflow-implement.md", agent: "implement" },
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/devflow-check.md", agent: "check" },
    { platform: "cursor", rel: "packages/cli/src/templates/cursor/agents/devflow-implement.md", agent: "implement" },
    { platform: "cursor", rel: "packages/cli/src/templates/cursor/agents/devflow-check.md", agent: "check" },
    { platform: "codebuddy", rel: "packages/cli/src/templates/codebuddy/agents/devflow-implement.md", agent: "implement" },
    { platform: "codebuddy", rel: "packages/cli/src/templates/codebuddy/agents/devflow-check.md", agent: "check" },
    { platform: "opencode", rel: "packages/cli/src/templates/opencode/agents/devflow-implement.md", agent: "implement" },
    { platform: "opencode", rel: "packages/cli/src/templates/opencode/agents/devflow-check.md", agent: "check" },
    { platform: "droid", rel: "packages/cli/src/templates/droid/droids/devflow-implement.md", agent: "implement" },
    { platform: "droid", rel: "packages/cli/src/templates/droid/droids/devflow-check.md", agent: "check" },
  ];
  ...
  for (const { platform, rel, agent } of CLASS1_MD_AGENT_FILES) {
    it(`${platform}/${agent} markdown agent file carries marker + fallback protocol`, () => {
      ...
      expect(content).toContain("DevFlow Context Loading Protocol");
  ...
  for (const agent of ["implement", "check"] as const) {
    it(`kiro/${agent} JSON agent carries marker + fallback protocol in prompt`, () => {
      ...
      const filePath = path.join(
        repoRootFb,
        `packages/cli/src/templates/kiro/agents/devflow-${agent}.json`,
      );
      const json = JSON.parse(fs.readFileSync(filePath, "utf-8"));
      const prompt: string = json.prompt ?? "";
      expect(prompt).toContain(HOOK_INJECTED_MARKER);
      expect(prompt).toContain("DevFlow Context Loading Protocol");
  ...
  const GEMINI_QODER_AGENT_FILES = [
    "packages/cli/src/templates/gemini/agents/devflow-implement.md",
    "packages/cli/src/templates/gemini/agents/devflow-check.md",
  ...
  for (const agent of ["implement", "check"] as const) {
    it(`pi/${agent} agent references task artifacts`, () => {
      const content = fs.readFileSync(
        path.join(repoRootFb, `packages/cli/src/templates/pi/agents/devflow-${agent}.md`),
        "utf-8",
      );
      expectTaskArtifactContract(content);
    });
  ...
  it("[issue-247] kiro agent JSON files use Kiro CLI's current schema (prompt / hooks-object)", () => {
    // Kiro CLI rejected DevFlow's pre-0.5.7 agent JSON with "invalid agent"
  ...
  it("workflow.md dispatch protocol covers all platforms (not class-2 only)", () => {
    ...
    // devflow-research carve-out as of 0.5.8 — research sub-agents need the
    ...
    expect(wf).not.toContain("EXCEPT devflow-research");
    expect(wf).toContain("devflow-research");
...
describe("regression: configSectionsAdded (issue-codex-dispatch-mode)", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-config-section-"));
  });
  ...
  it("[config-sections] applyConfigSectionsAdded appends section when sentinel missing, idempotent on rerun", async () => {
    ...
    const userConfigPath = path.join(devflowDir, "config.yaml");
    const userConfig = [
  ...
  it("[config-sections] applyConfigSectionsAdded skips when target file does not exist", async () => {
    ...
    const result = applyConfigSectionsAdded(
      ...
      tmpDir,
      new Map<string, string>([[".devflow/config.yaml", "# fake template"]]),
    );
  ...
  it("[config-sections] manifest 0.5.7 declares the codex dispatch_mode section", () => {
    ...
    expect(entry?.file).toBe(".devflow/config.yaml");
...
// =============================================================================
// safe-commit: gitignored .devflow/ recovery (0.5.10 → 0.5.11)
// =============================================================================
//
// Real user incident: project .gitignore listed `.devflow/`. add_session.py's
// auto-commit ran `git add .devflow/workspace .devflow/tasks`, got `ignored
// by .gitignore`, fell back to a hint suggesting `git add .devflow &&
// commit`. The AI agent driving the workflow extrapolated that to
// `git add -f .devflow/`, which forced in `.devflow/.backup-*/`,
// `.devflow/worktrees/`, `.devflow/.template-hashes.json`, etc. — 548 files
...
// That auto-`-f` was an over-fix — when a user gitignores `.devflow/` they
// mean "keep .devflow/ local-only", and forcing the commit through (even on
// narrow paths) violates user intent. Group-chat report: a finish-work auto
// committed `.devflow/workspace/` straight into a repo whose .gitignore
// excluded `.devflow/`.
//
// 0.5.11 fix (current):
...
//   - The warning explicitly says ``Do NOT use `git add -f .devflow/```` so
//     AI re-reading the log doesn't reinvent the bug, and points at the new
//     `session_auto_commit: false` knob.
//
// These tests synthesize a tmp git repo with `.devflow/` gitignored and
...
describe("regression: safe auto-commit when .devflow/ is gitignored (0.5.10 → 0.5.11)", () => {
  let tmpDir: string;
  const pyCmd = process.platform === "win32" ? "python" : "python3";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-safe-commit-"));
    execSync("git init -q -b main", { cwd: tmpDir });
    // Configure user so git commit succeeds in CI sandboxes.
    execSync('git config user.email "test@devflow.local"', { cwd: tmpDir });
    execSync('git config user.name "DevFlow Test"', { cwd: tmpDir });
  });
  ...
  function writeWorkspaceIndex(): void {
    writeFile(
      ".devflow/workspace/test-dev/index.md",
      [
  ...
  function setupRepo(options?: { gitignoreDevFlow?: boolean }): void {
    writeDevFlowScripts();
    writeFile(
      ".devflow/.developer",
      "name=test-dev\ninitialized_at=2026-05-09T00:00:00\n",
    );
    writeFile(".devflow/workspace/test-dev/journal-1.md",
      "# Journal - test-dev (Part 1)\n\n---\n",
    );
    writeWorkspaceIndex();
    // Ignored caches/backups must exist on disk to prove they don't get
    // staged when -f is forced on specific paths.
    writeFile(".devflow/.backup-2026-05-09/should-not-be-committed.txt",
      "secret-backup\n",
    );
    writeFile(".devflow/worktrees/wt-a/should-not-be-committed.txt",
      "secret-worktree\n",
    );
    writeFile(".devflow/.template-hashes.json", '{"_": "should-not-be-committed"}\n');
    writeFile(".devflow/.runtime/sessions/should-not-be-committed.json", "{}\n");

    if (options?.gitignoreDevFlow) {
      writeFile(".gitignore", ".devflow/\n");
    }
  ...
  it("[gitignore-devflow] add_session warns and skips when .devflow/ is ignored (default mode)", () => {
    setupRepo({ gitignoreDevFlow: true });
    ...
    expect(stderr).toContain("Do NOT use `git add -f .devflow/`");
    expect(stderr).toContain("session_auto_commit: false");

    // Nothing under .devflow/ should be tracked: the user's .gitignore
    // intent is preserved.
    const tracked = listCommittedFiles();
    for (const tracked_path of tracked) {
      expect(
        tracked_path.startsWith(".devflow/"),
        `should not commit anything under .devflow/ (got: ${tracked_path})`,
      ).toBe(false);
    }
  ...
  it("[gitignore-devflow] add_session works normally when .devflow/ is NOT ignored", () => {
    // Regression guard: pre-existing behavior must not change for users
    // whose .gitignore does not exclude .devflow/.
    setupRepo({ gitignoreDevFlow: false });
    const { stderr } = runAddSession();
    expect(stderr).toContain("Auto-committed");

    const tracked = listCommittedFiles();
    expect(tracked).toContain(".devflow/workspace/test-dev/journal-1.md");
  });

  it("[gitignore-devflow] safe_commit module ships and contains the negative warning + new config hint", () => {
    ...
    expect(safeCommit).toContain("Do NOT use `git add -f .devflow/`");
    expect(safeCommit).toContain("safe_devflow_paths_to_add");
  ...
  it("[gitignore-devflow] task.py archive warns and skips when .devflow/ is ignored (default mode)", () => {
    setupRepo({ gitignoreDevFlow: true });
    // Create a task to archive.
    writeFile(
      ".devflow/tasks/issue-500/task.json",
      JSON.stringify(
    ...
    writeFile(".devflow/tasks/issue-500/prd.md", "# PRD\n");
    ...
    const result = spawnSync(
      ...
      [taskScriptPath, "archive", "issue-500"],
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: { ...process.env, DEVFLOW_CONTEXT_ID: "session-arch" },
      },
    );
    ...
    const tracked = listCommittedFiles();
    // Nothing under .devflow/ should be tracked.
    for (const t of tracked) {
      expect(
        t.startsWith(".devflow/"),
        `should not commit anything under .devflow/ (got: ${t})`,
      ).toBe(false);
    }
    ...
    const archiveExists = fs
      .readdirSync(path.join(tmpDir, ".devflow/tasks/archive"))
      .some((monthDir) => {
        const monthPath = path.join(
          tmpDir,
          ".devflow/tasks/archive",
          monthDir,
        );
        return (
  ...
  function writeConfigYaml(content: string): void {
    writeFile(".devflow/config.yaml", content);
  }
  ...
  it("[session_auto_commit=false] task.py archive skips git entirely", () => {
    ...
    writeFile(
      ".devflow/tasks/issue-600/task.json",
      JSON.stringify(
        { title: "Test archive", status: "in_progress", package: null },
        null,
        2,
    ...
    writeFile(".devflow/tasks/issue-600/prd.md", "# PRD\n");
    ...
    const result = spawnSync(
      ...
      [taskScriptPath, "archive", "issue-600"],
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: { ...process.env, DEVFLOW_CONTEXT_ID: "session-arch-2" },
      },
    );
    const stderr = result.stderr ?? "";
    expect(stderr).not.toContain("Auto-committed");
  ...
  it("[session_auto_commit] inline comment is stripped before parsing", () => {
    ...
    // verifies the helper is shared with devflow_config.py's parser.
    setupRepo({ gitignoreDevFlow: false });
    writeConfigYaml(
