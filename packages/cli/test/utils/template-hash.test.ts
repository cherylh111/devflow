...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-test-"));
    // Create .devflow directory for hashes file
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".devflow", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("returns 0 when no template directories exist", () => {
    // Create .devflow dir for saving hashes but no template files
    fs.mkdirSync(path.join(tmpDir, ".devflow"), { recursive: true });
    const count = initializeHashes(tmpDir);
    expect(count).toBe(0);
  });

  it("hashes files in .devflow/ and tracked platform paths", () => {
    // .devflow/ is always walked recursively. Platform paths (.claude/, etc.)
    ...
    fs.mkdirSync(path.join(tmpDir, ".devflow", "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", "scripts", "task.py"),
      "print('hello')",
    );
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".devflow/scripts/task.py");
  ...
  it("does NOT hash platform-dir files that are not in trackedPaths", () => {
    ...
    // Now: only paths devflow actually wrote (recorded via writeFile) make
  ...
  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".devflow", "workspace"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".devflow", "workspace", "data.md"), "user data");
    fs.mkdirSync(path.join(tmpDir, ".devflow", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".devflow", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".devflow/workspace/data.md");
    expect(hashes).not.toHaveProperty(".devflow/tasks/task.json");
    expect(count).toBe(0);
  });

  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(path.join(tmpDir, ".devflow", "spec", "guides"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".devflow", "spec", "frontend"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".devflow", "spec", "backend"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".devflow", "spec", "guides", "index.md"), "# Guides");
    fs.writeFileSync(path.join(tmpDir, ".devflow", "spec", "frontend", "index.md"), "# Frontend");
    fs.writeFileSync(path.join(tmpDir, ".devflow", "spec", "backend", "index.md"), "# Backend");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // All spec/ files should be excluded
    expect(hashes).not.toHaveProperty(".devflow/spec/guides/index.md");
    expect(hashes).not.toHaveProperty(".devflow/spec/frontend/index.md");
    expect(hashes).not.toHaveProperty(".devflow/spec/backend/index.md");
    expect(count).toBe(0);
  });
  ...
  it("collectFiles returns POSIX-normalized paths (no backslashes)", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".devflow", "scripts", "common"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".devflow", "scripts", "common", "task.py"),
      "print('x')",
    );
    ...
    expect(hashes).toHaveProperty(".devflow/scripts/common/task.py");
  });
  ...
  it("does not exclude generated update-spec skills from hashing", () => {
    ...
    // `.pi/skills/devflow-update-spec/`. The new model doesn't use that
    ...
    const count = initializeHashes(tmpDir, {
      trackedPaths: new Set([".pi/skills/devflow-update-spec/SKILL.md"]),
    });
    const hashes = loadHashes(tmpDir);

    expect(hashes).toHaveProperty(
      ".pi/skills/devflow-update-spec/SKILL.md",
    );
    expect(count).toBe(1);
  });
});
...
describe("cross-platform hash storage (POSIX keys + v2 schema)", () => {
  let tmpDir: string;
  const HASHES_REL = path.join(".devflow", ".template-hashes.json");
  ...
  it("legacy file is discarded then initializeHashes regenerates v2", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, HASHES_REL),
      JSON.stringify({ ".devflow\\config.yaml": "deadbeef" }),
    );
    ...
    expect(parsed.hashes).not.toHaveProperty(".devflow\\config.yaml");
    expect(parsed.hashes).not.toHaveProperty(".devflow/config.yaml");
    // Newly hashed file is present with POSIX key
    expect(parsed.hashes).toHaveProperty(".devflow/scripts/task.py");
  });
