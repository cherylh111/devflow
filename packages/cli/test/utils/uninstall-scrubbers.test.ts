...
describe("scrubHooksJson — nested schema", () => {
  it("strips devflow hook entries from a Claude-style file", () => {
    const input = {
  ...
  it("reports fullyEmpty when only devflow hooks existed", () => {
    const input = {
      hooks: {
        SessionStart: [
          {
...
describe("scrubHooksJson — flat schema", () => {
  it("strips devflow hook entries from a Cursor-style file", () => {
    const input = {
      version: 1,
      hooks: {
        preToolUse: [
...
describe("scrubPiSettings", () => {
  it("strips devflow entries and reports fullyEmpty", () => {
    const input = {
      enableSkillCommands: true,
      extensions: ["./extensions/devflow/index.ts"],
      skills: ["./skills"],
      prompts: ["./prompts"],
      packages: [
        {
  ...
  it("preserves user-added array entries", () => {
    const input = {
      enableSkillCommands: true,
      extensions: ["./extensions/devflow/index.ts", "./extensions/my-ext"],
...
describe("scrubCodexConfigToml", () => {
  const TEMPLATE = `# Project-scoped Codex defaults for DevFlow workflows.
...
`;
  ...
  it("removes the entire devflow-shipped file and reports fullyEmpty", () => {
...
`;
    ...
    expect(content).not.toContain("DevFlow's SessionStart");
  });
...
`;
  ...
  it("strips the new `hooks = true` marker line (Codex 0.129+) alongside the legacy `codex_hooks` line", () => {
    const newTemplate = `# Project-scoped Codex defaults for DevFlow workflows.
# Codex loads this after ~/.codex/config.toml when you work in this project.
