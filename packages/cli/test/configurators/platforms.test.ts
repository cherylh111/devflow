...
describe("getConfiguredPlatforms", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-platforms-"));
  });
...
describe("configurePlatform", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-configure-"));
  ...
  it("configurePlatform writes collected templates byte-for-byte for every platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `devflow-parity-${id}-`),
      );
      try {
  ...
  it("configurePlatform('codex') writes shared skill templates from common source", async () => {
    ...
    // `devflow-start` is included via `resolveAllAsSkillsNeutral` directly —
    // it's the user-invocable fallback referenced by the <devflow-bootstrap>
  ...
  it("configurePlatform('codex') writes custom agents and config", async () => {
    ...
    for (const agent of expectedAgents) {
      ...
      const needsPrelude = ["devflow-implement", "devflow-check"].includes(
        agent.name,
      );
      if (needsPrelude) {
  ...
  it("configurePlatform('kiro') writes main agent, IDE hook, and shared hooks", async () => {
    ...
    // Main `devflow` agent wires per-turn + session-start hooks; PYTHON_CMD
    // resolved.
    const devflowPath = path.join(tmpDir, ".kiro", "agents", "devflow.json");
    expect(fs.existsSync(devflowPath)).toBe(true);
    const devflowRaw = fs.readFileSync(devflowPath, "utf-8");
    expect(devflowRaw).not.toContain("{{PYTHON_CMD}}");
    const devflow = JSON.parse(devflowRaw) as {
      resources?: string[];
      hooks?: Record<string, { command: string }[]>;
    };
    expect(devflow.hooks?.userPromptSubmit?.[0].command).toBe(
      `${expectedPythonCmd} .kiro/hooks/inject-workflow-state.py`,
    );
    expect(devflow.hooks?.agentSpawn?.[0].command).toBe(
  ...
  it("configurePlatform('gemini') writes TOML commands + SKILL.md skills", async () => {
    ...
    // Commands as TOML
    const commandsDir = path.join(tmpDir, ".gemini", "commands", "devflow");
  ...
  it("configurePlatform('qoder') writes commands + skills with the correct split", async () => {
    ...
    expect(actualCommandFiles).toEqual(
      expectedCommands.map((c) => `devflow-${c.name}.md`).sort(),
    );
    ...
    expect(actualSkillDirs).not.toContain("devflow-finish-work");
    expect(actualSkillDirs).not.toContain("devflow-continue");
    expect(actualSkillDirs).not.toContain("devflow-start");
  });
  ...
  it("configurePlatform('zcode') keeps command fallbacks out of shared .agents skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".zcode", "commands", "devflow", "start.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(templates?.has(".zcode/commands/devflow/start.md")).toBe(true);
    expect(templates?.has(".agents/skills/devflow-start/SKILL.md")).toBe(false);
    expect(templates?.has(".agents/skills/devflow-continue/SKILL.md")).toBe(
      false,
    );
    expect(templates?.has(".agents/skills/devflow-check/SKILL.md")).toBe(true);
  });
  ...
  it("configurePlatform('codebuddy') writes all command templates from common source", async () => {
    ...
    const commandsDir = path.join(tmpDir, ".codebuddy", "commands", "devflow");
    expect(fs.existsSync(commandsDir)).toBe(true);
  ...
  it("configurePlatform('copilot') writes both tracked and discovery hooks config", async () => {
    ...
    const discovery = path.join(tmpDir, ".github", "hooks", "devflow.json");
  ...
  it("configurePlatform('droid') creates .factory/commands/devflow directory", async () => {
    await configurePlatform("droid", tmpDir);
    expect(
      fs.existsSync(path.join(tmpDir, ".factory", "commands", "devflow")),
    ).toBe(true);
  });
  ...
  it("droid configuration writes commands + skills", async () => {
    ...
    expect(content).toContain("name: devflow-check");
  });
  ...
  it("configurePlatform('pi') creates extension-backed Pi assets", async () => {
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "devflow-check.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "devflow-research.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(extension).toContain("DEVFLOW_PI_CLI_JS");
    expect(extension).toContain("function formatPiOutput");
    expect(extension).toContain('"## DevFlow Agent Definition"');
    ...
    expect(extension).not.toContain("message: buildDevFlowContext");
    ...
    // get_context.py is allowed: it lives in .devflow/scripts/ and is the
  ...
  it("configurePlatform('pi') writes tracked templates exactly", async () => {
    ...
    for (const agent of getPiAgents()) {
      ...
      if (["devflow-implement", "devflow-check"].includes(agent.name)) {
  ...
  it("collectPlatformTemplates('pi') maps prompts, skills, agents, extension, and settings", () => {
    ...
    expect(templates?.get(".pi/prompts/devflow-start.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/devflow-finish-work.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/devflow-continue.md")).toBeDefined();
    expect(templates?.get(".pi/skills/devflow-check/SKILL.md")).toBeDefined();
    expect(
      templates?.get(
        ".pi/skills/devflow-meta/references/local-architecture/overview.md",
      ),
    ).toBeDefined();
    expect(
      templates?.get(
    ...
    expect(templates?.get(".pi/extensions/devflow/index.ts")).toBe(
  ...
  it("collectPlatformTemplates('droid') maps commands under .factory/commands/devflow/", () => {
    const templates = collectPlatformTemplates("droid");
    expect(templates).toBeInstanceOf(Map);
    // Droid is agent-capable → start.md not emitted.
    expect(
      templates?.get(".factory/commands/devflow/start.md"),
    ).toBeUndefined();
    expect(
      templates?.get(".factory/commands/devflow/finish-work.md"),
    ).toBeDefined();
    expect(
      templates?.get(".factory/commands/devflow/continue.md"),
    ).toBeDefined();
  });
  ...
  it("does not throw for any platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `devflow-cfg-${id}-`),
      );
      try {
        setWriteMode("force");
  ...
  it("collectPlatformTemplates('copilot') includes tracked + discovery hooks config", () => {
    ...
    expect(templates?.get(".github/hooks/devflow.json")).toBe(
      resolvePlaceholders(getCopilotHooksConfig()),
    );
  });
