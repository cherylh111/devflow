...
describe("replacePythonCommandLiterals", () => {
  ...
  it("handles multiline content with mixed shebangs and commands", () => {
    ...
    const input = [
      ...
      "exec python3 \"$0\" \"$@\"",
      "python3 ./.devflow/scripts/task.py",
    ].join("\n");
    const expected = [
      "#!/usr/bin/env python3",
      "# comment about python",
      "exec python \"$0\" \"$@\"",
      "python ./.devflow/scripts/task.py",
    ].join("\n");
...
describe("resolvePlaceholders", () => {
  ...
  describe("{{CMD_REF:name}}", () => {
    it("resolves with /devflow: prefix (Claude)", () => {
      const result = resolvePlaceholders(
        "See {{CMD_REF:brainstorm}} for details",
        claudeCtx,
      );
      expect(result).toBe("See /devflow:brainstorm for details");
    });
    ...
    it("resolves with /devflow- prefix (Cursor)", () => {
      const result = resolvePlaceholders(
        "Use {{CMD_REF:finish-work}} when done",
        cursorCtx,
      );
      expect(result).toBe("Use /devflow-finish-work when done");
    });
    ...
    it("handles multiple CMD_REF in one template", () => {
      ...
      expect(resolvePlaceholders(input, claudeCtx)).toBe(
        "/devflow:start then /devflow:brainstorm then /devflow:check",
      );
    });
  ...
  describe("simple substitutions", () => {
    ...
    it("resolves {{PYTHON_CMD}} alongside context placeholders", () => {
      const result = resolvePlaceholders(
        "{{PYTHON_CMD}} ./.devflow/scripts/task.py and {{CMD_REF:start}}",
        claudeCtx,
      );
      const py = process.platform === "win32" ? "python" : "python3";
      expect(result).toBe(
        `${py} ./.devflow/scripts/task.py and /devflow:start`,
      );
    });
  });
  ...
  describe("{{CLI_FLAG}}", () => {
    ...
    it("works alongside {{PYTHON_CMD}} in a realistic init-context invocation", () => {
      const input =
        '{{PYTHON_CMD}} ./.devflow/scripts/task.py init-context "$TASK_DIR" <type> --platform {{CLI_FLAG}}';
      const py = process.platform === "win32" ? "python" : "python3";
      expect(resolvePlaceholders(input, codexCtx)).toBe(
        `${py} ./.devflow/scripts/task.py init-context "$TASK_DIR" <type> --platform codex`,
      );
    });
  });
...
describe("resolvePlaceholdersNeutral", () => {
  it("renders {{CMD_REF:name}} as `name` (DevFlow command) — platform-neutral", () => {
    expect(
      resolvePlaceholdersNeutral("See {{CMD_REF:brainstorm}}", claudeCtx),
    ).toBe("See `brainstorm` (DevFlow command)");
    expect(
