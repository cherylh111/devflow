...
function setup(): TmpEnv {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "devflow-channel-guard-test-"),
  );
  const projectDir = path.join(tmpDir, "project");
  fs.mkdirSync(projectDir);
  const channelsRoot = path.join(tmpDir, "channels");
  const oldRoot = process.env.DEVFLOW_CHANNEL_ROOT;
  const oldProject = process.env.DEVFLOW_CHANNEL_PROJECT;
  process.env.DEVFLOW_CHANNEL_ROOT = channelsRoot;
...
function teardown(env: TmpEnv): void {
  if (env.oldRoot === undefined) delete process.env.DEVFLOW_CHANNEL_ROOT;
  else process.env.DEVFLOW_CHANNEL_ROOT = env.oldRoot;
  if (env.oldProject === undefined) delete process.env.DEVFLOW_CHANNEL_PROJECT;
  else process.env.DEVFLOW_CHANNEL_PROJECT = env.oldProject;
...
describe("scanLiveWorkers + enforceSpawnBudget (integration)", () => {
  ...
  it("enforceSpawnBudget rejects when budget is still exhausted post-cleanup", async () => {
    ...
    expect(msg).toContain("devflow channel kill");
