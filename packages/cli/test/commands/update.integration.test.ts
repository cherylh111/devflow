...
describe("update() integration", () => {
  ...
  /**
   * Stage a project as if an older DevFlow version installed pristine template
  ...
  it("#4d preserves user AGENTS.md without DEVFLOW markers by appending the managed block", async () => {
    await setupProject();
    ...
    // User has a hand-written AGENTS.md with no DEVFLOW:START/END markers at
  ...
  it("#18 safe-file-delete preserves user-modified deprecated file", async () => {
    ...
    const deprecatedDir = path.join(tmpDir, ".claude", "commands", "devflow");
  ...
  it("#19 safe-file-delete handles missing deprecated files without crash", async () => {
    ...
    // The manifest has safe-file-delete entries for .claude/commands/devflow/before-backend-dev.md etc.
  ...
  const ORIGINAL_CHECK_BACKEND_CONTENT = [
    ...
    "2. Read `.devflow/spec/backend/index.md` to understand which guidelines apply",
    "3. Based on what you changed, read the relevant guideline files:",
    "   - Database changes → `.devflow/spec/backend/database-guidelines.md`",
    "   - Error handling → `.devflow/spec/backend/error-handling.md`",
    "   - Logging changes → `.devflow/spec/backend/logging-guidelines.md`",
    "   - Type changes → `.devflow/spec/backend/type-safety.md`",
    "   - Any changes → `.devflow/spec/backend/quality-guidelines.md`",
  ...
  function stageLegacy040Project(): void {
    ...
    const legacyDir = path.join(tmpDir, ".claude", "commands", "devflow");
  ...
  function clearMigrationTarget(): void {
    fs.rmSync(path.join(tmpDir, ".claude/skills/devflow-before-dev"), {
      recursive: true,
      force: true,
    });
  }
  ...
  // diff/merge their customizations without digging through .devflow/.backup-*/.
  ...
  it("#25 backup-rename leaves inline <new-path>.backup with original content", async () => {
    ...
    const legacyPath = path.join(
      tmpDir,
      ".claude/commands/devflow/before-dev.md",
    );
  ...
  it("#26 rename-anyway does NOT leave an inline .backup (relies on project snapshot)", async () => {
    ...
    expect(fs.existsSync(newPath)).toBe(true);
    // No inline .backup — the full-project snapshot under .devflow/.backup-*
