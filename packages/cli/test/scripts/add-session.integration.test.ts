 ...
 * `src/templates/devflow/scripts/add_session.py`; this test stamps the real
...
function setupRepo(tmp: string): void {
  ...
  const scriptsDest = path.join(tmp, ".devflow", "scripts");
  ...
  fs.writeFileSync(
    path.join(tmp, ".devflow", "config.yaml"),
  ...
  const r = spawnSync(
    "python3",
    [".devflow/scripts/init_developer.py", DEVELOPER],
...
function makeTask(repo: string, name: string, prdBody: string): void {
  const dir = path.join(repo, ".devflow", "tasks", name);
...
function setCurrentTask(repo: string, taskName: string): void {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      current_task: `.devflow/tasks/${taskName}`,
      platform: "session",
    }) + "\n",
  );
}
...
function runAddSession(repo: string, title: string): void {
  const r = spawnSync(
    "python3",
    [".devflow/scripts/add_session.py", "--title", title],
...
describe.skipIf(!hasPython())("add_session.py auto-commit", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-session-test-"));
    setupRepo(tmp);
  });
  ...
  it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
    ...
    fs.appendFileSync(
      path.join(tmp, ".devflow", "tasks", "task-b", "prd.md"),
    ...
    expect(status).toMatch(/\.devflow\/tasks\/task-b\/prd\.md/);
  });
  ...
  it("does not wide-scan task dirs when the current task is unresolvable (>=2 sessions)", () => {
    ...
    setCurrentTask(tmp, "task-a");
    const sessionsDir = path.join(tmp, ".devflow", ".runtime", "sessions");
    fs.writeFileSync(
      path.join(sessionsDir, "session2.json"),
      JSON.stringify({
        current_task: ".devflow/tasks/task-b",
        platform: "session",
      }) + "\n",
    );
    git(tmp, "add", "-A");
    ...
    fs.appendFileSync(
      path.join(tmp, ".devflow", "tasks", "task-a", "prd.md"),
      "DIRTY A\n",
    );
    fs.appendFileSync(
      path.join(tmp, ".devflow", "tasks", "task-b", "prd.md"),
    ...
    const status = git(tmp, "status", "--porcelain");
    expect(status).toMatch(/\.devflow\/tasks\/task-a\/prd\.md/);
    expect(status).toMatch(/\.devflow\/tasks\/task-b\/prd\.md/);
  });
});
