 ...
 *   1. Version match between `@enpd/devflow` and
 *      `@enpd/devflow-core` (and the current git tag when checked from
 *      a tag context).
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@enpd/devflow-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @enpd/devflow-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @enpd/devflow-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @enpd/devflow-core to exact ${v.cliVersion}.`,
    );
  } finally {
