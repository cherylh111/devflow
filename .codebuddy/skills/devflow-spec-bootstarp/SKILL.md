...
Use this skill to create or refresh `.devflow/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm DevFlow is initialized and inspect the current `.devflow/spec/` tree.
...
| Writing high-signal DevFlow spec files | [references/spec-writing.md](references/spec-writing.md) |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.devflow/spec/`.

## Done Criteria

- `.devflow/spec/` describes the project as it exists now.
