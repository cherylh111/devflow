...
```
Development Flow:
  Learn something → /devflow:update-spec → Knowledge captured
       ↑                                  ↓
  /devflow:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/devflow:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/devflow:update-spec` - Actually makes the updates
- `/devflow:finish-work` - Reminds you to check if specs need updates
