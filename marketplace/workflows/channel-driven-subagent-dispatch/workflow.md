...
3. **Implementation and checking run in channel workers** — use `devflow channel spawn` for implement/check workers by default instead of host-native sub-agents.
4. **Pass context explicitly** — worker context order is `jsonl entries -> prd.md -> design.md -> implement.md`.
5. **Keep results auditable** — use `devflow channel messages --raw` for worker events; pretty output is an operator dashboard and may truncate progress.
...
`.devflow/spec/` stores project engineering guidelines. Before writing code, load the package/layer specs relevant to the task:
...
Each task has its own directory under `.devflow/tasks/{MM-DD-name}/` with `task.json`, `prd.md`, optional `design.md`, optional `implement.md`, optional `research/`, and `implement.jsonl` / `check.jsonl`.

Common commands:

```bash
python3 ./.devflow/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python3 ./.devflow/scripts/task.py start <name>
python3 ./.devflow/scripts/task.py current --source
python3 ./.devflow/scripts/task.py finish
python3 ./.devflow/scripts/task.py archive <name>
...
Phase 2: Execute -> implement/check through devflow channel workers
...
- Complex task: ask whether you may create a DevFlow task and enter planning. If the user says no, do not do broad inline implementation.
...
Channel-driven sub-agent dispatch is the default execution model for this workflow. The main session uses `devflow channel create`, `devflow channel spawn`, `devflow channel send`, and `devflow channel wait` to coordinate workers. Fall back to native host sub-agents only when the user explicitly asks for native dispatch or a host-only capability is required.

[workflow-state:in_progress]
Flow: channel-driven `implement` worker -> channel-driven `check` worker -> `devflow-update-spec` -> commit (Phase 3.4) -> `/devflow:finish-work`.
Main-session default: use `devflow channel spawn` with `.devflow/agents/implement.md` and `.devflow/agents/check.md`; do not use native Claude Task / Codex sub_agent unless explicitly requested or host-only tools require it.
Worker context order: jsonl entries -> `prd.md` -> `design.md if present` -> `implement.md if present`. Use stable worker handles such as `implement`, `check`, `check-cx`, `check-cc`; read results with `devflow channel messages --raw` when precision matters.
[/workflow-state:in_progress]
...
5. `devflow channel messages --raw` is the precise audit path; pretty output is only for quick status checks.
...
Load `devflow-brainstorm` and write user requirements into `prd.md`. Complex tasks also need `design.md` and `implement.md`.
...
```bash
TASK=.devflow/tasks/<active-task>
devflow channel create impl-<topic> --task "$TASK" --by main --ephemeral
devflow channel spawn impl-<topic> \
  --agent implement \
  --as implement \
  ...
  --cwd "$PWD" \
  --timeout 60m
devflow channel send impl-<topic> --as main --to implement --text-file /tmp/implement-brief.md
devflow channel wait impl-<topic> --as main --kind done --from implement --timeout 60m
devflow channel messages impl-<topic> --raw --from implement --last 20
```
...
```bash
TASK=.devflow/tasks/<active-task>
devflow channel create cr-<topic> --task "$TASK" --by main --ephemeral
devflow channel spawn cr-<topic> \
  --agent check \
  --as check \
  ...
  --cwd "$PWD" \
  --timeout 30m
devflow channel send cr-<topic> --as main --to check --text-file /tmp/check-brief.md
devflow channel wait cr-<topic> --as main --kind done --from check --timeout 30m
devflow channel messages cr-<topic> --raw --from check --last 40
```

For independent cross-provider review, spawn `check-cc` and `check-cx` in the same channel:

```bash
devflow channel spawn cr-<topic> --agent check --provider claude --as check-cc --cwd "$PWD" --timeout 30m
devflow channel spawn cr-<topic> --agent check --provider codex --as check-cx --cwd "$PWD" --timeout 30m
devflow channel send cr-<topic> --as main --to check-cc --text-file /tmp/check-brief.md
devflow channel send cr-<topic> --as main --to check-cx --text-file /tmp/check-brief.md
devflow channel wait cr-<topic> --as main --kind done --from check-cc,check-cx --all --timeout 30m
```
...
Load `devflow-check` or use a channel-driven check worker. If issues are found, fix and re-check until green.
...
Load `devflow-check` or dispatch a channel-driven check worker for final verification:
...
If the same class of issue recurred, load `devflow-break-loop` and record root cause plus prevention.

#### 3.3 Spec update `[required · once]`

Load `devflow-update-spec` and decide whether new patterns, pitfalls, or technical decisions should be written back to `.devflow/spec/`.
...
After committing, remind the user to run `/devflow:finish-work` to archive the task and record the session.

---

## Customizing DevFlow

This workflow is customized through `.devflow/workflow.md`. Scripts parse tags and headings; they do not store fallback prose.
