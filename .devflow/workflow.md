...
```bash
python ./.devflow/scripts/init_developer.py <your-name>
```
...
```bash
python ./.devflow/scripts/get_context.py --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
python ./.devflow/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python ./.devflow/scripts/task.py start <name>          # set active task (session-scoped when available)
python ./.devflow/scripts/task.py current --source      # show active task and source
python ./.devflow/scripts/task.py finish                # clear active task (triggers after_finish hooks)
python ./.devflow/scripts/task.py archive <name>        # move to archive/{year-month}/
python ./.devflow/scripts/task.py list [--mine] [--status <s>]
python ./.devflow/scripts/task.py list-archive

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
python ./.devflow/scripts/task.py add-context <name> <action> <file> <reason>
python ./.devflow/scripts/task.py list-context <name> [action]
python ./.devflow/scripts/task.py validate <name>

# Task metadata
python ./.devflow/scripts/task.py set-branch <name> <branch>
python ./.devflow/scripts/task.py set-base-branch <name> <branch>    # PR target
python ./.devflow/scripts/task.py set-scope <name> <scope>

# Hierarchy (parent/child)
python ./.devflow/scripts/task.py add-subtask <parent> <child>
python ./.devflow/scripts/task.py remove-subtask <parent> <child>

# PR creation
python ./.devflow/scripts/task.py create-pr [name] [--dry-run]
```

> Run `python ./.devflow/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
python ./.devflow/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
python ./.devflow/scripts/get_context.py                            # full session runtime
python ./.devflow/scripts/get_context.py --mode packages            # available packages + spec layers
python ./.devflow/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
```bash
python ./.devflow/scripts/get_context.py --mode phase --step <step>
# e.g. python ./.devflow/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
python ./.devflow/scripts/task.py create "<task title>" --slug <name>
```
...
Skip when `python ./.devflow/scripts/task.py current --source` already points to a task.
...
`file` paths are repo-root relative. `knowledge` ids are structured entries searchable via `python ./.devflow/scripts/knowledge.py search`, including wiki entries under `.devflow/spec/wiki/` and learnings under `.devflow/spec/guides/learnings.md`.
...
```bash
python ./.devflow/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python ./.devflow/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
python ./.devflow/scripts/task.py add-context "$TASK_DIR" implement "knowledge:<id>" "<reason>"
python ./.devflow/scripts/task.py add-context "$TASK_DIR" check "wiki:<id>" "<reason>"
```
...
```bash
python ./.devflow/scripts/task.py start <task-dir>
```
