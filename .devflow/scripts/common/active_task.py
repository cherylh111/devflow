...
The user-facing concept is a single "active task". DevFlow stores that pointer
per AI session/window under `.devflow/.runtime/sessions/`; without a stable
...
def _detect_platform(platform_input: dict[str, Any] | None, platform: str | None) -> str:
    ...
    if platform_input:
        for key in ("_devflow_platform", "devflow_platform", "platform", "source"):
...
def _lookup_env_context_key(platform_name: str | None) -> str | None:
    ...
    Hooks pass `DEVFLOW_CONTEXT_ID` to subprocesses they launch, but an AI-run
...
def resolve_context_key(
    ...
    `DEVFLOW_CONTEXT_ID` is an explicit context-key override used by CLI
    scripts and subprocesses. It does not store the task itself.
    """
    override = _string_value(os.environ.get("DEVFLOW_CONTEXT_ID"))
    if override:
