...
def _collect_package_git_info(
    ...
    When the DevFlow root is not a Git repo and no configured package repos are
...
def _fetch_devflow_version_output() -> str | None:
    try:
...
def _resolve_available_update_version() -> str | None:
    output = _fetch_devflow_version_output()
    if not output:
        return None
...
def _get_update_hint(repo_root: Path) -> str | None:
    ...
    return (
        f"DevFlow update available: {current_version} -> {latest_version}, "
        "run devflow upgrade"
    )
