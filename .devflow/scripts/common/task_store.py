...
_SEED_EXAMPLE = (
    ...
    "Run `python .devflow/scripts/get_context.py --mode packages` to list specs and `python .devflow/scripts/knowledge.py search <query>` to find knowledge. "
...
def _auto_commit_archive(
    ...
    """Stage DevFlow-owned task paths and commit after archive.
    ...
    ``git add -f .devflow/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.devflow/config.yaml``: when
