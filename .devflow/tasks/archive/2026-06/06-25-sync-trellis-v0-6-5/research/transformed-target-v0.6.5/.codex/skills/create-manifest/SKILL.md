...
description: "Create a DevFlow migration manifest and matching docs-site changelogs for a target release by analyzing commits since the previous release. Use when preparing a patch, beta, rc, or minor release manifest."
---
...
DevFlow currently publishes two npm packages from the same git tag:
...
Voice: technical reference doc. Short, clear, plain. Not a story, not a sales pitch. Follow `.devflow/spec/docs-site/docs/style-guide.md` -> "Changelog / Release Notes Voice".
...
- Keep it shorter than the MDX changelog because it prints in terminal during `devflow update`.
...
| `recommendMigrate` | Any rename/delete migration the user should run. Default `false` for patch fixes. When `breaking=true` and `recommendMigrate=true`, `devflow update` exits 1 without `--migrate`. |
...
`rename` uses the project-local `.devflow/.template-hashes.json`; it does not use manifest `allowed_hashes`.
...
Full reference: `.devflow/spec/docs-site/docs/release-lifecycle.md`.
...
5. `@enpd/devflow` and `@enpd/devflow-core` versions still match.
...
npx -y @enpd/devflow@<last-ga> init -y -u test --claude --cursor --<platforms>
