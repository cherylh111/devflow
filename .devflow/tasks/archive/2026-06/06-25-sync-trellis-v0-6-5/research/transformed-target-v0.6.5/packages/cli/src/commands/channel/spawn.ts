...
import type { InboxPolicy } from "@enpd/devflow-core/channel";
...
export interface SpawnOptions {
  ...
  files?: string[];
  /** DevFlow jsonl manifests to expand into the system prompt. */
  jsonls?: string[];
  scope?: string;
  /** Identity recorded as the `spawned` event author. Defaults to
   *  the calling worker (`DEVFLOW_CHANNEL_AS` env) or "main". */
  by?: string;
...
/**
 * Compose the worker's system prompt: DevFlow channel protocol prefix
...
function buildSystemPrompt(
  ...
  const protocol = [
    "[DEVFLOW CHANNEL PROTOCOL — placeholder]",
...
async function spawnLocked(
  ...
  const spawnedBy =
    opts.by ??
    (typeof process.env.DEVFLOW_CHANNEL_AS === "string" &&
    process.env.DEVFLOW_CHANNEL_AS.length > 0
      ? process.env.DEVFLOW_CHANNEL_AS
      : "main");
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        DEVFLOW_CHANNEL_PROJECT: project,
      },
    },
  );
