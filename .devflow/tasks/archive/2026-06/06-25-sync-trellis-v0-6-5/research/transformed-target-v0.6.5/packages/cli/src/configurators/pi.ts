...
export function collectPiTemplates(): Map<string, string> {
  ...
  for (const command of resolvePiCommands()) {
    files.set(`.pi/prompts/devflow-${command.name}.md`, command.content);
  }
  ...
  files.set(".pi/extensions/devflow/index.ts", getExtensionTemplate());
...
export async function configurePi(cwd: string): Promise<void> {
  ...
  for (const command of resolvePiCommands()) {
    await writeFile(
      path.join(configRoot, "prompts", `devflow-${command.name}.md`),
      command.content,
    );
  }
  ...
  ensureDir(path.join(configRoot, "extensions", "devflow"));
  await writeFile(
