...
/**
 * Resolve the auto-triggered skill templates from `common/skills/` with devflow- prefix + SKILL.md frontmatter.
...
function mapLegacyToolToCopilot(tool: string): string[] {
  switch (tool) {
    ...
    case "Bash":
      return ["execute"];
    // Generic MCP wildcard — used by devflow-research to opt into "any MCP
