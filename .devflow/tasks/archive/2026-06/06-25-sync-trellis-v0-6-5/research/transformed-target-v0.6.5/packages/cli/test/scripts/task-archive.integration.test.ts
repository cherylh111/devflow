 ...
 * `src/templates/devflow/scripts/common/task_store.py`; this test stamps
...
function runArchive(repo: string, taskName: string): void {
  const r = spawnSync(
    "python3",
    [".devflow/scripts/task.py", "archive", taskName],
...
describe.skipIf(!hasPython())(
  ...
  () => {
    ...
    beforeEach(() => {
      tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-archive-test-"));
      setupRepo(tmp);
    });
    ...
    it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
      ...
      expect(status).toMatch(/M\s+\.devflow\/tasks\/task-b\/prd\.md/);
    });
    ...
    it(
      ...
      () => {
        ...
        expect(
          deletes.every((p) => p.startsWith(".devflow/tasks/big/")),
        ).toBe(true);
      },
    ...
    it("fails when archive auto-commit cannot record tracked source deletes", () => {
      ...
      const r = spawnSync(
        "python3",
        [".devflow/scripts/task.py", "archive", "tracked"],
      ...
      const status = git(tmp, "status", "--porcelain");
      expect(status).toContain(".devflow/tasks/tracked/");
      expect(status).toContain(".devflow/tasks/archive/");
    });
  },
);
