...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-cleanup-"));
  });
  ...
  it("[CR#1] does not delete .devflow root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".devflow"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".devflow");
    expect(fs.existsSync(path.join(tmpDir, ".devflow"))).toBe(true);
  });

  it("recursively cleans parent directories but stops at root", () => {
    // Create .devflow/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".devflow", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".devflow/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".devflow", "scripts", "multi_agent"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".devflow", "scripts")),
    ).toBe(false);
    // .devflow root must survive
    expect(fs.existsSync(path.join(tmpDir, ".devflow"))).toBe(true);
  });
...
describe("loadUpdateSkipPaths", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-skip-"));
    fs.mkdirSync(path.join(tmpDir, ".devflow"), { recursive: true });
  });
...
describe("sortMigrationsForExecution", () => {
  ...
  it("puts rename-dir before rename and delete", () => {
    const items = [
      ...
      { type: "rename-dir" as const, from: ".devflow/old", to: ".devflow/new" },
  ...
  it("sorts rename-dir by path depth (deeper first)", () => {
    const items = [
      { type: "rename-dir" as const, from: ".devflow/a", to: ".devflow/x" },
      {
        type: "rename-dir" as const,
        from: ".devflow/a/b/c",
        to: ".devflow/x/y/z",
      },
      { type: "rename-dir" as const, from: ".devflow/a/b", to: ".devflow/x/y" },
    ];
    const sorted = sortMigrationsForExecution(items);
    expect(sorted[0].from).toBe(".devflow/a/b/c"); // depth 4
    expect(sorted[1].from).toBe(".devflow/a/b"); // depth 3
    expect(sorted[2].from).toBe(".devflow/a"); // depth 2
  });
...
describe("shouldExcludeFromBackup", () => {
  ...
  it.each([
    ...
    ".devflow/.backup-2026-04-22T10-24-27/.opencode/node_modules/zod/index.js",
  ])("excludes dependency tree %s", (p) => {
    expect(shouldExcludeFromBackup(p)).toBe(true);
  });

  it.each([
    ".devflow/workspace/developer/journal-1.md",
    ".devflow/tasks/04-17-foo/prd.md",
    ".devflow/spec/cli/backend/index.md",
    ".devflow/backlog/idea.md",
    ".devflow/agent-traces/trace.jsonl",
  ...
  it("excludes previous backups", () => {
    expect(
      shouldExcludeFromBackup(".devflow/.backup-2026-04-20T01-00-00/x"),
    ).toBe(true);
  });

  it.each([
    ".claude/commands/devflow/continue.md",
    ".claude/skills/devflow-check/SKILL.md",
    ".devflow/workflow.md",
    ".devflow/scripts/get_context.py",
  ...
  // after normalization, otherwise DevFlow's native worktree protection
  ...
  // `devflow update --migrate`, late April 2026).
  it.each([
    ".claude\\worktrees\\feat-x\\src\\main.ts",
    ".devflow\\tasks\\04-17-foo\\prd.md",
    ".devflow\\workspace\\dev\\journal-1.md",
