...
**Real example**: `devflow update` had a manual `files.set()` list for 11 scripts that `getAllScripts()` already tracked. Fix: replaced the manual list with a `for..of getAllScripts()` loop. See `update.ts` refactor in v0.4.0-beta.3.

---

## Template File Registration (DevFlow-specific)

When adding new files to `src/templates/devflow/scripts/`:

**Single registration point**: `src/templates/devflow/index.ts`
...
**Why this matters**: Without registration in `getAllScripts()`, `devflow update` won't sync the file to user projects. Bug fixes and features won't propagate.

**History**: Before v0.4.0-beta.3, `update.ts` had its own hand-maintained file list that frequently fell out of sync with `getAllScripts()`. This caused 11 Python files to be silently skipped during `devflow update`. The fix was to eliminate the duplicate list and use `getAllScripts()` as the single source of truth.

### Quick Checklist for New Scripts

```bash
# After adding a new .py file, verify it's in getAllScripts():
grep -l "newFileName" src/templates/devflow/index.ts  # Should match
```

### Template Sync Convention

`.devflow/scripts/` (dogfooded) and `packages/cli/src/templates/devflow/scripts/` (template) must stay identical. After editing `.devflow/scripts/`, always sync:

```bash
rsync -av --delete --exclude='__pycache__' .devflow/scripts/ packages/cli/src/templates/devflow/scripts/
```

**Gotcha**: Running rsync with wrong source/destination paths can create nested garbage directories (e.g., `.devflow/scripts/packages/cli/...`). Always double-check paths before running.
