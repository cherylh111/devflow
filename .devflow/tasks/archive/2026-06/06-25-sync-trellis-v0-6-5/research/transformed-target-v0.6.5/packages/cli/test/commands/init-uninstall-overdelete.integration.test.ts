 ...
 *   - init's manifest only contains paths devflow actually wrote
...
describe("init + uninstall: manifest accuracy + homedir guard", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-overdelete-"));
  ...
  it("#R1.1 init does not hash pre-existing .codex/sessions/ user data (issue #221)", async () => {
    ...
    // Sanity: devflow's own codex files ARE tracked.
  ...
  it("#R3.2 uninstall self-heals + preserves user file even without prior update", async () => {
    ...
    // and runs `devflow uninstall` directly. Prune must fire before plan
  ...
  it("#R3.3 prune keeps migration-referenced paths even if not in collectTemplates", async () => {
    ...
    // CAN assert the prune behavior preserves .devflow/ entries which is the
    ...
    const hashes = loadHashes(tmpDir);
    hashes[".devflow/workflow.md"] = "ok";
    saveHashes(tmpDir, hashes);

    await update({});

    // .devflow/* entries are kept.
  ...
  it("#R2.1 init refuses to run when cwd === $HOME", async () => {
    ...
    try {
      ...
      // No .devflow dir was created.
      expect(fs.existsSync(path.join(fakeHome, ".devflow"))).toBe(false);
    } finally {
  ...
  it("#R2.2 uninstall refuses to run when cwd === $HOME", async () => {
    // Set up a valid devflow project, then pretend its cwd is the homedir.
    await init({ yes: true, claude: true, force: true });
  ...
  it("#R2.3 DEVFLOW_ALLOW_HOMEDIR=1 bypasses the guard for init", async () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "fake-home-"));
    try {
      vi.spyOn(process, "cwd").mockReturnValue(fakeHome);
      ...
      expect(fs.existsSync(path.join(fakeHome, ".devflow"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
  ...
  it("#R2.4 subdirectories of $HOME are NOT blocked", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(subDir, ".devflow"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
