 ...
 * `devflow update` (before migration classification) and `devflow uninstall`
 * (before plan building). Together they ensure existing poisoned manifests
 * self-correct on the next routine command.
 *
 * Rules:
 *   - `.devflow/*` entries are ALWAYS kept. `devflow uninstall` removes
 *     `.devflow/` wholesale via `fs.rmSync(..., { recursive: true })`, so
 ...
 *   - Root-level `AGENTS.md` is kept only when it still looks DevFlow-managed
 ...
 *     pruned. This matches "files devflow actually wrote during init/update".
 */
...
/**
 * Compute the union of "what devflow writes" across:
 *   - every configured platform's collectTemplates() output
 *   - root-level AGENTS.md when it still carries DevFlow managed-block markers
...
function shouldKeepAgentsMd(cwd: string): boolean {
  ...
  try {
    ...
    return (
      content.includes(DEVFLOW_BLOCK_START) &&
      content.includes(DEVFLOW_BLOCK_END)
    );
  } catch {
    return true;
  }
...
export function pruneOrphanManifestKeys(
  ...
  for (const [rawKey, value] of Object.entries(hashes)) {
    ...
    // Always preserve .devflow/ entries — they're for the workflow tree
    // which uninstall removes wholesale and which update needs for
    // modified-file detection.
    if (key.startsWith(".devflow/") || key === ".devflow") {
      kept[key] = value;
      continue;
    }
