...
describe("registry-config", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-reg-config-"));
  ...
  it("preserves self-hosted SSH registry source strings", () => {
    ...
    writeSpecRegistryConfig(tmpDir, {
      source: "git@git.ppdaicorp.com:xionghongwei/devflow-spec.git",
      template: "golang-spec",
    });
  ...
  it("does not duplicate an existing registry section", () => {
    const configPath = path.join(tmpDir, ".devflow", "config.yaml");
    fs.writeFileSync(
      configPath,
