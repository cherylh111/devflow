 ...
 *
 * Run as: `devflow channel __supervisor <channel> <worker> <config-path>`
 *
...
export interface SupervisorConfig {
  ...
  systemPrompt: string;
  /** Extra env vars (DEVFLOW_HOOKS=0 etc. are added automatically). */
...
/**
 * Entry point invoked by `devflow channel __supervisor <channel> <worker> <config>`.
 */
export async function runSupervisor(
  ...
  // Self-pid file lets `devflow channel kill` find us.
  const project = process.env.DEVFLOW_CHANNEL_PROJECT;
  fs.writeFileSync(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...config.env,
    DEVFLOW_HOOKS: "0",
    DEVFLOW_CHANNEL: channelName,
    DEVFLOW_CHANNEL_AS: workerName,
  };
    ...
    try {
      fs.unlinkSync(
        workerFile(
          ...
          workerName,
          suffix,
          process.env.DEVFLOW_CHANNEL_PROJECT,
        ),
      );
    } catch {
      // already gone
