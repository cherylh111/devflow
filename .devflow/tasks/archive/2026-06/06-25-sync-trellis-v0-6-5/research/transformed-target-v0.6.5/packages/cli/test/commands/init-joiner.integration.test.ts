 ...
 *   no .devflow/                        → creator bootstrap task
 *   .devflow/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-joiner-int-"));
  ...
  /**
   * Helper: simulate a fresh clone of an existing DevFlow project — `.devflow/`
  ...
  /** Helper: simulate same-dev re-init — both `.devflow/` and `.developer` exist */
  ...
  it("#2 existing .devflow/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".devflow/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.devflow/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(`${expectedPythonCmd} ./.devflow/scripts/task.py finish`);
    expect(prd).toContain(
      `${expectedPythonCmd} ./.devflow/scripts/task.py archive 00-join-bob`,
    );
  ...
  it("#2b issue #204: existing .devflow/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .devflow/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .devflow/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .devflow/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .devflow/
  ...
  it("#7 handleReinit path: existing .devflow/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .devflow/ + .developer → no task created", async () => {
