...
# -*- coding: utf-8 -*-
"""
DevFlow StatusLine — project-level status display for Claude Code.

Reads Claude Code session JSON from stdin + DevFlow task data from filesystem.
Outputs 1-2 lines:
...
def _find_devflow_dir() -> Path | None:
    """Walk up from cwd to find .devflow/ directory."""
    current = Path.cwd()
    for parent in [current, *current.parents]:
        candidate = parent / ".devflow"
        if candidate.is_dir():
            return candidate
    return None


def _get_current_task(devflow_dir: Path) -> dict | None:
    """Load current task info through DevFlow' active task resolver."""
    return _get_current_task_for_input(devflow_dir, {})


def _get_current_task_for_input(devflow_dir: Path, cc_data: dict) -> dict | None:
    ...
    active = resolve_active_task(devflow_dir.parent, cc_data, platform="claude")
    if not active.task_path:
        return None

    task_path = _resolve_task_dir(devflow_dir, active.task_path)
    if active.stale:
        return {
...
def _count_active_tasks(devflow_dir: Path) -> int:
...
def _get_developer(devflow_dir: Path) -> str:
    content = _read_text(devflow_dir / ".developer")
    if not content:
        return "unknown"
...
def main() -> None:
    ...
    devflow_dir = _find_devflow_dir()
    SEP = " \033[90m·\033[0m "

    # --- DevFlow data ---
    task = _get_current_task_for_input(devflow_dir, cc_data) if devflow_dir else None
    dev = _get_developer(devflow_dir) if devflow_dir else ""
    task_count = _count_active_tasks(devflow_dir) if devflow_dir else 0
