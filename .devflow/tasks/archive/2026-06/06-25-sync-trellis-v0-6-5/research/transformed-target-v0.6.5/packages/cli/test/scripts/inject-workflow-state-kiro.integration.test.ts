...
function setupRepo(tmp: string): void {
  fs.mkdirSync(path.join(tmp, ".devflow", "scripts"), { recursive: true });
  fs.cpSync(TEMPLATE_SCRIPTS, path.join(tmp, ".devflow", "scripts"), {
    recursive: true,
  });
  // workflow.md with a no_task breadcrumb so the body is deterministic.
  fs.writeFileSync(
    path.join(tmp, ".devflow", "workflow.md"),
    [
      "# Workflow",
      "",
      "## Phase Index",
      ...
      "No active task. Classify the turn before creating a DevFlow task.",
      "[/workflow-state:no_task]",
      "",
      "## Phase 1: Plan",
      "",
...
describeFn("Kiro hook output branch", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-kiro-hook-"));
    setupRepo(tmp);
  });
