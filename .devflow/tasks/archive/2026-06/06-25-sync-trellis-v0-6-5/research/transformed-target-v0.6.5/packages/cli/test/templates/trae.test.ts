...
describe("trae shared skills (from common source)", () => {
  ...
  it("does not include platform-specific syntax in resolved output", () => {
    const skills = resolveAllAsSkills(AI_TOOLS.trae.templateContext);
    for (const skill of skills) {
      // Trae uses /devflow- prefix, not /devflow:
      expect(skill.content).not.toContain("/devflow:");
...
// =============================================================================
//
// devflow-implement / devflow-check agent markdown MUST contain a hard recursion
// guard so the dispatched sub-agent does not spawn another
// devflow-implement / devflow-check sub-agent.
describe("trae sub-agent recursion guard", () => {
  for (const name of ["devflow-implement", "devflow-check"] as const) {
    it(`${name}.md forbids spawning devflow-implement / devflow-check`, () => {
      ...
      expect(content).toContain("devflow-implement");
      expect(content).toContain("devflow-check");
...
// so they load DevFlow context themselves.
describe("trae pull-based prelude injection (class-2)", () => {
  it("applyPullBasedPreludeMarkdown injects context-loading instructions", () => {
    ...
    for (const agent of agents) {
      if (
        agent.name === "devflow-implement" ||
        agent.name === "devflow-check"
      ) {
        expect(agent.content).toContain("Load DevFlow Context First");
  ...
  it("research agent does not get the prelude (no task context needed)", () => {
    const agents = applyPullBasedPreludeMarkdown(getAllAgents());
    const research = agents.find((a) => a.name === "devflow-research");
    expect(research).toBeDefined();
    if (!research) return; // guard for type-safety
    expect(research.content).not.toContain("Load DevFlow Context First");
  });
});
