...
const EXPECTED_AGENT_NAMES = ["devflow-check", "devflow-implement"];
...
describe("collectReasonixTemplates", () => {
  it("writes both subagent skills under .reasonix/skills/", () => {
    ...
    expect(files.has(".reasonix/skills/devflow-check/SKILL.md")).toBe(true);
    expect(files.has(".reasonix/skills/devflow-implement/SKILL.md")).toBe(true);
  });

  it("does not duplicate devflow-check / devflow-implement as workflow skills", () => {
    // Subagent skills replace their common-skill equivalents — workflow skills
    // must not also emit a `.reasonix/skills/devflow-check/SKILL.md` from the
    // shared resolver, or the bundled subagent variant would be overwritten.
    const files = collectReasonixTemplates();
    const checkPaths = [...files.keys()].filter((p) =>
      p.endsWith("/devflow-check/SKILL.md"),
    );
    const implementPaths = [...files.keys()].filter((p) =>
      p.endsWith("/devflow-implement/SKILL.md"),
    );
  ...
  it("subagent SKILL.md content carries runAs: subagent frontmatter", () => {
    const files = collectReasonixTemplates();
    const checkBody = files.get(".reasonix/skills/devflow-check/SKILL.md");
    const implementBody = files.get(
      ".reasonix/skills/devflow-implement/SKILL.md",
    );
