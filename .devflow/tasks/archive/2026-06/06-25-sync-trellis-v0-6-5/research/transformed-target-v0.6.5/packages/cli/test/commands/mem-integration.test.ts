...
const { fakeHome } = vi.hoisted(() => {
  ...
  const fakeHome = f.mkdtempSync(p.join(o.tmpdir(), "devflow-mem-int-"));
  return { fakeHome };
});
  ...
  writeJsonl(
    ...
    [
      ...
      },
      {
        ...
        message: {
          role: "bashExecution",
          command: "task.py start .devflow/tasks/06-18-pi-cli",
          output: "",
        },
      },
      {
...
describe("runMem subcommand integration", () => {
        ...
        message: {
          ...
          content: [
            { type: "text", text: "creating task" },
            {
              ...
              input: {
                command:
                  "python3 ./.devflow/scripts/task.py create --slug demo",
              },
            },
          ],
        },
        ...
        message: {
          ...
          content: [
            { type: "text", text: "starting" },
            {
              ...
              input: {
                command:
                  "python3 ./.devflow/scripts/task.py start .devflow/tasks/demo",
              },
            },
          ],
        },
  ...
  it("help command prints usage", () => {
    runMem(["help"]);
    const joined = logs.join("\n");
    expect(joined).toContain("devflow mem");
