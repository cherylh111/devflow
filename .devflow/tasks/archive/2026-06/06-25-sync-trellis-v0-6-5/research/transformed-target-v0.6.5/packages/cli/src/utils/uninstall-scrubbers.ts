/**
 * Scrubbers for structured config files during `devflow uninstall`.
 *
 ...
 * - `fullyEmpty` is true when, after stripping every devflow-managed value,
 ...
 *
 * DevFlow-emitted hook commands always have the shape
...
/**
 * DevFlow-specific values written by the Pi configurator.
 *
 * The `extensions`/`skills`/`prompts` arrays are paths relative to `.pi/`. We
 * remove the exact entries that the Pi configurator emits.
 */
const PI_DEVFLOW_EXTENSION = "./extensions/devflow/index.ts";
const PI_DEVFLOW_SKILLS = "./skills";
const PI_DEVFLOW_PROMPTS = "./prompts";
const PI_SUBAGENTS_PACKAGE = "npm:pi-subagents";

function isDevFlowPiEntry(value: unknown, target: string): boolean {
  return typeof value === "string" && value === target;
}

/**
 * Scrub `.pi/settings.json`:
 * - drop `enableSkillCommands` (devflow-flagged)
 * - remove devflow entries from `extensions`/`skills`/`prompts` arrays
 * - remove devflow-managed `packages["npm:pi-subagents"]` isolation override
...
export function scrubPiSettings(content: string): ScrubResult {
  ...
  const arrayCleanups: [string, string][] = [
    ["extensions", PI_DEVFLOW_EXTENSION],
    ["skills", PI_DEVFLOW_SKILLS],
    ["prompts", PI_DEVFLOW_PROMPTS],
  ];
  for (const [key, target] of arrayCleanups) {
    const arr = root[key];
    if (!Array.isArray(arr)) continue;
    const filtered = arr.filter((v) => !isDevFlowPiEntry(v, target));
 ...
 *
 * The current devflow-emitted file has two distinct chunks:
 * 1. The line `project_doc_fallback_filenames = ["AGENTS.md"]`
 * 2. A multi-line comment block that begins with the marker
 *    `# NOTE: DevFlow's SessionStart + UserPromptSubmit hooks require opt-in.`
 ...
 *  - any line that is *only* a comment introduced by devflow (the entire file
 *    as shipped is comments + that one assignment)
 *  - blank lines that surrounded those removals
 *
 * If the user added their own non-devflow lines, they are preserved as-is.
...
export function scrubCodexConfigToml(content: string): ScrubResult {
  const devflowCommentMarkers = [
    "Project-scoped Codex defaults for DevFlow workflows.",
    ...
    "hooks = true",
    "codex_hooks = true",
    "Without this flag, hooks.json is ignored and DevFlow context won't",
    "be injected into Codex sessions.",
  ];

  // A comment line is "devflow-known" if its content (after `#` and spaces)
  // matches one of the known marker strings exactly OR is an empty `#` line.
  function isDevFlowCommentLine(line: string): boolean {
    const trimmed = line.trim();
    if (!trimmed.startsWith("#")) return false;
    const inner = trimmed.replace(/^#+\s?/, "").trim();
    if (inner.length === 0) return true; // bare `#` line inside devflow block
    return devflowCommentMarkers.some((m) => inner === m);
  }

  function isDevFlowAssignment(line: string): boolean {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    if (isDevFlowAssignment(rawLine) || isDevFlowCommentLine(rawLine)) {
      continue; // drop
    }
