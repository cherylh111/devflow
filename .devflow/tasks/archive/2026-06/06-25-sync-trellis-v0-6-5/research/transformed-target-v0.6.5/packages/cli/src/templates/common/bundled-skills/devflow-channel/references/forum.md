...
```bash
devflow channel create design-feedback \
  --type forum \
  --scope global \
...
```bash
devflow channel post design-feedback opened \
  --scope global \
  --as main \
...
devflow channel post design-feedback comment \
  --scope global \
  --as reviewer \
  --thread login-empty-state \
  --text-file /tmp/review.md

devflow channel post design-feedback status \
  --scope global \
  --as main \
  --thread login-empty-state \
  --status closed

devflow channel post design-feedback summary \
  --scope global \
  --as main \
  --thread login-empty-state \
  --summary "Adopted the option-B layout; ticket DEVFLOW-123 owns the fix."
```
...
```bash
devflow channel messages design-feedback --scope global
devflow channel forum design-feedback --scope global --status open
devflow channel thread design-feedback login-empty-state --scope global
devflow channel messages design-feedback --scope global --raw --thread login-empty-state
```
...
devflow channel context add design-feedback \
  --scope global \
...
devflow channel context add design-feedback \
  --scope global \
  --thread login-empty-state \
  --file "$PWD/.devflow/tasks/05-13-login-redesign/design.md"
```
...
```bash
devflow channel context list design-feedback --scope global
devflow channel context list design-feedback --scope global --thread login-empty-state --raw
```
...
```bash
devflow channel context delete design-feedback \
  --scope global \
  --thread login-empty-state \
  --raw "stale note"
```
...
```bash
devflow channel title set design-feedback \
  --scope global \
  --title "Design feedback board"

devflow channel title clear design-feedback --scope global
```
...
```bash
devflow channel thread rename design-feedback old-key new-key \
  --scope global \
  --as main
```
...
```bash
devflow channel create release-notes \
  --type forum \
  --scope global \
...
devflow channel post release-notes opened \
  --scope global \
  --as main \
