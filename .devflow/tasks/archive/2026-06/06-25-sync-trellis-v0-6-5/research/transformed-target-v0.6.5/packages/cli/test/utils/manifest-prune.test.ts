 ...
 * (.devflow/tasks/05-13-uninstall-overdelete-manifest-leak).
 */
...
describe("pruneOrphanManifestKeys", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-prune-"));
  ...
  it("preserves every .devflow/* entry regardless of platform-collect output", () => {
    const hashes = {
      ".devflow/workflow.md": "h1",
      ".devflow/scripts/task.py": "h2",
      ".devflow/config.yaml": "h3",
    };
  ...
  it("keeps root-level AGENTS.md when it has DevFlow managed-block markers", () => {
  ...
  it("prunes poisoned root-level AGENTS.md when the file lacks DevFlow markers", () => {
  ...
  it("persists pruned manifest to disk by default", () => {
    ...
    expect(loadHashes(tmpDir)).toHaveProperty(".devflow/workflow.md");
  });
  ...
  it("does NOT rewrite disk when nothing was pruned", () => {
    const hashes = { ".devflow/workflow.md": "h1" };
    saveHashes(tmpDir, hashes);

    const hashFile = path.join(tmpDir, ".devflow", ".template-hashes.json");
...
describe("isCwdHomedir / homedir guard helpers", () => {
  ...
  it("homedirBypassEnabled reflects DEVFLOW_ALLOW_HOMEDIR env var", () => {
    const orig = process.env.DEVFLOW_ALLOW_HOMEDIR;
    try {
      delete process.env.DEVFLOW_ALLOW_HOMEDIR;
      expect(homedirBypassEnabled()).toBe(false);
      process.env.DEVFLOW_ALLOW_HOMEDIR = "1";
      expect(homedirBypassEnabled()).toBe(true);
      for (const value of ["0", "false", "true", ""]) {
        process.env.DEVFLOW_ALLOW_HOMEDIR = value;
        expect(homedirBypassEnabled()).toBe(false);
      }
    } finally {
      if (orig === undefined) delete process.env.DEVFLOW_ALLOW_HOMEDIR;
      else process.env.DEVFLOW_ALLOW_HOMEDIR = orig;
    }
  });
  ...
  it("homedirGuardMessage mentions the command and the bypass env var", () => {
    ...
    expect(msgInit).toContain("DEVFLOW_ALLOW_HOMEDIR=1");

    const msgUninstall = homedirGuardMessage("uninstall");
    expect(msgUninstall).toContain("uninstall");
    expect(msgUninstall).toContain("DEVFLOW_ALLOW_HOMEDIR=1");
  });
});
