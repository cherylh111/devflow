...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "devflow-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'devflow-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'devflow', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/devflow-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Devin uses workflow directory: .devin/workflows/devflow-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/devflow-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/devflow/<name>.md
        """
        if self.platform == "pi":
            prompts_dir = self.get_config_dir(project_root) / "prompts"
            if not parts:
                return prompts_dir
            if len(parts) >= 2 and parts[0] == "devflow":
                filename = parts[-1]
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"devflow-{filename}.md"
        ...
        if self.platform == "devin":
            ...
            if len(parts) >= 2 and parts[0] == "devflow":
                filename = parts[-1]
                return workflow_dir / f"devflow-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "devflow":
            # Convert devflow/<name>.md to devflow-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"devflow-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_devflow_command_path(self, name: str) -> str:
        """Get relative path to a devflow command file.
        ...
        Note:
            Cursor: .cursor/commands/devflow-<name>.md
            Codex: .agents/skills/devflow-<name>/SKILL.md
            Kiro: .kiro/skills/devflow-<name>/SKILL.md
            Gemini: .gemini/commands/devflow/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Devin: .devin/workflows/devflow-<name>.md
            Pi: .pi/prompts/devflow-<name>.md
            Others: .{platform}/commands/devflow/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/devflow-{name}.md"
        elif self.platform == "codex":
            # 0.5.0-beta.0 renamed all skill dirs to add the `devflow-` prefix
            # (see that release's manifest for the 60+ rename entries).
            return f".agents/skills/devflow-{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/devflow-{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/devflow/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "devin":
            return f".devin/workflows/devflow-{name}.md"
        elif self.platform == "kilo":
            ...
            return f"{self.config_dir_name}/commands/devflow/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    # as devin for back-compat until users migrate via `devflow update --migrate`).
    if (
