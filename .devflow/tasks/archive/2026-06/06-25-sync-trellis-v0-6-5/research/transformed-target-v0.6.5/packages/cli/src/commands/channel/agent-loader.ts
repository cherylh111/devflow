/**
 * Load a DevFlow agent definition from `.devflow/agents/<name>.md`.
 *
 ...
 * channel runtime (they may be consumed by other DevFlow layers).
 */
...
export function findAgentFile(name: string, cwd: string): string | null {
  ...
  const agentsRoot = path.resolve(cwd, ".devflow", "agents");
  const candidates = [
  ...
  if (!file) {
    throw new Error(
      `Agent '${name}' not found. Looked in:\n  ${[
        path.join(cwd, ".devflow", "agents", `${name}.md`),
        path.join(cwd, ".devflow", "agents", name, "AGENT.md"),
      ].join("\n  ")}`,
    );
  }
