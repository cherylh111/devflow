/**
 * End-to-end tests for the public `@enpd/devflow-core/mem` API against a
...
const { fakeHome } = vi.hoisted(() => {
  ...
  const fakeHome = f.mkdtempSync(p.join(o.tmpdir(), "devflow-mem-api-"));
  return { fakeHome };
});
...
function seedPiPhaseSession(id: string, cwd: string): void {
  writeJsonl(nodePath.join(piProjectDir(cwd), `2026-06-18_${id}.jsonl`), [
    ...
    },
    {
      ...
      message: {
        role: "bashExecution",
        command: "task.py start .devflow/tasks/06-18-pi-api",
        output: "",
      },
    },
    {
