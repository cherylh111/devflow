/**
 * Integration tests for `devflow workflow` and the init/update hash boundary
 * for non-native workflow selection.
 *
 * Coverage:
 * - `devflow workflow --template native`: writes bundled content, keeps hash.
 * - `devflow workflow --template tdd`: writes marketplace content, removes hash.
 * - `devflow init --workflow tdd`: marketplace content is written, hash removed.
 * - `devflow update` after switch to tdd does NOT silently restore native.
...
describe("devflow workflow integration", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "devflow-workflow-int-"));
  ...
  it("devflow workflow --template native refreshes hash after switching from tdd", async () => {
    stubMarketplaceFetch();
    await init({ yes: true, workflow: "tdd" } as Record<string, unknown>);
    expect(
  ...
  it("devflow workflow --template tdd writes marketplace content and removes the hash", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    expect(loadHashes(tmpDir)[PATHS.WORKFLOW_GUIDE_FILE]).toBeTruthy();
  ...
  it("devflow update after switching to tdd does not silently restore native workflow", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    await runWorkflowCommand({ template: "tdd" });
