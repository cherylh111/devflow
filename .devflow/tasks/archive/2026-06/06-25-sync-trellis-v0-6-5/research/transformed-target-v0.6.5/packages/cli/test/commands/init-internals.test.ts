...
describe("requireSupportedPython", () => {
  ...
  it("throws when the detected version is below the supported floor", () => {
    ...
    expect(() => requireSupportedPython("python3")).toThrow(
      'Python 3.8.18 detected via "python3", but DevFlow init requires Python ≥ 3.9.',
    );
  });
  ...
  it("throws when the command is missing", () => {
    ...
    expect(() => requireSupportedPython("python")).toThrow(
      'Python command "python" not found. DevFlow init requires Python ≥ 3.9.',
    );
  });
  ...
  it("skips the probe entirely when DEVFLOW_SKIP_PYTHON_CHECK=1", () => {
    const prev = process.env.DEVFLOW_SKIP_PYTHON_CHECK;
    process.env.DEVFLOW_SKIP_PYTHON_CHECK = "1";
    try {
      // execSync should not be called at all
      const result = requireSupportedPython("python3");
      expect(result).toBe("version check skipped (DEVFLOW_SKIP_PYTHON_CHECK=1)");
      expect(execSync).not.toHaveBeenCalled();
    } finally {
      if (prev === undefined) {
        delete process.env.DEVFLOW_SKIP_PYTHON_CHECK;
      } else {
        process.env.DEVFLOW_SKIP_PYTHON_CHECK = prev;
      }
    }
  });
});
...
describe("resolveSupportedPython", () => {
  beforeEach(() => {
    vi.mocked(execSync).mockReset();
    resetResolvedPythonCommand();
    delete process.env.DEVFLOW_PYTHON_CMD;
    delete process.env.DEVFLOW_SKIP_PYTHON_CHECK;
  });
  ...
  it("honors DEVFLOW_PYTHON_CMD as an explicit override (no probe)", () => {
    process.env.DEVFLOW_PYTHON_CMD = "py -3.12";
  ...
  it("honors DEVFLOW_SKIP_PYTHON_CHECK=1 as last-resort escape hatch", () => {
    process.env.DEVFLOW_SKIP_PYTHON_CHECK = "1";
