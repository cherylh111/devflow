...
export function requireSupportedPython(command: string): string {
  ...
  if (process.env.DEVFLOW_SKIP_PYTHON_CHECK === "1") {
    return `version check skipped (DEVFLOW_SKIP_PYTHON_CHECK=1)`;
  }
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. DevFlow init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but DevFlow init requires Python ≥ 3.9.`,
    );
  }
 ...
 * `python`) had `devflow init` fail outright.
 *
 ...
 *
 * Honors `DEVFLOW_PYTHON_CMD` (explicit override, no probe) and
 * `DEVFLOW_SKIP_PYTHON_CHECK=1` (skip probe, trust platform default).
 *
...
export function resolveSupportedPython(): {
  ...
  const override = process.env.DEVFLOW_PYTHON_CMD?.trim();
  if (override) {
    setResolvedPythonCommand(override);
    return { command: override, version: "set via DEVFLOW_PYTHON_CMD" };
  }
  ...
  if (process.env.DEVFLOW_SKIP_PYTHON_CHECK === "1") {
    ...
    return {
      command: fallback,
      version: "version check skipped (DEVFLOW_SKIP_PYTHON_CHECK=1)",
    };
  }
  ...
  const installHint = isWindows
    ? `Install Python ≥ 3.9 from https://www.python.org/downloads/windows/ — make sure ` +
      ...
      `installed under a different name, set DEVFLOW_PYTHON_CMD=<your-cmd> ` +
      `before re-running init (e.g. \`set DEVFLOW_PYTHON_CMD=py -3\`).`
    : `Install Python ≥ 3.9 from https://www.python.org/downloads/ or via your ` +
      `package manager. Or set DEVFLOW_PYTHON_CMD=<your-cmd> before re-running.`;

  throw new Error(
    `No supported Python command found. Tried: ${candidates.join(", ")}.\n` +
      `Probe results:\n  ${probeFailures.join("\n  ")}\n\n` +
      `DevFlow init requires Python ≥ 3.9. ${installHint}\n` +
      `Last-resort escape hatch: set DEVFLOW_SKIP_PYTHON_CHECK=1 to skip the probe entirely.`,
  );
}
...
function logPythonAdaptationNotice(command: string): void {
  ...
  console.log(
    chalk.blue(
      `📌 ${osName} detected: DevFlow rendered Python commands as "${command}" in generated hooks, settings, and help text`,
    ),
  );
}
...
function getBootstrapRelatedFiles(
  ...
  if (packages && packages.length > 0) {
    return packages.map((pkg) => `.devflow/spec/${sanitizePkgName(pkg.name)}/`);
  }
  if (projectType === "frontend") {
    return [".devflow/spec/frontend/"];
  }
  if (projectType === "backend") {
    return [".devflow/spec/backend/"];
  }
  return [".devflow/spec/backend/", ".devflow/spec/frontend/"];
}
...
The developer just ran \`devflow init\` on this project for the first time.
\`.devflow/\` now exists with empty spec scaffolding, and this bootstrap task
exists under \`.devflow/tasks/\`. When they want to work on it, they should start
this task from a session that provides DevFlow session identity.

**Your job**: help them populate \`.devflow/spec/\` with the team's real
coding conventions. Every future AI session — this project's
\`devflow-implement\` and \`devflow-check\` sub-agents — auto-loads spec files
...
|------|------------------|
| \`.devflow/spec/backend/directory-structure.md\` | Where different file types go (routes, services, utils) |
| \`.devflow/spec/backend/database-guidelines.md\` | ORM, migrations, query patterns, naming conventions |
| \`.devflow/spec/backend/error-handling.md\` | How errors are caught, logged, and returned |
| \`.devflow/spec/backend/logging-guidelines.md\` | Log levels, format, what to log |
| \`.devflow/spec/backend/quality-guidelines.md\` | Code review standards, testing requirements |
`;
...
| File | What to document |
|------|------------------|
| \`.devflow/spec/frontend/directory-structure.md\` | Component/page/hook organization |
| \`.devflow/spec/frontend/component-guidelines.md\` | Component patterns, props conventions |
| \`.devflow/spec/frontend/hook-guidelines.md\` | Custom hook naming, patterns |
| \`.devflow/spec/frontend/state-management.md\` | State library, patterns, what goes where |
| \`.devflow/spec/frontend/type-safety.md\` | TypeScript conventions, type organization |
| \`.devflow/spec/frontend/quality-guidelines.md\` | Linting, testing, accessibility |
`;
...
\`.devflow/spec/guides/\` contains general thinking guides pre-filled with
...
extract the relevant rules into the matching \`.devflow/spec/\` files —
...
- Every AI coding task spawns two sub-agents: \`devflow-implement\` (writes
  code) and \`devflow-check\` (verifies quality).
- Each task has \`implement.jsonl\` / \`check.jsonl\` manifests listing which
  spec files to load.
- The platform hook auto-injects those spec files + the task's \`prd.md\`
  into every sub-agent prompt, so the sub-agent codes/reviews per team
  conventions without anyone pasting them manually.
- Source of truth: \`.devflow/spec/\`. That's why filling it well now pays
  off forever.
...
\`\`\`bash
${pythonCmd} ./.devflow/scripts/task.py finish
${pythonCmd} ./.devflow/scripts/task.py archive 00-bootstrap-guidelines
\`\`\`
...
"Welcome to DevFlow! Your init just set me up to help you fill the project
...
`;
  ...
  if (packages && packages.length > 0) {
    ...
    for (const pkg of packages) {
      ...
      if (pkgType !== "frontend") {
        content += `\n- Backend guidelines: \`.devflow/spec/${specName}/backend/\`\n`;
      }
      if (pkgType !== "backend") {
        content += `\n- Frontend guidelines: \`.devflow/spec/${specName}/frontend/\`\n`;
      }
    }
...
function getBootstrapTaskJson(
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    relatedFiles,
    notes: `First-time setup task created by devflow init (${projectType} project)`,
  });
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    id: taskName,
    name: taskName,
    title: `Joining: Onboard to this DevFlow project (${developer})`,
    description:
      "Onboard a new developer to an existing DevFlow project: learn the workflow, conventions, and find assigned work",
    status: "in_progress",
    dev_type: "docs",
    priority: "P1",
    creator: developer,
    ...
    createdAt: today,
    notes:
      "Generated by devflow init for a new developer joining an existing DevFlow project",
  });
}
...
\`${developer}\` just ran \`devflow init\` on a fresh clone, saw "Developer
initialized", and will now start asking you questions in chat. This joiner task
exists under \`.devflow/tasks/\`; when they want to work on it, they should
start it from a session that provides DevFlow session identity.

Your job is to orient them to DevFlow. Don't dump all of this at them — open
...
### 1. What DevFlow is + the workflow

DevFlow is a workflow layer over Claude Code / Cursor / etc. that keeps AI
...
- **Three phases**: Plan (brainstorm → \`prd.md\`) → Execute (code + check) →
  Finish (capture + wrap). Full reference: \`.devflow/workflow.md\`.
- **Task lifecycle**: planning → in_progress → done → archive, under
  \`.devflow/tasks/\`.
- **Core slash commands**:
  - \`/devflow:continue\` — resume the current session's active task
  - \`/devflow:finish-work\` — wrap up a finished task
  - \`/devflow:start\` — session boot from scratch (not needed here; the
...
- **\`/devflow:continue\`** loads the Phase Index, reads \`prd.md\` + recent
  activity, and routes to the right skill (\`devflow-brainstorm\` for planning,
  \`devflow-implement\` for coding, \`devflow-check\` for verification).
- **\`devflow-implement\` sub-agent** is spawned when code needs to be written.
  ...
  conventions.
- **\`devflow-check\` sub-agent** follows the same pattern with \`check.jsonl\`
...
- \`.devflow/.runtime/sessions/<session>.json\` — session active-task state, gitignored
- \`.devflow/tasks/<task>/{implement,check}.jsonl\` — per-task context manifests
- \`.devflow/spec/\` — project-wide conventions (source of truth)
- \`.devflow/workspace/${developer}/journal-*.md\` — their session log,
  rotated at ~2000 lines

### 3. This project's actual conventions

- Summarize \`.devflow/spec/\` for them — what coding conventions this
  specific team enforces.
- Point at the last 5 entries in \`.devflow/tasks/archive/\` as a rhythm
...
- Check if \`.devflow/workspace/${developer}/\` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run \`${pythonCmd} ./.devflow/scripts/task.py list --assignee ${developer}\` to
...
P3 task or a typo fix and run the full cycle together: \`/devflow:continue\`
→ you implement via sub-agents → \`/devflow:finish-work\`.
...
\`\`\`bash
${pythonCmd} ./.devflow/scripts/task.py finish
${pythonCmd} ./.devflow/scripts/task.py archive 00-join-${slug}
\`\`\`
...
"Welcome! Your \`devflow init\` set me up to onboard you to this project. I
...
/**
 * Create joiner onboarding task for a new developer on an existing DevFlow
...
/**
 * Handle re-init when .devflow/ already exists.
...
async function handleReinit(
  ...
  if (doAddDeveloper) {
    ...
    // joiner onboarding onto an existing DevFlow project.
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  ${pythonCmd} .devflow/scripts/init_developer.py ${devName}`,
        ),
      );
    }
...
export async function init(options: InitOptions): Promise<void> {
  ...
  // (Claude/Codex/OpenCode session histories etc.) into the devflow hash
  // manifest, and a subsequent `devflow uninstall` would wipe it.
  ...
  // (.devflow/ exists, .developer missing) apart from "creator first init".
  const hadDeveloperFileAtStart = fs.existsSync(
    path.join(cwd, DIR_NAMES.WORKFLOW, FILE_NAMES.DEVELOPER),
  );

  // Generate ASCII art banner dynamically using FIGlet "Rebel" font
  const banner = figlet.textSync("DevFlow", { font: "Rebel" });
  ...
  // ==========================================================================
  // Re-init fast path: skip full flow when .devflow/ already exists
  // ==========================================================================

  // Aborted-init recovery (issue #204): if .devflow/ exists but tasks/ is
  ...
  // `.devflow/workflow.md` hash entry so `devflow update` classifies it as
  ...
  if (developerName) {
    ...
    //
    // Tasks-empty fallback (issue #204): if .devflow/ exists but tasks dir is
