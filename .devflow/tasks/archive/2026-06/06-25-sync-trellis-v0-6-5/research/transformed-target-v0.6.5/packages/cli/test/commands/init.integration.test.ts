...
describe("init() integration", () => {
  ...
  it("#3f devin platform creates .devin/workflows", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".devin", "workflows", "devflow-start.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3l trae platform writes hooks, commands, agents, and tracked templates", async () => {
    ...
    // Trae is agentCapable && hasHooks, so devflow-start is filtered like other
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trae", "commands", "devflow-finish-work.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trae", "commands", "devflow-start.md"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".trae", "agents", "devflow-implement.md")),
    ).toBe(true);
    expect(
      fs.readFileSync(
        path.join(tmpDir, ".trae", "agents", "devflow-implement.md"),
        "utf-8",
      ),
    ).toContain("Load DevFlow Context First");
  ...
  it("#3m zcode platform emits start slash command without shared command-as-skill fallback", async () => {
    ...
    // .zcode/commands/devflow/ instead of the shared .agents/skills/ path
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".zcode", "cli", "agents", "devflow-implement.md"),
      ),
    ).toBe(true);
  });
  ...
  it("#3n opencode platform emits start slash command", async () => {
    ...
    // provides equivalent injection, but the user-invocable /devflow:start is
    // still emitted as fallback for plugin failures / manual reload).
    expect(
      fs.existsSync(
        path.join(tmpDir, ".opencode", "commands", "devflow", "start.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".opencode", "commands", "devflow", "finish-work.md"),
      ),
    ).toBe(true);
  });
