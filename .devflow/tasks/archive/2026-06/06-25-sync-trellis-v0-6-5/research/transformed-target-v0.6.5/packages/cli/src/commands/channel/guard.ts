...
export const ENV_IDLE_TIMEOUT = "DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT";

/** Env var override for the live-worker budget. */
export const ENV_MAX_LIVE_WORKERS = "DEVFLOW_CHANNEL_MAX_LIVE_WORKERS";
 ...
 *   3. `.devflow/config.yaml` `channel.worker_guard`
...
/**
 * Parse the `channel.worker_guard` section out of `.devflow/config.yaml`.
...
export function formatBudgetOverflowError(args: {
  ...
  const hint = [
    ...
    `  devflow channel kill <channel> --as <worker>`,
    "Or override per spawn:",
    `  devflow channel spawn ... --max-live-workers ${live.length + 1}`,
    "Or raise the default in .devflow/config.yaml under channel.worker_guard.max_live_workers.",
  ].join("\n");
