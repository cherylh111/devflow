...
| Kind | Example | Action |
|------|---------|--------|
| **Schema / creator** | `task_store.cmd_create`, `@enpd/devflow-core/task:emptyTaskRecord` (re-exported by `utils/task-json.ts:emptyTaskJson` for legacy CLI call sites) | Drop field from output |
...
`@enpd/devflow-core/task` as `DevFlowTaskRecord` +
...
(runtime) + core `DevFlowTaskRecord` / `emptyTaskRecord` (TS schema + factory) +
