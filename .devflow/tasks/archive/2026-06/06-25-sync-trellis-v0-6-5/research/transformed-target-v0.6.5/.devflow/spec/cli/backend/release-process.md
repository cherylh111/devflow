...
> Release, versioning, docs, and npm publishing rules for the DevFlow monorepo.

---

## Overview

DevFlow publishes two npm packages from one git tag:

| Package | Role | Published by |
|---|---|---|
| `@enpd/devflow` | User-facing CLI | GitHub Actions only |
| `@enpd/devflow-core` | Programmatic core APIs used by the CLI and external integrations | GitHub Actions only |
...
Do not run `npm publish` or `pnpm publish` locally for official DevFlow packages. Local machines may run `pnpm pack`, `release-preflight`, tests, lint, typecheck, and dry-run checks, but not package publication.
...
2. Verify public npm visibility:
   ```bash
   npm view @enpd/devflow@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   npm view @enpd/devflow-core@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   ```
...
| Packed dependency | Published CLI package must depend on `@enpd/devflow-core` with the exact release version. |
...
Full docs details live in `.devflow/spec/docs-site/docs/release-lifecycle.md`.
...
### Contract: the pre-release sweep MUST exclude `.devflow/`

The pre-release `git add` in `release.js` (the `chore: pre-release updates`
commit) **must** exclude `.devflow/` from its pathspec, alongside `docs-site`
and `marketplace`:
...
`.devflow/tasks/` is not gitignored, so a blanket `git add -A` sweeps in any
dirty in-progress task dirs, workspace journal drafts, and runtime artifacts
that happen to be present in the release session. Staging `.devflow/` is only
ever allowed through `common/safe_commit.py`'s precise allowlist (see the
"unscoped `.devflow` staging" bug class in `script-conventions.md`) — never
...
3. core tests
4. CLI tests
5. pre-release commit excluding `docs-site`, `marketplace`, and `.devflow`
...
8. publish `@enpd/devflow-core` if missing
9. publish `@enpd/devflow` if missing
...
"installed automatically", or "included with DevFlow" must be verified against
...
3. Use the built binary (`node packages/cli/bin/devflow.js`) in a fresh temp
   git repository and run the user-facing command that should install the
   asset.
4. Check both the generated files and `.devflow/.template-hashes.json` for the
   expected paths.
5. Run `devflow update --dry-run` from the temp repository and confirm it
...
tmpdir=$(mktemp -d /tmp/devflow-release-smoke-XXXXXX)
printf '{"name":"devflow-smoke","version":"0.0.0"}\n' > "$tmpdir/package.json"
git -C "$tmpdir" init -q
(
  cd "$tmpdir"
  node /path/to/DevFlow/packages/cli/bin/devflow.js init -u smoke --yes --claude --codex
  test -f .claude/skills/<skill>/SKILL.md
  test -f .agents/skills/<skill>/SKILL.md
  grep -q '<skill>' .devflow/.template-hashes.json
  node /path/to/DevFlow/packages/cli/bin/devflow.js update --dry-run
)
```
...
- [ ] Release-claimed bundled assets are verified in `npm pack --dry-run --json` and a fresh temp-directory `devflow init` / `devflow update --dry-run` smoke test.
...
- Core/CLI code ownership and package boundaries: `devflow-core-sdk.md`
- Manifest format and migration types: `migrations.md`
- Docs lifecycle: `.devflow/spec/docs-site/docs/release-lifecycle.md`
