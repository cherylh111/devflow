...
> When DevFlow changes **workflow**, **platforms**, **commands**, or **skills**, which docs-site pages must update in lockstep. A concrete "Audit ALL Writers" checklist for documentation.
...
```bash
cd docs-site
rg -n "task-creation consent|codex-mode|<devflow-workflow>|planning artifact|`design\\.md`|`implement\\.md`" \
...
Scope: any edit to `packages/cli/src/templates/devflow/workflow.md` that adds/removes a step, renames a phase, or changes required/optional/once tags.
...
| `packages/cli/src/templates/common/bundled-skills/devflow-meta/references/customize-local/change-workflow.md` | resume-at status-transition table: `Phase <step> (...)` |
...
Also verify the script tolerates a now-missing step: `python3 .devflow/scripts/get_context.py --mode phase --step <deleted>` must return a friendly "Step not found", not crash.
...
```bash
cd docs-site && grep -rln "devflow-<skill-name>" --include="*.mdx" \
...
See `.devflow/spec/cli/backend/platform-integration.md` → "Agent-Curated JSONL Contract (Phase 1.3)" for the code-side contract.
...
## Trigger 7: `.devflow/config.yaml` Template or Reader Changes

Scope: any edit to `packages/cli/src/templates/devflow/config.yaml`, config readers under `packages/cli/src/templates/devflow/scripts/common/`, or `configSectionsAdded` behavior in update manifests.
...
| `advanced/appendix-a.mdx` | Update the `.devflow/config.yaml` one-line purpose if the file's responsibility changes |
