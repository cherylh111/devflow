# `devflow upgrade` Command

How `devflow upgrade` upgrades the globally installed DevFlow CLI package.

This command is intentionally separate from `devflow update`:

- `devflow upgrade` updates the **CLI binary** by running npm's global install.
- `devflow update` updates a **project's bundled DevFlow files** under `.devflow/`
...
```text
devflow upgrade [--tag <tag-or-version>] [--dry-run]
```

Behavior:

- Builds and runs `npm install -g @enpd/devflow@<tag>`.
...
The implementation does not detect or preserve the original installer. DevFlow
...
```text
which devflow   # POSIX
where devflow   # Windows
```

This catches the common case where npm installed into one global prefix while
the user's shell still resolves an older `devflow` binary earlier on PATH.
...
This applies to CLI startup warnings, `devflow update` downgrade guidance, and
