...
How `devflow update` upgrades a user project's bundled DevFlow assets (Python scripts, workflow.md, AGENTS.md, platform configs) from the version recorded in `.devflow/.version` to the version of the installed CLI.
...
- Input: a project directory containing `.devflow/`, the CLI binary on `PATH`.
- Output: bundled templates on disk match the CLI version; `.devflow/.version` advanced; modified files preserved or backed up; renamed/deleted files migrated when `--migrate`; legacy deprecated files cleaned up via hash-verified `safe-file-delete`; a follow-up migration task created when the upgrade crosses a breaking release with a `migrationGuide`.
- Side effects: snapshot backup at `.devflow/.backup-<timestamp>/`, `.devflow/.template-hashes.json` rewritten, optional `.devflow/tasks/<MM-DD>-migrate-to-<version>/` task tree.

Two big invariants:

1. **Idempotent**: re-running `devflow update` immediately after a successful run prints `✓ Already up to date!` and writes nothing. If you ever see auto-update churn on a clean re-run, the cause is almost always a placeholder unresolved in `collectTemplateFiles` (see Common Pitfalls).
2. **User edits are never silently overwritten**. Anything outside DevFlow-managed templates is in `PROTECTED_PATHS`; anything inside whose hash differs from the recorded one drops to the conflict prompt or `--force` / `--skip-all` / `--create-new` policy.
...
|---|---|
| Python scripts under `.devflow/scripts/` | `templates/devflow/index.ts:getAllScripts` |
...
- **`.devflow/workflow.md`** stays on the normal whole-file template path. `collectTemplateFiles` inserts the bundled `workflowMdTemplate`; `analyzeChanges` decides whether to auto-update, prompt, skip, or create `.new` by comparing the current file hash with `.devflow/.template-hashes.json`. Do not partially merge only `[workflow-state:*]` blocks.
- **`AGENTS.md`** (`commands/update.ts:buildAgentsMdTemplate`) merges only the `<!-- DEVFLOW:START -->`…`<!-- DEVFLOW:END -->` region via `commands/update.ts:replaceDevFlowManagedBlock`; if no markers exist, the template managed block is appended. The legacy untracked-hash whitelist `LEGACY_UNTRACKED_AGENTS_MD_BLOCK_HASHES` lets a pristine pre-tracking AGENTS.md auto-update without a "modified by you" false positive (see `commands/update.ts:isKnownUntrackedTemplate`).

Why workflow is whole-file: `.devflow/workflow.md` is parsed by `get_context.py`,
...
Non-native workflow variants selected through `devflow workflow --template` or
`devflow init --workflow` are deliberately removed from
`.devflow/.template-hashes.json`. That makes `devflow update` classify the file
...
2. **Migration mode** (`commands/update.ts:executeMigrations`): for every `confirm`-bucket migration, treat as `rename`/`delete` (no inline `.backup`). The full snapshot under `.devflow/.backup-<timestamp>/` is the only safety net in this mode — see `update.integration.test.ts > #26 rename-anyway does NOT leave an inline .backup`.
3. **Final confirm** (`commands/update.ts:update`): skip the global `Proceed?` prompt. This is what makes `devflow update --force --migrate` viable for CI / scripted upgrades.
...
There is no `--tag` flag on `devflow update` today. Version selection is implicit: `update()` always uses the version of the installed CLI (`constants/version.ts:VERSION`). Users who want a specific CLI channel should run `devflow upgrade --tag beta` (or `latest` / `rc`) first, then run `devflow update`. The npm-version check in `commands/update.ts:getLatestNpmVersion` only looks at the `latest` dist-tag and is purely advisory ("⚠️ Your CLI is behind npm").
...
`commands/update.ts:update` calls `migrations/index.ts:getMigrationsForVersion(projectVersion, cliVersion)` to get the migration set, then merges in **orphaned migrations** — items whose source still exists and target doesn't, regardless of version range. Orphans show up when a previous update bumped `.devflow/.version` but a migration was skipped or interrupted; they get added to `pendingMigrations` so the next `--migrate` cleans them up.
...
Why hard-fail: the alternative path silently bumps `.devflow/.version` on success, leaving deprecated files orphaned next to the new architecture forever. The user has no signal that something went wrong until much later, when `update` re-flags the same orphan list every release.
...
10. **Migration task creation** — only when the upgrade crosses a manifest with `breaking: true` AND a non-empty `migrationGuide` (collected via `migrations/index.ts:getMigrationMetadata`). `update()` writes `.devflow/tasks/<MM-DD>-migrate-to-<cliVersion>/` containing `task.json` (built via `utils/task-json.ts:emptyTaskJson`) and `prd.md` listing every guide and AI-instruction block. Skipped if the directory already exists. Assignee is read from `.devflow/.developer` via the strict `name=<value>` regex — DO NOT change this to a raw `.trim()` (see Common Pitfalls).
...
`.devflow/.template-hashes.json` is the contract that makes `analyzeChanges` work. Schema and helpers live in `utils/template-hash.ts`. Update interacts with it via:
...
`migrations.md` documents the relationship to `allowed_hashes` in `safe-file-delete` migrations: the hash file tracks "DevFlow-installed bytes" (so update can detect user edits); `allowed_hashes` is a bounded set of "known-pristine bytes" the manifest blesses for auto-deletion. They are different sets — a user file might have a recorded hash but not be `allowed_hashes`-eligible.
...
| Python script bundle | `templates/devflow/index.ts:getAllScripts` |
...
- Snapshot backup at `.devflow/.backup-<timestamp>/`.
...
`init_developer.py` writes `.devflow/.developer` as `key=value` lines:
...
Symptom: every `devflow update` shows the same hooks/settings file as auto-updated. Root cause: a configurator's `configure()` resolves a placeholder before writing, but `collectTemplates` returns the unresolved template. Fix: every placeholder must be resolved in BOTH places. The cleanest pattern is to share a `resolvePlaceholders(...)` call in both code paths inside `configurators/<platform>.ts`. See `platform-integration.md > Common Mistakes > "Template placeholder not resolved in collectTemplates"`.
