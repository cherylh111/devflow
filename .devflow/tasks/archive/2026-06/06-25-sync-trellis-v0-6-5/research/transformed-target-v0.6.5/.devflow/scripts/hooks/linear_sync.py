#!/usr/bin/env python3
"""Linear sync hook for DevFlow task lifecycle.

Syncs task events to Linear via the `linearis` CLI.

Usage (called automatically by task.py hooks):
    python3 .devflow/scripts/hooks/linear_sync.py create
    python3 .devflow/scripts/hooks/linear_sync.py start
    python3 .devflow/scripts/hooks/linear_sync.py archive

Manual usage:
    TASK_JSON_PATH=.devflow/tasks/<name>/task.json python3 .devflow/scripts/hooks/linear_sync.py sync

Environment:
    TASK_JSON_PATH  - Absolute path to task.json (set by task.py)

Configuration:
    .devflow/hooks.local.json  - Local config (gitignored), example:
    {
      "linear": {
        "team": "TEAM_KEY",
...
# DevFlow priority → Linear priority (1=Urgent, 2=High, 3=Medium, 4=Low)
...
def _load_config() -> dict:
    """Load local hook config from .devflow/hooks.local.json."""
    task_json_path = os.environ.get("TASK_JSON_PATH", "")
    if task_json_path:
        # Walk up from task.json to find .devflow/
        devflow_dir = Path(task_json_path).parent.parent.parent
    else:
        devflow_dir = Path(".devflow")

    config_path = devflow_dir / "hooks.local.json"
    try:
