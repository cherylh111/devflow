# `devflow workflow` Command

`devflow workflow` lists and switches the project's active `.devflow/workflow.md`
...
template, marketplace lookup behavior, and `.devflow/.template-hashes.json`
ownership.
...
```text
devflow workflow
devflow workflow --list
devflow workflow --template <id>
devflow workflow --marketplace <source> --template <id>
devflow workflow --template <id> --force
devflow workflow --template <id> --create-new

devflow init --workflow <id>
devflow init --workflow-source <source> --workflow <id>
```
...
```json
{
  ...
  "type": "workflow",
  "name": "TDD Workflow",
  "description": "DevFlow workflow variant that drives Phase 2 with one red / green / refactor behavior slice at a time",
...
- `native` is DevFlow-managed. After writing it, refresh the
  `.devflow/workflow.md` hash with `updateHashes`.
- Every non-native workflow is user-managed local content. After writing it,
  remove `.devflow/workflow.md` from `.devflow/.template-hashes.json` with
  `removeHash`.
- Do not add `workflow.variant` or any other long-lived config field to make
  `devflow update` chase a selected variant. Switching is an explicit project
  action.
...
- `packages/cli/src/templates/devflow/workflow.md` is the source of truth for
  native workflow.
...
| Condition | Behavior |
|---|---|
| `devflow workflow --template <id>` and current workflow is modified | Exit 1 with guidance to use `--force` or `--create-new`; do not prompt, even on a TTY |
| Interactive `devflow workflow` picker and current workflow is modified | Prompt for overwrite, create-new, or skip |
| `--create-new` | Write a generated `workflow.md.new` file beside `.devflow/workflow.md`; do not change active workflow or hash file |
...
- Good: `devflow workflow --template tdd` replaces a pristine native workflow,
  removes the workflow hash, and later `devflow update --skip-all` leaves TDD
  content in place.
- Base: `devflow init --workflow native` writes bundled native workflow and
  keeps `.devflow/workflow.md` hash-tracked.
- Bad: `devflow workflow --template tdd` writes TDD content and records the TDD
  hash. The next `devflow update` sees a pristine file and overwrites it with
  native workflow.
...
- `init --workflow native` keeps `.devflow/workflow.md` hash-tracked.
...
- `devflow workflow --template tdd` writes marketplace content and removes the
  hash.
- Explicit `--template` with modified workflow fails even when `stdin.isTTY` is
  true.
- `--create-new` writes a generated `workflow.md.new` file beside `.devflow/workflow.md` and does not touch the active
  workflow or hash.
- `devflow update` after switching to non-native does not restore native.
...
```bash
python3 ./.devflow/scripts/get_context.py --mode phase
python3 ./.devflow/scripts/get_context.py --mode phase --step 2.1
python3 ./.devflow/scripts/get_context.py --mode phase --step 2.2 --platform codex
python3 ./.devflow/scripts/get_context.py --mode phase --step 2.1 --platform codex-sub-agent
python3 ./.devflow/scripts/get_context.py --mode phase --step 2.1 --platform claude
```
...
fs.writeFileSync(".devflow/workflow.md", tddContent);
updateHashes(cwd, new Map([[PATHS.WORKFLOW_GUIDE_FILE, tddContent]]));
```

This makes `devflow update` auto-replace TDD with bundled native workflow later.
...
An explicit `devflow workflow --template tdd` can hang in a TTY even though it is
