...
> Standards for Python scripts in the `.devflow/scripts/` directory.
...
│   ├── devflow_config.py # Standalone .devflow/config.yaml reader (no task/repo deps)
│   ├── workflow_phase.py # Extract Phase Index / step sections from .devflow/workflow.md (with platform filter)
...
Use this contract when a generated `.devflow/scripts/` module performs an
advisory check during hook/session context generation, such as checking whether
a DevFlow update is available. These checks must never block context output.
...
- Marker files live under `.devflow/.runtime/` and are keyed by the current
  DevFlow session identity when available.
...
- Good: `devflow --version` prints an existing CLI update hint or final version,
  project `.version` is `0.5.0`, so context prints the update hint once.
- Base: `devflow --version` returns `0.5.9`; no registry parsing is needed.
...
- Existing `devflow --version` update output is parsed and normalized.
...
`.devflow/.current-task` directly. The resolver is the single source of truth
for session/window scoped task state:

1. Derive a context key from platform input, `DEVFLOW_CONTEXT_ID`, a
...
2. Read `.devflow/.runtime/sessions/<session-key>.json`.
...
`DEVFLOW_CONTEXT_ID` is a context-key override for subprocesses. It is not a
...
`DEVFLOW_CONTEXT_ID`; without that identity, `task.py start` fails and explains
how to provide a session runtime. For Claude Code, SessionStart receives
`CLAUDE_ENV_FILE`; DevFlow must append `export DEVFLOW_CONTEXT_ID=<context-key>`
...
`DEVFLOW_CONTEXT_ID` from plugin session identity when the command does not
already set it, because some TUI sessions do not expose `OPENCODE_RUN_ID` to
Bash. The prefix must match the host shell: use
`export DEVFLOW_CONTEXT_ID=<context-key>;` for POSIX shells and
`$env:DEVFLOW_CONTEXT_ID = '<context-key>';` for Windows PowerShell. Keep the
...
and write a short-lived `.devflow/.runtime/cursor-shell/*.json` ticket for
...
`tool_call` by prefixing `export DEVFLOW_CONTEXT_ID=<context-key>;`. The Python
resolver then sees the explicit `DEVFLOW_CONTEXT_ID` override; Pi does not need
...
- `task.py create` creates only task-owned files under
  `.devflow/tasks/<date-slug>/`. It must not create `.devflow/.runtime/` and
  must not write `.devflow/.current-task`.
...
- Session state is stored at
  `.devflow/.runtime/sessions/<session-key>.json`. The runtime directory is
  created lazily by the JSON write path.
- Context filenames are derived from the resolved context key:
  - `DEVFLOW_CONTEXT_ID=session-demo` -> `session-demo.json`
...
- `DEVFLOW_CONTEXT_ID` is already a complete context key. Do not prepend a
  platform name to it.
...
| `start` without context key | Fails; no `.runtime`; no `.current-task`; hints IDE/session identity or `DEVFLOW_CONTEXT_ID` |
| `start` with `DEVFLOW_CONTEXT_ID` | Writes `.runtime/sessions/<key>.json`; does not require `.current-task` |
...
- Bad: `task.py create` pre-creates `.runtime`, or any resolver reads/writes
  `.devflow/.current-task` as an active-task fallback.
...
- Regression tests for `start` without a context key failing without creating
  `.current-task`.
- Regression tests for `DEVFLOW_CONTEXT_ID` and platform-native env keys.
...
if not resolve_context_key():
    write_file(".devflow/.current-task", task_path)
```
...
Windows and `python3` on macOS/Linux. `devflow init` probes that same
...
print("Run: {{PYTHON_CMD}} ./.devflow/scripts/init_developer.py <name>")
...
path = Path(".devflow") / "scripts" / "task.py"

# Bad - Unix-only
path = ".devflow/scripts/task.py"
```
...
hooks:
  after_create:
    - "python3 .devflow/scripts/hooks/my_hook.py create"
  after_start:
    - "python3 .devflow/scripts/hooks/my_hook.py start"
  after_archive:
    - "python3 .devflow/scripts/hooks/my_hook.py archive"
```
...
1. Store config in a **gitignored** local file (e.g., `.devflow/hooks.local.json`)
2. Read config at startup, fail with clear message if missing
3. Keep the script itself committable (no hardcoded secrets)

```python
# .devflow/scripts/hooks/my_hook.py — committable, no secrets
CONFIG = _load_config()  # reads from .devflow/hooks.local.json (gitignored)
...
Scripts that auto-stage / auto-commit `.devflow/` paths must go through the
...
### Absolute prohibition: never blanket-stage (`git add -A` / `git add .` / `git add .devflow/`)

> **In this repository, NEVER run `git add -A`, `git add .`, or `git add .devflow/`
> — in any language, any script, any person, any AI. Always stage by precise path.**

Staging `.devflow/` is only ever allowed via one of two precise routes:
...
2. **`release.js`'s precise pathspec** — for release commits. The pre-release
   sweep MUST exclude `.devflow/` (see `release-process.md`).

For a human/AI assembling an ad-hoc commit: `git status` first, then
`git add <path>` per file. Never blanket-stage.

#### Why: "unscoped `.devflow` staging" is a bug CLASS, not one bug (#303)

The same defect — auto-staging more of `.devflow/` than the current scope —
recurs across **three independent triggers**, and a fix to one does not
propagate to the others:

| Trigger | Site | Staging route |
|---|---|---|
| Session auto-commit | `add_session.py:_auto_commit_workspace` | `safe_devflow_paths_to_add` (Python) |
...
This holds for both `safe_devflow_paths_to_add(..., task_name=...)` and
...
|---|---|---|
| `safe_devflow_paths_to_add(repo_root, task_name=None)` | `templates/devflow/scripts/common/safe_commit.py:safe_devflow_paths_to_add` | Path whitelist for `add_session.py` — current developer's journal files + index.md, and (when `task_name` is passed) ONLY the current task dir. Callers MUST pass `task_name` so parallel-window dirty task dirs never leak into the session commit (#303). |
| `safe_archive_paths_to_add(repo_root, task_name=None, modified_children=None)` | `templates/devflow/scripts/common/safe_commit.py:safe_archive_paths_to_add` | Path whitelist for `task.py archive` — archive subtree + explicitly-passed `modified_children` task dirs (parent/child relationship updates). Callers MUST pass `task_name`. |
| `safe_git_add(paths, repo_root)` | `templates/devflow/scripts/common/safe_commit.py:safe_git_add` | Plain `git add -- <paths>`; never `-f`. Returns `(success, used_force=False, stderr)` |
| `print_gitignore_warning(paths)` | `templates/devflow/scripts/common/safe_commit.py:print_gitignore_warning` | Single source of truth for the "ignored by .gitignore" warning, including the AI-defense negative example |
| `get_session_auto_commit(repo_root)` | `templates/devflow/scripts/common/config.py:get_session_auto_commit` | Reads `session_auto_commit` from `.devflow/config.yaml` (default `True`) |
...
### Anti-pattern: AI-invented `git add -f .devflow/`

A real user incident (pre-0.5.10): a project's `.gitignore` listed `.devflow/`
...
`git add -f .devflow/`. That fan-out included every ignored subtree
(`.devflow/.backup-*/`, `.devflow/worktrees/`, `.devflow/.template-hashes.json`,
`.devflow/.runtime/`), committing 548 files / 83474 lines of caches and
backups before anyone noticed.

The root cause is generic fallback hint text in scripts, e.g. "run
`git add .devflow && git commit`" — AI agents see "ignored by" and reinvent
...
intent — putting `.devflow/` in `.gitignore` is an explicit signal "do not
track this." A script silently bypassing that with `-f`, even on a narrow
path list, is unacceptable.

The wider-grain `git add -f .devflow/` stays forbidden, AND the narrow-grain
...
def _auto_commit_workspace(repo_root: Path) -> None:
    ...
    if current:
        paths = safe_devflow_paths_to_add(repo_root, task_name=Path(current).name)
    else:
        # Task unknown (0 / >=2 parallel sessions): stage journal/index only,
        # drop every task dir — do NOT re-open the wide scan.
        paths = [
            p for p in safe_devflow_paths_to_add(repo_root, task_name=None)
            if not p.startswith(".devflow/tasks/")
        ]
    if not paths:
        return
...
`.gitignore` intentionally excludes `.devflow/` and who want session data kept
local-only.

### Pattern: warning text as canonical AI-defense surface

`print_gitignore_warning` in `templates/devflow/scripts/common/safe_commit.py`
...
```
[WARN] Do NOT use `git add -f .devflow/` — it pulls in backups, worktrees,
...
#### Wrong — any blanket stage (`git add -A` / `git add .` / `git add .devflow/`)

```python
# All three blanket the working tree and sweep in untracked files under
# .devflow/ — parallel-window task dirs, .devflow/.backup-*/,
# .devflow/worktrees/, runtime caches. Forbidden everywhere (#303).
subprocess.run(["git", "add", "-A"], cwd=repo_root)
subprocess.run(["git", "add", "."], cwd=repo_root)
subprocess.run(["git", "add", "-A", ".devflow/"], cwd=repo_root)
```
...
# unrelated commit (the original #303 body in safe_devflow_paths_to_add).
for child in sorted(tasks_dir.iterdir()):
    paths.append(f".devflow/tasks/{child.name}")
```
...
paths = safe_devflow_paths_to_add(repo_root, task_name=task_name)
...
- `safe_devflow_paths_to_add` excludes `.devflow/.backup-*`, `.devflow/worktrees/`,
...
- **Parity invariant:** `safe_devflow_paths_to_add(repo_root, task_name=...)`
...
`common/session_context.py` must probe the DevFlow root with
...
All keys in `.devflow/config.yaml` MUST be read through `common/config.py`
(or its hook-side mirror `common/devflow_config.py` for hooks that cannot
...
This is a load-bearing chain. Any new key added to `.devflow/config.yaml`
...
configs with the `# explanation` annotation in `templates/devflow/config.yaml`
break silently.
...
### Pattern: document every key in `templates/devflow/config.yaml`

Every accessor in `common/config.py` must have a corresponding commented-out
example in `packages/cli/src/templates/devflow/config.yaml`, with:
...
# test fixture
config_yaml = """
session_auto_commit: false  # opt out — gitignored .devflow/
...
def _read_session_auto_commit(repo_root: Path) -> bool:
    text = (repo_root / ".devflow/config.yaml").read_text(encoding="utf-8")
...
```yaml
# Auto-detected monorepo packages (written by devflow init)
packages:
  cli:
    path: packages/cli
  docs-site:
...
update:
  skip:
    - .claude/commands/devflow/my-custom.md
...
# Usage
if not repo_root:
    error("Not in a DevFlow project (no .devflow directory found)")
    sys.exit(1)
```
...
**Key rules**:
- Original file path stays stable (e.g., `python3 .devflow/scripts/task.py`)
...
See `.devflow/scripts/task.py` for a comprehensive example with:
