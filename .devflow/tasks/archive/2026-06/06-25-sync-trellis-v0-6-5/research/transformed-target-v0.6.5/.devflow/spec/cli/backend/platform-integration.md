...
- **Common templates**: `src/templates/common/` — single source of truth for commands (start, finish-work), single-file workflow skills (before-dev, brainstorm, check, break-loop, update-spec), and multi-file bundled skills (devflow-meta) with `{{placeholder}}` resolution per platform
...
>
> Trae follows this shared-hook template pattern but writes `.trae/hooks.json`, `.trae/commands/devflow-*.md` with command frontmatter, `.trae/skills/`, `.trae/agents/`, and `.trae/hooks/`. Its main session uses `SessionStart` / `UserPromptSubmit` hooks; sub-agent context remains class-2 pull-based because Trae does not expose a DevFlow-supported sub-agent prompt mutation surface.
...
|-----------|----------|
| `src/templates/{platform}/` | Root directory |
| `src/templates/{platform}/commands/devflow/` | Slash commands (`.md` files) |
...
> Note: OpenCode uses JS plugins instead of Python hooks, has no `index.ts` template module, and has no `collectTemplates` — so `devflow update` does not track OpenCode template files. If a new platform uses JS plugins, follow this pattern.
...
| `src/templates/{platform}/agents/` | Agent definitions (`.md` files — implement, check, research) |
| `src/templates/{platform}/extensions/devflow/index.ts.txt` | Project-local extension source written to `.pi/extensions/devflow/index.ts` |
| `src/templates/{platform}/settings.json` | Platform settings that enable extension, skills, and prompts |

> Note: Pi Agent uses project-local TypeScript extensions instead of DevFlow Python hooks. Keep generated hooks under `.pi/extensions/`, write prompt templates under `.pi/prompts/devflow-*.md`, write Agent Skills under `.pi/skills/`, and do not copy `shared-hooks/*.py` into `.pi/`. Do not redirect Pi to shared `.agents/skills` until shared Agent Skill text is platform-neutral; Codex and Pi command references can differ. For the nested Pi launcher contract, see "Scenario: Pi Sub-Agent Launcher".
>
> Pi is an explicit `devflow-start` exception: `session_start` is notify-only and cannot mutate model-visible context, so the configurator must keep `.pi/prompts/devflow-start.md` as a manual bootstrap fallback while the extension injects compact startup context through `before_agent_start`.
>
> Project-local package isolation rule: when DevFlow enables Pi for a project, `.pi/settings.json` does not include `npm:pi-subagents` in `packages` — DevFlow's own tool is named `devflow_subagent`, so no name collision with community `subagent` tool exists. Users may install community sub-agent packages (nicobailon/pi-subagents or tintinweb/pi-subagents) independently.
...
**Command-as-skill fallback files under `.agents/skills/`** (currently `devflow-start/SKILL.md`, `devflow-continue/SKILL.md`, and `devflow-finish-work/SKILL.md`, written via `resolveAllAsSkillsNeutral()` by Codex) may use per-platform `{{CLI_FLAG}}` / `{{PYTHON_CMD}}` because they are user-invoked fallback entrypoints. They still go through the neutral helper to keep `{{CMD_REF}}` neutralized for consistency with the surrounding shared skills. A platform that has its own command surface (for example ZCode's `.zcode/commands/devflow/*.md`) must write only `resolveSkillsNeutral()` output under `.agents/skills/`; otherwise combined installs with Codex produce last-writer/hash churn on `devflow-start` / `devflow-continue`.
...
- Good: `devflow-meta` installs as `<platform-skill-root>/devflow-meta/SKILL.md` plus `references/**`, and `collectPlatformTemplates(platform)` returns the same files.
- Base: no bundled skills exist; existing `resolveSkills()` / `resolveAllAsSkills()` behavior remains unchanged.
- Bad: platform-specific configurators copy `devflow-meta` manually, creating a second installer that update hash tracking can miss.

##### 6. Tests Required

- Init integration test proving at least Claude and Codex write `devflow-meta/SKILL.md` plus one reference file.
...
- Regression test proving `.devflow/.template-hashes.json` includes bundled skill reference files after init.
- Release smoke test when a changelog or docs page claims the skill is
  ...
  files and `.devflow/.template-hashes.json` contain the skill paths.
...
```typescript
files.set(".claude/skills/devflow-meta/SKILL.md", metaSkillContent);
```
...
**Rule**: Do not add a parallel installer for built-in multi-file skills. If `devflow init` writes a bundled skill file, the platform's `collectTemplates()` path must return the same relative path and byte-identical content so `.devflow/.template-hashes.json` can track it. At minimum, tests must cover one reference file (for example `devflow-meta/references/core/template-pipeline.md`) and the platform-specific install root.
...
built-binary `devflow init` -> `.devflow/.template-hashes.json` -> built-binary
`devflow update --dry-run` with no pending changes.
...
> Note: Platforms using `createTemplateReader(import.meta.url)` in their `index.ts` handle their own template reading. The `extract.ts` functions (`getDevFlowSourcePath()`, `readDevFlowFile()`, `copyDevFlowDir()`) are primarily for the `.devflow/` workflow files, not platform templates.
...
| File | Change |
|------|--------|
| `src/templates/devflow/scripts/common/cli_adapter.py` | Add to `Platform` literal type, `config_dir_name` property, `detect_platform()`, `get_cli_adapter()` validation |
...
| `get_devflow_command_path()` | Command file path format | `.toml` vs `.md`, subdirectory vs flat |
...
| `get_commands_path()` | Command directory structure | `commands/devflow/` or `workflows/` |
...
|---------|------------------|
| Python hooks/statusline/scripts | `.devflow/scripts/common/active_task.py` |
| Existing Python callers | `common.paths.get_current_task()` / `get_current_task_abs()` / `get_current_task_source()` |
| OpenCode plugin | JS resolver in `lib/devflow-context.js`, mirroring `active_task.py` |
| Pi extension | Extension-local resolver using `ctx.sessionManager.getSessionId()` and Bash `tool_call` env injection |

Do not add direct `.devflow/.current-task` reads in hooks, statusline scripts,
...
1. `DEVFLOW_CONTEXT_ID` environment override for subprocesses.
...
plugin must therefore inject a shell-aware `DEVFLOW_CONTEXT_ID` prefix into Bash
tool commands in `tool.execute.before` when the command does not already set
it: POSIX shells use `export DEVFLOW_CONTEXT_ID=<context-key>;`, while Windows
PowerShell uses `$env:DEVFLOW_CONTEXT_ID = '<context-key>';`. Do not infer the
...
must continue to cover POSIX, `env ... DEVFLOW_CONTEXT_ID=...`, and PowerShell
forms.
Cursor must use `beforeShellExecution` as the shell bridge. The hook writes a
short-lived `.devflow/.runtime/cursor-shell/*.json` ticket containing the
...
through ordinary Bash environment. The generated `.pi/extensions/devflow/index.ts`
...
`export DEVFLOW_CONTEXT_ID=<context-key>;` when the command does not already set
one.
Copilot should use
...
through `DEVFLOW_CONTEXT_ID` or a platform-native environment variable exposed
by the host. Hooks and plugins should pass `DEVFLOW_CONTEXT_ID` to subprocesses
they launch. Claude Code is special: SessionStart provides `CLAUDE_ENV_FILE`,
so the shared hook must persist `export DEVFLOW_CONTEXT_ID=<context-key>` there
...
with a shell-aware `DEVFLOW_CONTEXT_ID` assignment using plugin session identity
...
`event.input.command` before Bash execution, and the custom `devflow_subagent` tool must
spawn child `pi` processes with `DEVFLOW_CONTEXT_ID` in `env`.
...
|------|----------|----------------|
| `src/templates/devflow/scripts/common/task_store.py` | `_SUBAGENT_CONFIG_DIRS` (tuple) | Add `.{configDir}/` if the new platform can spawn sub-agents (Class-1 hook-inject, Class-2 pull-based, or extension-backed) |
...
> - **CRITICAL**: Template copy (`src/templates/devflow/scripts/`) must be byte-identical to live copy (`.devflow/scripts/`)
...
If DevFlow project itself should support the new platform:

| Directory | Contents |
|-----------|----------|
| `.{platform}/` | Project's own config directory |
| `.{platform}/commands/devflow/` | Slash commands |
...
Use this pattern when a platform provides project-local JS/TS extension events and custom tools rather than DevFlow-compatible Python hooks. Pi Agent is the reference implementation.
...
```text
.pi/settings.json
.pi/prompts/devflow-<command>.md
.pi/skills/<skill>/SKILL.md
.pi/agents/devflow-<agent>.md
.pi/extensions/devflow/index.ts
```
...
Extension-backed platforms MUST NOT receive `.devflow/templates/shared-hooks/*.py` under their config directory. Their hook-equivalent behavior belongs in generated extension source.

For Pi Agent:

| DevFlow concept | Pi surface |
|---|---|
...
| Startup context | first `before_agent_start` for a resolved context key appends compact SessionStart-equivalent context, `<first-reply-notice>`, `<session-overview>`, and `<devflow-workflow>` to `systemPrompt` |
...
| Per-Bash-tool session identity | `tool_call` extension event; mutates `event.input.command` in place via `injectDevFlowContextIntoBash()` to prefix `export DEVFLOW_CONTEXT_ID=<context-key>;` |
| Sub-agent dispatch | custom `devflow_subagent` tool with `promptSnippet`/`promptGuidelines = SUBAGENT_DISPATCH_PROTOCOL`; resolves the Pi CLI JS entrypoint when possible, runs `--mode text -p --no-session`, sends the delegated prompt through stdin, and forwards `DEVFLOW_CONTEXT_ID` |
...
| Platform can spawn DevFlow sub-agents | Add config dir to `_SUBAGENT_CONFIG_DIRS` |
...
| Extension path contains `spec/` in a skill name such as `devflow-update-spec` | Template hash exclusion must not drop it; only `.devflow/spec/` is user-owned spec data |
...
```text
.pi/extensions/devflow/index.ts
.pi/agents/devflow-implement.md
...
```text
.pi/prompts/devflow-continue.md
.pi/settings.json
```
...
- Runtime script copies (`src/templates/devflow/scripts/**` and live `.devflow/scripts/**`) both recognize the platform.
...
- The TS-port workflow-state regex (`WORKFLOW_STATE_TAG_RE`) matches the same status names and body content as the Python `_TAG_RE` on a shared fixture from `templates/devflow/workflow.md`.
...
```typescript
await writeFile(
  path.join(configRoot, "extensions", "devflow", "index.ts"),
...
Use this contract when `.pi/extensions/devflow/index.ts` launches a nested Pi process for the DevFlow `subagent` tool.
...
spawn(invocation.command, [
  ...
  cwd: projectRoot,
  env: { ...process.env, DEVFLOW_CONTEXT_ID: contextKey },
...
| Field / Env | Contract |
|---|---|
| `DEVFLOW_PI_CLI_JS` | Optional absolute or relative path to `@mariozechner/pi-coding-agent/dist/cli.js`; if set, it is authoritative |
...
| Context | Forward `DEVFLOW_CONTEXT_ID` into the child env when available |
| Agent config | Parse `model`, `thinking`, and `fallbackModels` from `.pi/agents/*.md` frontmatter |
| Per-call overrides | `devflow_subagent` tool input may override frontmatter with `model` and `thinking` |
| Agent validation | `isDevFlowAgent()` checks `existsSync(.pi/agents/devflow-{agent}.md)` before spawn; invalid → returns error text listing community alternatives |
...
| Condition | Behavior |
|---|---|
| `DEVFLOW_PI_CLI_JS` points to an existing file | Launch with `process.execPath` and `[cliJs, "--mode", "text", "-p", "--no-session"]` |
| `DEVFLOW_PI_CLI_JS` points to a missing file | Reject with an error naming `DEVFLOW_PI_CLI_JS` and the resolved missing path |
...
- the generated extension contains `resolvePiInvocation`, `DEVFLOW_PI_CLI_JS`, `process.execPath`, `APPDATA`, npm prefix lookup, and PATH splitting by `delimiter`;
...
- `.pi/extensions/devflow/index.ts` stays byte-for-byte aligned with `src/templates/pi/extensions/devflow/index.ts.txt`;
...
|----------|---------------|-------------|-----------------------|
| Claude Code | `/devflow:xxx` | Markdown (`.md`) | `/devflow:finish-work` |
| Cursor | `/devflow-xxx` | Markdown (`.md`) | `/devflow-finish-work` |
| OpenCode | `/devflow:xxx` | Markdown (`.md`) | `/devflow:finish-work` |
| Gemini CLI | `/devflow:xxx` | TOML (`.toml`) | `/devflow:finish-work` |
...
| Qoder | `/devflow-<name>` (commands) + `$<skill-name>` / `/skills` (workflows) | Markdown (`.md` with frontmatter + `SKILL.md`) | `/devflow-finish-work` |
| Antigravity | `/<workflow-name>` | Markdown (`.md`) | `/finish-work` |
| CodeBuddy | `/devflow:xxx` | Markdown (`.md`) | `/devflow:finish-work` |
| Copilot | `/devflow:xxx` | Markdown (`.prompt.md`) | `/devflow:finish-work` |
| Droid | `/devflow:xxx` | Markdown (`.md`) | `/devflow:finish-work` |
| Devin | `/devflow-xxx` | Markdown (`.md`) + `SKILL.md` | `/devflow-finish-work` |
| Pi Agent | `/devflow-xxx` prompt templates + `/skill:<name>` skills | Markdown (`.md`) + `SKILL.md` + TypeScript extension | `/devflow-finish-work` |
| Trae IDE | `/devflow-xxx` commands + skills | Markdown (`.md` with frontmatter) + `SKILL.md` + `hooks.json` | `/devflow-finish-work` |
...
|---------|--------------------------------|----------------------------------|---------------------|
| `start` | ❌ filtered by the shared resolver — SessionStart-style hook injects opening context, user-facing `/start` would be redundant. Pi is the approved exception and re-adds `.pi/prompts/devflow-start.md` because `session_start` is notify-only. | ✅ emitted (skill and/or slash command per platform) — no hook fires, users need an invocable `start` | ✅ emitted — manual equivalent of session-start hook |
...
| **Class-2** — Pull-based | Platform's hook can't reliably mutate sub-agent prompts; DevFlow injects a "Required: Load DevFlow Context First" prelude into each sub-agent definition file so the sub-agent reads context itself at startup | Codex, Gemini CLI, Qoder, Copilot, ZCode, Reasonix, Trae IDE |
| **Class-3** — Extension-backed | Platform exposes hook-equivalent events and custom tools through a project-local TypeScript extension; DevFlow owns the sub-agent tool and the context injection path | Pi Agent |

### Class-1 — Hook-inject (6 platforms)

Platform's PreToolUse-equivalent hook can fire on the sub-agent spawn tool AND modify the tool's prompt input. DevFlow's `inject-subagent-context.py` (or OpenCode's plugin) reads `prd.md` + the JSONL-referenced spec files and rewrites the sub-agent's initial prompt.
...
```js
import { isDevFlowSubagent } from "../lib/devflow-context.js"

"chat.message": async (input, output) => {
  if (isDevFlowSubagent(input)) {
    // input.agent matched /^devflow-(implement|check|research)$/
...
`isDevFlowSubagent()` lives in `lib/devflow-context.js`; the regex matches `devflow-implement` / `devflow-check` / `devflow-research` exactly.
...
3. Single-session fallback in `DevFlowContext._resolveSingleSessionFallback()` — only when exactly 1 file exists in `.devflow/.runtime/sessions/`; refuses to guess when 0 or ≥2 files exist (source `session-fallback:<context_key>`). Mirrors Python `_resolve_single_session_fallback` (`active_task.py:497-519`).

`buildPrompt()` for implement / check / finish / research **must** prepend `<!-- devflow-hook-injected -->` so generated agent definitions in `.opencode/agents/*.md` can detect a successful injection (DevFlow-internal contract; OpenCode itself ignores the marker).

`getActiveTask()` in `lib/devflow-context.js` itself includes the single-session fallback so any caller (`workflow-state` breadcrumb, `session-start` task status) sees the same resolved task as the prompt injector. The fallback only activates when the explicit context-key lookup misses, so multi-window setups remain isolated.

### Class-2 — Pull-based (7 platforms)

Platform's hook either doesn't expose a sub-agent spawn event or can't modify the prompt. Sub-agents must Read context themselves at startup. DevFlow injects a "Required: Load DevFlow Context First" prelude into each sub-agent definition file.
...
| ZCode | No DevFlow-supported hook surface for sub-agent prompt mutation; generated `.zcode/cli/agents/*.md` files receive the pull-based prelude. |
...
| Trae IDE | `SessionStart` / `UserPromptSubmit` hooks cover main-session context, but no DevFlow-supported sub-agent prompt mutation surface exists; generated `.trae/agents/*.md` files receive the pull-based prelude. |
...
1. **`Active task: <path>` line in dispatch prompt** — primary path. The main agent is required by `workflow.md`'s `[workflow-state:in_progress]` breadcrumb to prefix every sub-agent dispatch (including `devflow-research`, since 0.5.8) with `Active task: <path from task.py current>`. The breadcrumb fires on every `UserPromptSubmit` while `task.json.status == in_progress`, so the rule is reinjected per turn.
...
`templates/pi/extensions/devflow/index.ts.txt` registers handlers for three platform events plus one custom tool. Each injection point has a distinct lifecycle and a distinct failure mode if dropped.
...
| `tool_call` (Bash) | `pi.on?.("tool_call", …)` | every Bash tool call | mutates `event.input.command` in place via `injectDevFlowContextIntoBash()` to prefix `export DEVFLOW_CONTEXT_ID=<context-key>;` so child Python scripts (e.g. `task.py current`) inherit session identity |
| `devflow_subagent` tool | `pi.registerTool?.({ name: "devflow_subagent", … })` | extension load time (once) | `promptSnippet` and `promptGuidelines` carry `SUBAGENT_DISPATCH_PROTOCOL` so the model sees the dispatch contract before it ever calls the tool |
...
- **Regex parity**: `templates/pi/extensions/devflow/index.ts.txt:WORKFLOW_STATE_TAG_RE` MUST mirror `templates/shared-hooks/inject-workflow-state.py:_TAG_RE` byte-for-byte. Both use the closing-tag backreference `\1` (or its TS equivalent in `[\/workflow-state:\1\]`) so a tag block parses identically in Python and TypeScript.
- **Breadcrumb body source**: `loadWorkflowBreadcrumbs()` in the Pi extension reads `.devflow/workflow.md` directly — same source as the Python hook. There is no separate TS-side template for breadcrumb bodies. If the regex drifts, the TS port silently falls back to hardcoded defaults and Pi loses parity.
...
- **`<session-overview>` parity**: Pi shells out to `python3 .devflow/scripts/get_context.py` rather than re-implementing context generation in TS, so output stays canonical. Don't replace this with an inline TS implementation — that's a parity drift waiting to happen.
...
function onInput(event, ctx) {
  const overview = spawnSync("python3", [".devflow/scripts/get_context.py"]);
...
|---|---|---|
| Workflow breadcrumb | `templates/devflow/workflow.md` `[workflow-state:in_progress]` block | Python `inject-workflow-state.py` and the Pi TS port — surfaced per-turn while a task is in progress |
| Pi extension constant | `templates/pi/extensions/devflow/index.ts.txt:SUBAGENT_DISPATCH_PROTOCOL` | Pi `devflow_subagent` tool's `promptSnippet` / `promptGuidelines` — surfaced at extension load and on each tool description render |
...
Every generated `devflow-implement` and `devflow-check` agent definition must
...
`devflow-implement` or `devflow-check` applies only to the main session, and
...
`.devflow/tasks/archive/2026-04/04-17-subagent-hook-reliability-audit/research/platform-hook-audit.md`
...
```json
{"_example": "Fill with {\"file\": \"<path>\", \"reason\": \"<why>\"}. Put spec/research files only — no code paths. Run `python3 .devflow/scripts/get_context.py --mode packages` to list available specs. Delete this line when done."}
```
...
```bash
python3 ./.devflow/scripts/task.py create "<title>" --slug <name> --parent <parent-dir>
...
`.devflow/workflow.md` contains per-phase step detail under `#### X.X` headings, with per-platform variants demarcated by `[Platform Name, ...]` … `[/Platform Name, ...]` blocks.

Load step detail on demand (both commands and hooks use this):

```bash
python3 ./.devflow/scripts/get_context.py --mode phase                                   # Phase Index (no --step)
python3 ./.devflow/scripts/get_context.py --mode phase --step 1.1                        # Step 1.1 (all platforms)
python3 ./.devflow/scripts/get_context.py --mode phase --step 1.2 --platform cursor      # Step 1.2, cursor-filtered
```
...
When a hook can resolve the DevFlow project directory before printing, prefer
the shared helper from `.devflow/scripts/common/__init__.py`:
...
For standalone hooks that cannot safely import `.devflow/scripts/common`, use
...
Every DevFlow-owned SessionStart implementation that injects model-visible
...
only in implementations where DevFlow can actually put context into the model
conversation:
...
| `opencode/plugins/session-start.js` | ✅ | Plugin prepends DevFlow context into the first user message and persists it |
| `pi/extensions/devflow/index.ts.txt` | ✅ | Pi cannot inject through `session_start`, so the first `before_agent_start` emits a compact SessionStart-equivalent payload into `systemPrompt` |
| `copilot/hooks/session-start.py` | ❌ | Microsoft documents `SessionStart.hookSpecificOutput.additionalContext` (preview, VS Code 1.110+), but consumption depends on the user's VS Code/Copilot version. DevFlow emits the spec-compliant payload; do not add a first-reply notice until consumption is verified end-to-end. |
...
### Size Budget (measured on DevFlow dev repo)

| Block | Size | Notes |
|---|---:|---|
| `<session-context>` | 0.1 KB | Fixed |
| `<current-state>` | 0.3 KB | Compact developer/git/task state |
| `<devflow-workflow>` | 4.4 KB | Compact Phase Index after stripping workflow-state blocks, comments, and platform markers; detailed phase bodies are loaded on demand |
| `<guidelines>` | 0.5 KB | Context order + spec index paths only |
| `<ready>` | 0.1 KB | Fixed |
| **Total** | **6.0 KB** | **Under 20 KB ✓** |

Historical note: pre-workflow-rewrite (v0.4.0-beta.10) the payload included a 16 KB `<instructions>` block (start.md content). Later iterations injected a large `<workflow>` block. Current SessionStart uses `<devflow-workflow>` with a compact Phase Index and leaves detailed steps to `/devflow:continue` / phase-context loading.

### Guidelines: Paths-only

Before: every `.devflow/spec/*/index.md` was inlined in `<guidelines>` (10 KB+
...
index paths, including `.devflow/spec/guides/index.md`. Agents read the relevant
...
1. compact current state (developer, git summary, active task, journal, spec
   index count)
2. compact `<devflow-workflow>` Phase Index
...
through `/devflow:continue`, `get_context.py --mode phase --step <X.Y>`, skills,
...
`SessionStart` only fires once per session. In long conversations, Claude's context compression can push the SessionStart message out of recent context, and the AI forgets the active DevFlow task — resulting in workflow drift (skips `check`, forgets to `update-spec`, doesn't return to `finish` after user interruptions).
...
Breadcrumb text lives in `workflow.md` as `[workflow-state:STATUS]...[/workflow-state:STATUS]` blocks (same tag style as existing `[Platform, ...]` blocks). Users who fork the DevFlow workflow edit **only the markdown**; the hook script stays untouched.

```markdown
[workflow-state:in_progress]
Flow: devflow-implement → devflow-check → devflow-update-spec → finish
Next required action: inspect conversation history + git status, then execute the next uncompleted step in that sequence.
For agent-capable platforms, do NOT edit code in the main session; dispatch `devflow-implement` for implementation and dispatch `devflow-check` before reporting completion.
[/workflow-state:in_progress]
```
...
4. No session active task → emit `no_task` pseudo-status breadcrumb instead of silent-exit. Header is `Status: no_task`; body nudges AI to load `devflow-brainstorm` + `task.py create` for multi-step work (or answer directly for trivial asks).
...
```
Net effect on a "no task" session: AI sees the Next-Action only at SessionStart; after 20 turns of context compression, the guidance is gone and AI forgets to use `devflow-brainstorm` for new multi-step requests.
...
The same rule applies to every other hook that's positioned as "repeated reminder": if the hook isn't emitting, the reminder loop is broken. The only legitimate silent-exit case is when **the hook doesn't own this codebase at all** (e.g. `.devflow/` not found → definitely not a DevFlow project → OK to no-op).
...
| Codex | `UserPromptSubmit` | `.codex/hooks.json` | **Requires `features.hooks = true` in user's `~/.codex/config.toml` (Codex 0.129+; legacy: `codex_hooks = true`).** Codex 0.129+ also requires running `/hooks` once to approve the installed hook before it activates — until approved, hooks never fire (the devflow-bootstrap fallback in inject-workflow-state.py covers the gap by directing the AI to read `devflow-start` skill manually) |
| OpenCode | `chat.message` (Bun plugin) | `plugins/inject-workflow-state.js` | Equivalent JS implementation |
| Kiro | CLI: `userPromptSubmit` (main `devflow` agent JSON) · IDE: `promptSubmit` (`.kiro/hooks/*.kiro.hook`) | CLI: `.kiro/agents/devflow.json` `hooks` · IDE: `.kiro/hooks/devflow-workflow-state.kiro.hook` (`then.runCommand`) | Plain stdout (NOT the `hookSpecificOutput` JSON envelope) — Kiro adds a hook's raw stdout to conversation context. `inject-workflow-state.py` has a `platform == "kiro"` branch (detected via `KIRO_PROJECT_DIR` env or `.kiro` script path) that prints the bare breadcrumb. **Activation:** CLI users must make `devflow` the active agent (`kiro-cli settings chat.defaultAgent devflow` or `/agent swap devflow`) — Kiro defaults to built-in `kiro_default`. **Real-machine note:** the plain-stdout→context contract (and whether IDE `runCommand` stdout is injected vs only `askAgent`) is per official docs, pending Kiro hardware verification; fallback is `askAgent` + static steering. Sub-agent context injection unchanged via `agentSpawn → inject-subagent-context.py` |
| Trae IDE | `UserPromptSubmit` | `.trae/hooks.json` | Auto; shared Python hook written under `.trae/hooks/inject-workflow-state.py` |

### CWD Robustness

The hook uses `find_devflow_root()` to walk up from CWD until it finds `.devflow/`, so it works when the terminal is in a subdirectory (monorepo package, etc.) or when sub-agent spawn inherits a drifted CWD.

### Why No State Machine / No Extra `task.json` Fields

After first-principles analysis (historical task:
`.devflow/tasks/archive/2026-04/04-17-workflow-enforcement-v2/prd.md`), we
...
`devflow init` generates a first-session task based on checkout state. Three branches dispatch off two filesystem flags:

| `.devflow/` exists? | `.devflow/.developer` exists? | Meaning | Task generated |
|---|---|---|---|
...
**Options Considered**:
1. `.devflow/workspace/<name>/` directory existence — ❌ this dir is committed to git, so a fresh clone already has it
2. A registry file listing onboarded developers — ❌ needs migration + bookkeeping, over-engineered for single-user checkouts
3. `.devflow/.developer` file existence — ✅ **chosen**

**Decision**: Use `.devflow/.developer` (gitignored) as the per-checkout onboarding signal.

**Why**: `.devflow/.developer` is declared in `.devflow/.gitignore` (template `gitignore.txt`), so it is never committed. A fresh clone has an empty `.developer` slot by construction; the first `init` writes it. Subsequent same-machine re-inits see the file and no-op.
...
**Anti-pattern**: Do not use `.devflow/workspace/<name>/` existence as "this developer already onboarded" — that directory is the journal archive and belongs to git.

### Gotcha: Joiner Dispatch Must Be Wired in Two Places

`devflow init` has two code paths that both reach the end of initialization but through different branches of `init()`. Any new init-time trigger (joiner onboarding, future first-session tasks, etc.) must be registered in **both**:
...
- Reached when `.devflow/` already exists AND user runs default `devflow init --user <name>` (no `--force`, no `--skip-existing`)
...
**Symptom**: `devflow init` configures the platform, but `devflow update` doesn't track its template files.
...
**Symptom**: In production builds (`dist/`), `devflow init` copies compiled `index.js` (and `.js.map`, `.d.ts`) into the user's config directory (e.g., `.gemini/index.js`).
...
**Symptom**: `devflow init --{platform}` doesn't work.
...
**Symptom**: `devflow update` auto-updates platform files on every run, even when nothing changed. The update summary shows hooks/settings as "changed".
...
**Symptom**: On a freshly initialized project that used an opt-in feature (e.g., `--with-statusline`), the very first `devflow update` reports `.claude/settings.json` as "Template updated (will auto-update)", rewrites it, and leaves a spurious backup — with zero actual changes.
...
**Symptom**: `devflow update` always detects a "new file" to add, even on a freshly initialized project with the same version.
...
**Symptom**: Pure backend project gets empty frontend spec templates after `devflow init`. After user deletes the unwanted `spec/frontend/` dir, `devflow update` recreates it.
...
- Command invocation syntax (e.g., `/devflow:xxx` vs `$skill-name`)
...
**Symptom**: The trigger works when users pass `--force` / `--skip-existing` / run init on an empty dir, but the default `devflow init --user <name>` on an existing checkout silently does nothing. Integration tests pass.

**Cause**: `init()` at `src/commands/init.ts` early-returns into `handleReinit` when `.devflow/` already exists and neither `--force` nor `--skip-existing` is set. Main dispatch at the end of `init()` is never reached. If the new trigger is only wired into main dispatch, the most common real-user path is uncovered.
