...
How `devflow uninstall` performs **paragraph-level deletion** on structured config files (`settings.json`, `hooks.json`, `config.toml`, `package.json`) so that DevFlow-emitted fields are removed while user-added neighbors stay intact.
...
Most files DevFlow writes are opaque (`.py`, `.md`, `.ts`) — `devflow uninstall` `unlink`s them outright. But a handful of platform config files are **shared** with the user:

| File | What's shared |
|------|----------------|
| `.claude/settings.json` | DevFlow writes the `hooks` block; user may have set `env`, `model`, `permissions`, `version` |
| `.cursor/hooks.json` | Same idea, but a flat schema |
| `.opencode/package.json` | DevFlow adds `dependencies["@opencode-ai/plugin"]`; user may have other deps |
| `.pi/settings.json` | DevFlow adds `enableSkillCommands` plus entries in `extensions`/`skills`/`prompts` arrays; user may have entries of their own |
| `.codex/config.toml` | DevFlow writes a documented `project_doc_fallback_filenames` line + a comment block; user may have added more TOML directives |
...
If `uninstall` simply `rm`-ed these files, the user would lose their own config. If it **left** them alone, the dangling DevFlow hook entries would point at deleted scripts and the platform would error on the next session.

The scrubbers walk each file's structure, drop only the DevFlow-known parts, and report whether anything meaningful remains.
...
- Identifying *which* paths count as "deleted by this uninstall" (passed in to hooks-shaped scrubbers as `deletedPaths`). This is the full list of POSIX paths from `.devflow/.template-hashes.json`.
...
Hooks-JSON scrubbers need the delete-set because they identify DevFlow hook entries by **whether the entry's command refers to a path being deleted**. The other three identify DevFlow content by exact-match values that DevFlow-the-configurator hard-codes.
...
User-defined keys outside `hooks` (`env`, `model`, `permissions`, `version`) are preserved verbatim — only the DevFlow-claimed `hooks` subtree is touched.
...
This rule assumes the DevFlow-emitted shape:
...
`utils/uninstall-scrubbers.ts:getEntryCommand` reads `command` first, then falls back to `bash`, then `powershell`. Copilot's flat schema uses dual `bash`/`powershell` fields instead of a unified `command`. Either field is enough to identify a DevFlow entry; we don't require both to match because DevFlow emits the same script path on both fields.
...
1. Drop `enableSkillCommands` (DevFlow-only flag).
2. Filter three arrays for the DevFlow-emitted entries (exact string match):
   - `extensions` — remove `"./extensions/devflow/index.ts"`
...
Constants `PI_DEVFLOW_EXTENSION`, `PI_DEVFLOW_SKILLS`, `PI_DEVFLOW_PROMPTS` in `utils/uninstall-scrubbers.ts` define the exact strings the Pi configurator emits. If the configurator changes the path emitted, this scrubber must change in lockstep — there is no shared source of truth across the two halves.

### `utils/uninstall-scrubbers.ts:scrubCodexConfigToml`

Scrubs `.codex/config.toml`. Unlike the JSON scrubbers, this one is **line-based**: TOML is harder to round-trip without a real parser, and the DevFlow-emitted file is small + flat enough that a marker-line approach is safer than a structural one.

DevFlow writes two distinct content classes into this file:
...
- Walk lines. Drop any line that:
  ...
  - Is a comment line whose inner text (after stripping `#` and spaces) **exactly** matches one of the strings in `devflowCommentMarkers` (a hard-coded array inside `utils/uninstall-scrubbers.ts:scrubCodexConfigToml`).
  - Is a bare `#` comment line — these are inside the DevFlow comment block.
- Collapse consecutive blank lines created by removals.
- Trim trailing blanks.
- `fullyEmpty` iff the result has no non-whitespace characters.

User-added lines (their own TOML keys, their own comments, blank gaps) survive because they do not match the assignment regex AND their comment text is not in `devflowCommentMarkers`.

---

## Marker block format

Scrubbers identify DevFlow content via three distinct mechanisms — there is no single uniform marker syntax.
...
| **Hard-coded comment-line allowlist** + assignment regex | `scrubCodexConfigToml` | Lines whose stripped text matches any of `devflowCommentMarkers` |

### Why no "BEGIN DEVFLOW / END DEVFLOW" comment markers?

Earlier designs considered wrapping DevFlow content in delimited blocks (`# BEGIN DEVFLOW …` / `# END DEVFLOW`). We rejected that because:
...
- `commands/uninstall.ts:buildPlan` reads `.devflow/.template-hashes.json` and only considers manifest-listed files. Files outside the manifest are never seen by any scrubber.
...
**Symptom**: `devflow uninstall` leaves stale DevFlow fields in a platform config file because the scrubber's hard-coded matcher (`PI_DEVFLOW_EXTENSION`, `devflowCommentMarkers`) doesn't recognize the new emission.

**Cause**: configurator and scrubber maintain parallel hard-coded tables of "what DevFlow writes". When the configurator changes (e.g. moves the Pi extension to a new path), the scrubber's table goes stale.
...
**Symptom**: After `devflow uninstall`, a `.codex/config.toml` retains half of the DevFlow comment block (e.g. the user deleted `# NOTE:` but left `# Without this flag, …`).

**Cause**: `scrubCodexConfigToml` matches on **per-line exact text**, not on a block boundary. Any surviving DevFlow-known line will be removed individually; any user-edited line whose text no longer matches the allowlist will be preserved.

**Mitigation**: this is the correct behavior — we cannot tell whether a near-miss line is a typo or an intentional user customization. Documentation should warn users: editing DevFlow-emitted comments may leave fragments after uninstall. They can always delete manually.
...
**Symptom**: A platform's `hooks.{Event}` array contains two matcher blocks that both target DevFlow. After scrubbing, both should be removed.

**Mitigation**: `scrubHooksJson` already filters per-entry independently. Duplicate DevFlow entries are handled correctly. Nested-within-nested is not a real shape any platform emits — the schema is exactly two levels deep — but the scrubber's per-entry filter wouldn't blow up either; it just wouldn't recurse further.

### External tool rewrites the file before uninstall

**Symptom**: A user's editor or formatter normalized `.codex/config.toml` (e.g. reordered keys, changed comment wrapping). The scrubber leaves DevFlow content behind because it didn't match the allowlist exactly.

**Mitigation**: the line-allowlist approach is intentionally strict to avoid false positives. If a user's formatter has rewritten DevFlow content, we treat it as user-customized and preserve it. Document the workaround: re-run `devflow init` to restore canonical content, then `devflow uninstall` to remove it cleanly.
...
**Rule**: do not bypass this gate. Adding "scrub any file with this shape" logic outside the manifest gate would risk modifying user files DevFlow never wrote.
...
1. **Fixture** — a string literal of the file content (with a DevFlow section + a user-owned section).
2. **Call** — invoke the scrubber directly.
3. **Assert** — DevFlow section is gone, user section is intact, `fullyEmpty` matches expectation.

### Required test cases per scrubber

| Case | What to assert |
|------|----------------|
| Pure DevFlow content | After scrub, `fullyEmpty === true` |
| Mixed DevFlow + user content | After scrub, `fullyEmpty === false`; user content survives byte-for-byte (modulo JSON re-pretty-print) |
...
- `scrubCodexConfigToml`:
  - User added their own TOML keys above/below the DevFlow block → preserved.
  - User edited a DevFlow comment line (typo) → that single line preserved as user content; rest of DevFlow block removed.
- `scrubPiSettings`:
  - User has their own entry in `extensions`/`skills`/`prompts` → kept; only DevFlow entries removed.
- `scrubOpencodePackageJson`:
  - User has other dev/runtime deps → kept.

### Cross-cutting integration test

`commands/uninstall.ts` integration tests should cover the **full** init → uninstall round-trip per platform: confirm that after `init({ <platform>: true })` followed by `uninstall({ yes: true })`, the platform config dir is either gone (if DevFlow was the only writer) or contains only the user's pre-existing content. This catches regressions where a configurator change isn't mirrored in a scrubber update.
...
- `commandMatchesDeletedPath` assumes the DevFlow-emitted command has the exact shape `<python-cmd> <script-path>`. If we ever add launcher flags or wrappers, the helper needs a richer parser (full token scan, possibly drop known shell prefixes like `env VAR=val`).
- The Pi exact-string constants (`PI_DEVFLOW_EXTENSION`, `PI_DEVFLOW_SKILLS`, `PI_DEVFLOW_PROMPTS`) duplicate values that live in the Pi configurator. A shared module exporting these would prevent drift; today they are independently hard-coded in two places.
- `scrubCodexConfigToml`'s comment-line allowlist (`devflowCommentMarkers`) is a hand-maintained list mirroring the configurator's emitted comment block. Same drift risk as Pi. Consider deriving the list from the same template file the configurator uses.
