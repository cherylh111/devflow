...
- **Roots**: inspect the default root under
  ...
  to the current DevFlow process. Custom session dirs contain direct `.jsonl`
...
- Good: `devflow mem search kw --platform pi --cwd /repo` searches only the
  cleaned active branch for `/repo`, with compaction already applied.
- Base: `devflow mem list --platform pi --json` returns metadata rows with the
...
- New surface forms (e.g., `tl task create` if DevFlow ever ships a wrapper)
