...
`@enpd/devflow-core` owns reusable SDK/domain primitives that must stay
...
- CLI code must import core through public exports such as
  `@enpd/devflow-core/channel`, not deep paths under `packages/core/src`.
...
- Detailed package-boundary rules currently live in
  `.devflow/spec/cli/backend/devflow-core-sdk.md`; keep this file and that
  boundary spec consistent until the detailed core rules are split fully under
  `.devflow/spec/core/`.

## Pre-Development Checklist

- Read `.devflow/spec/cli/backend/devflow-core-sdk.md` before editing
  `packages/core/**` or moving logic between CLI and core.
- Read `.devflow/spec/cli/unit-test/conventions.md` before adding or changing
  core tests.
- For channel changes, also read
  `.devflow/spec/cli/backend/commands-channel.md`.
- For mem changes, also read `.devflow/spec/cli/backend/commands-mem.md`.
...
```bash
pnpm --filter @enpd/devflow-core lint
pnpm --filter @enpd/devflow-core typecheck
