# `devflow channel` — Multi-Agent Collaboration Runtime (Code Spec)
...
| Env wiring (`DEVFLOW_CHANNEL_ROOT/PROJECT/AS`) | Cross-process configuration |

### Current Core / CLI Boundary

`@enpd/devflow-core/channel` owns reusable channel domain behavior:
...
```
devflow channel create <name> [opts]
  --scope <scope>        : project | global (default project)
  --type <type>          : chat | forum (default chat)
  --task <path>          : associated DevFlow task directory (string)
...
devflow channel spawn <name> [opts]
  --scope <scope>        : project | global
  --agent <name>         : load .devflow/agents/<name>.md (sets provider / as / system prompt)
  ...
  --idle-timeout <duration>: OOM-guard idle-cleanup TTL for this worker
                           (default 5m from .devflow/config.yaml; "0" disables idle cleanup;
  ...
  --max-live-workers <n> : spawn-time live-worker budget for this project/scope
                           (default 6 from .devflow/config.yaml; "0" disables the
...
devflow channel send <name> [text] [opts]
...
devflow channel interrupt <name> [text] [opts]
...
devflow channel wait <name> [opts]
...
devflow channel messages <name> [opts]
  --scope <scope>        : project | global
...
devflow channel title clear <name> [opts]
  --scope <scope>        : project | global
  --as <agent>           : author identity (default "main")
  → stdout: appended `channel` title clear event as JSON

devflow channel prune [opts]
  --scope <scope>        : project | global
...
devflow channel post <name> <action> [opts]
...
devflow channel context add <name> [opts]
  --scope <scope>        : project | global
  --as <agent>           : author identity (default "main")
...
devflow channel context delete <name> [opts]
  --scope <scope>        : project | global
  --as <agent>           : author identity (default "main")
  --thread <key>         : mutate thread-level context instead of channel-level context
  --file <abs-path>      : absolute context file (repeatable)
...
devflow channel context list <name> [opts]
  --scope <scope>        : project | global
...
channelRoot(): string                                       // DEVFLOW_CHANNEL_ROOT ?? ~/.devflow/channels
projectKey(cwd: string): string                             // sanitize: /[\\/_]/g→"-" then /[^A-Za-z0-9.-]/g→"-"
currentProjectKey(): string                                 // DEVFLOW_CHANNEL_PROJECT env ?? projectKey(process.cwd())
...
metadataFromCreateEvent(ev?): ChannelMetadata
  ...
  // @enpd/devflow-core/channel and do not call from CLI renderers.
...
**Worker lifecycle / inbox / delivery contracts** (owned by `@enpd/devflow-core`):
...
- `sendMessage` delivery modes: `appendOnly` (default — append-only / pre-spawn
  ...
  `undeliverable`. CLI exposes this through `devflow channel send
  --delivery-mode <mode>`.
- Interrupt is a first-class API, not a magic tag. `requestInterrupt` appends
  ...
  `devflow channel interrupt`; message tags are not an interrupt path.
...
- Trigger: `@enpd/devflow-core` mutation APIs need replay safety for
...
```
DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT=5m
DEVFLOW_CHANNEL_MAX_LIVE_WORKERS=6
```

#### 3. Contracts

- Configuration precedence is CLI flag → env → `.devflow/config.yaml` →
...
| `idle_timeout: 0` or `DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT=0` | idle cleanup disabled; workers are still counted for budget unless budget is also disabled |
| `max_live_workers: 0` or `DEVFLOW_CHANNEL_MAX_LIVE_WORKERS=0` | budget check disabled; supervisor idle self-termination still works if TTL > 0 |
| Live count after expired-idle cleanup is `>= maxLiveWorkers` | reject `spawn` with live worker list, `devflow channel kill` hints, and override hint |
...
- `origin` records the public write entrypoint: `cli` for
  `devflow channel ...`, `api` for the future channel core/library, and
  `worker` for supervisor / worker runtime writes.
- `meta` is a pass-through JSON object for DevFlow runtime details and
  external systems. DevFlow persists it, emits it in `--raw`, and may support
...
`meta.external.authorId`. DevFlow must not define `user`, `org`, or
...
mode marker to `meta.devflow.createMode = "run"` or an equivalent
...
```
<root>/                              # DEVFLOW_CHANNEL_ROOT ?? ~/.devflow/channels
...
|----------|-----------|---------|---------|
| `DEVFLOW_CHANNEL_ROOT` | optional | `~/.devflow/channels` | `channelRoot()` — override storage root |
| `DEVFLOW_CHANNEL_PROJECT` | optional | `projectKey(process.cwd())` | `currentProjectKey()` — lock current project bucket |
| `DEVFLOW_CHANNEL_AS` | optional | `"main"` | `spawn.ts` — default for `spawnedBy` on `spawned` event (lets workers spawning workers record correct lineage) |
| `DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT` | optional | `.devflow/config.yaml` then `5m` | worker OOM guard idle-cleanup TTL; duration string, `0` disables |
| `DEVFLOW_CHANNEL_MAX_LIVE_WORKERS` | optional | `.devflow/config.yaml` then `6` | worker OOM guard live-worker budget; non-negative integer, `0` disables |
| `DEVFLOW_HOOKS` | set to `"0"` by supervisor | n/a | supervised workers — disables devflow hooks inside the worker process (prevents recursive hook injection) |

**Env precedence**:
- `DEVFLOW_CHANNEL_PROJECT` set externally → that bucket (advanced)
- `DEVFLOW_CHANNEL_PROJECT` not set → derive from `process.cwd()`
- `selectExistingChannelProject(name)` may **mutate `process.env.DEVFLOW_CHANNEL_PROJECT`** when falling back to a unique cross-bucket match, so the rest of the CLI invocation lands on the same bucket
...
```bash
$ DEVFLOW_CHANNEL_ROOT=/tmp/test devflow channel run --provider codex --message "say hi in 3 words"
Hi, glad you're here.
$ echo $?
0
...
```bash
$ devflow channel run --agent check --message-file /tmp/cr-brief.md --timeout 15m
## Files Checked
...
Issues Found
- ...
...
```bash
$ PATH=/usr/bin devflow channel run --provider claude --message "hi" --timeout 30s
...
**Good**:
```bash
devflow channel create cr-feature --ephemeral
devflow channel spawn cr-feature --agent check
devflow channel spawn cr-feature --agent check --provider codex --as check-cx
devflow channel send cr-feature --as main --to check --text-file brief.md
devflow channel send cr-feature --as main --to check-cx --text-file brief.md
devflow channel wait cr-feature --as main --kind done --from check,check-cx --all --timeout 15m
...
```bash
devflow channel wait cr-feature --as main --kind done --from check,check-cx --all --timeout 30s
...
**Good** (channel created in devflow repo, accessed from /tmp via unique-match fallback):
```bash
$ cd /Users/me/work/devflow && devflow channel create unique-name
$ cd /tmp && devflow channel send unique-name --as main --text "hi"
...
```bash
$ cd /tmp && devflow channel send cr-r1 --as main --text "hi"
Error: Channel 'cr-r1' exists in multiple project buckets: -Users-me-work-devflow, -Users-me-work-app. Run from the owning project cwd or use --scope.
```
...
```bash
devflow channel create devflow-issue --scope global --type forum \
  --description "Local DevFlow feedback channel" \
  --context-file /Users/me/work/DevFlow/.devflow/spec/cli/backend/commands-channel.md
devflow channel post devflow-issue opened --scope global --as main \
  --thread forum-mode \
  --title "Forum mode" \
  --description "Track forum feedback." \
  --labels channel,ux
devflow channel post devflow-issue comment --scope global --as arch \
  --thread forum-mode \
  --text "Reviewed the functional shape."
devflow channel messages devflow-issue --scope global
# forum-mode [open] Forum mode labels=channel,ux
```

**Bad** (`send` is not a thread primitive):
```bash
devflow channel send devflow-issue --scope global --as main --thread forum-mode "hi"
...
| `DEVFLOW_CHANNEL_ROOT` override | integration | create a channel with env override; assert events land under that root, not `~/.devflow/channels` |
...
| Spawn-fail path (ENOENT) | e2e | `PATH=/no/claude devflow channel spawn ...` → events.jsonl has ONE error event, no spawned, no killed; supervisor exited; pid file removed |
...
| Frontmatter prototype pollution | security | a `.devflow/agents/<name>.md` fixture with `__proto__: ...` frontmatter → key dropped, no pollution observable |
...
```ts
selectExistingChannelProject(name);            // mutates DEVFLOW_CHANNEL_PROJECT env if needed
...
- **Event attribution + pass-through metadata** — keep `by` as a lightweight alias, add `origin: "cli"|"api"|"worker"` for the write entrypoint, and store business identity/context in `meta` without teaching DevFlow user/org semantics.
