...
|------|--------|----------|
| **Internal** | Existing feature, refactor, bug area | Search project code + `.devflow/spec/` |
...
Read `.devflow/spec/` to understand what guidelines exist:
...
Write one file per topic at `.devflow/tasks/{id}/context/{topic}.md`, using
...
```jsonl
{"path": ".devflow/spec/{pkg}/{layer}/index.md", "description": "Coding guidelines overview"}
{"path": ".devflow/spec/{pkg}/{layer}/error-handling.md", "description": "Error handling conventions"}
{"path": "src/services/auth.ts", "description": "Existing pattern to follow"}
{"path": ".devflow/tasks/{id}/prd.md", "description": "Requirements"}
{"path": ".devflow/tasks/{id}/context/new-sdk-usage.md", "description": "SDK API reference"}
```

### What goes into `check.jsonl`

```jsonl
{"path": ".devflow/spec/{pkg}/{layer}/quality-guidelines.md", "description": "Quality criteria"}
{"path": ".devflow/spec/guides/cross-layer-thinking-guide.md", "description": "Cross-layer check"}
```
...
- Read and reference `.devflow/spec/` files
- Write context files under `.devflow/tasks/{id}/context/`
- Populate JSONL files with discovered paths

### Forbidden (unless explicitly asked)
- Suggest improvements or criticize implementation
- Modify source code (only write to .devflow/ task directories)
...
### Specs Reviewed
- `.devflow/spec/{pkg}/{layer}/index.md` — {what it covers}
- `.devflow/spec/{pkg}/{layer}/specific-file.md` — {key points}
...
- `.devflow/tasks/{id}/context/topic-name.md` — {what it contains}
