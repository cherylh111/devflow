...
0.6.0 was the first GA where both `@enpd/devflow` (CLI) and `@enpd/devflow-core` (SDK) ship in lockstep. `bump-versions.js promote` rewrites both `package.json` files and the CLI's `dependencies["@enpd/devflow-core"]` from `workspace:*` to the exact version at release time. `release-preflight verify-packed-cli` exists specifically to catch a divergence here — always run it before `pnpm release:promote`.
...
1. `@enpd/devflow@beta` left in a bundled-skill markdown table after the lifecycle flip
...
- Treat the banner as "RC docs in general — check `devflow --version` for your install" (current pattern), or
