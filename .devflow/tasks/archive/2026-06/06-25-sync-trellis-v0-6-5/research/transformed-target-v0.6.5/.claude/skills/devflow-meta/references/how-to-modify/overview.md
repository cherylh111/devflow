...
Common DevFlow customization scenarios and what files need to be modified.
...
|------|-----------------|----------|
| [Add slash command](#add-slash-command) | commands/, devflow-local | All |
| [Add agent](#add-agent) | agents/, hook, jsonl, devflow-local | CC |
| [Modify hook](#modify-hook) | hooks/, settings.json, devflow-local | CC |
| [Add spec category](#add-spec-category) | spec/, jsonl, devflow-local | All |
| [Change verify commands](#change-verify-commands) | worktree.yaml | CC |
| [Add workflow phase](#add-workflow-phase) | task.json, dispatch, devflow-local | CC |
| [Add post_create step](#add-post_create-step) | worktree.yaml | CC |
| [Modify session start](#modify-session-start) | session-start.py, devflow-local | CC |
| [Add core script](#add-core-script) | scripts/, devflow-local | All |
...
**Scenario**: Add a new `/devflow:my-command` command.

**Files to modify**:

```
.claude/commands/devflow/my-command.md    # Create: Command prompt
.cursor/commands/my-command.md            # Create: Mirror for Cursor (optional)
.devflow-local/SKILL.md                   # Update: Document the change
```

**Steps**:
1. Create command file with YAML frontmatter
2. Mirror to Cursor if needed
3. Document in devflow-local
...
.devflow/tasks/{template}/my-agent.jsonl            # Create: Context template
.devflow-local/SKILL.md                             # Update: Document the change
```
...
```
.devflow/spec/mobile/index.md             # Create: Category index
.devflow/spec/mobile/*.md                 # Create: Spec files
.devflow/tasks/{template}/*.jsonl         # Update: Reference new specs
.devflow-local/SKILL.md                   # Update: Document the change
```
...
```
.devflow/worktree.yaml                    # Modify: verify section
```
...
```
.devflow/worktree.yaml                    # Modify: post_create section
```
...
```
.devflow/scripts/my-script.py             # Create: Script
.devflow/scripts/common/*.py              # Create/Modify: If shared utilities
.devflow-local/SKILL.md                   # Update: Document the change
```
...
```
.devflow/scripts/task.py                  # Modify: init-context logic
.devflow/tasks/{template}/*.jsonl         # Create: New JSONL templates
.devflow-local/SKILL.md                   # Update: Document the change
```
