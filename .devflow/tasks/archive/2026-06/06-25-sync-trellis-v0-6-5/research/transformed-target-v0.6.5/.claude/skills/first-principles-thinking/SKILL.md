...
When used within a DevFlow-managed project, the analysis artifacts integrate with the task system.

### File Placement

```
.devflow/tasks/{MM-DD-slug}/
├── task.json              # Existing
...
During `/devflow:brainstorm`, when the task is classified as "Complex":
...
```bash
python3 ./.devflow/scripts/task.py add-context "$TASK_DIR" implement "fp-analysis.md" "Ground truths and reasoning chain"
python3 ./.devflow/scripts/task.py add-context "$TASK_DIR" check "fp-analysis.md" "Verify implementation traces to ground truths"
```
