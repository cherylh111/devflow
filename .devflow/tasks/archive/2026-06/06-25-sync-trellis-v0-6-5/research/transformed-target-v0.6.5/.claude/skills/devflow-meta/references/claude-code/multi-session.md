...
│                                                                             │
│  /devflow:parallel → Configure task → start.py                             │
│                                           │                                 │
│                                           │ Creates worktree               │
│                                           │ Starts agent                   │
│                                           ▼                                 │
...
Location: `.devflow/worktree.yaml`
...
copy:
  - .devflow/.developer      # Developer identity
...
```bash
python3 .devflow/scripts/multi_agent/start.py <task-dir>
```
...
2. Create git worktree:
   ```bash
   git worktree add -b <branch> ../devflow-worktrees/<branch>
   ```
...
**Example**:
```bash
python3 .devflow/scripts/multi_agent/start.py .devflow/tasks/01-31-add-login-taosu
# Output: Started agent in ../devflow-worktrees/feature/add-login
```
...
```bash
# Overview
python3 .devflow/scripts/multi_agent/status.py

# Detailed view
python3 .devflow/scripts/multi_agent/status.py --detail <task-name>

# Watch mode
python3 .devflow/scripts/multi_agent/status.py --watch <task-name>

# View logs
python3 .devflow/scripts/multi_agent/status.py --log <task-name>

# Show registry
python3 .devflow/scripts/multi_agent/status.py --registry
```
...
Resume stopped sessions:
  cd ../devflow-worktrees/feature/fix-api && claude --resume <session-id>
```
...
```bash
python3 .devflow/scripts/multi_agent/create_pr.py [--dry-run]
```

**Actions**:
1. Stage changes: `git add -A`
2. Exclude: `git reset .devflow/workspace/`
...
```bash
# Specific worktree
python3 .devflow/scripts/multi_agent/cleanup.py <branch-name>

# All merged worktrees
python3 .devflow/scripts/multi_agent/cleanup.py --merged

# All worktrees (with confirmation)
python3 .devflow/scripts/multi_agent/cleanup.py --all
```

**Actions**:
1. Archive task to `.devflow/tasks/archive/YYYY-MM/`
...
```bash
python3 .devflow/scripts/multi_agent/plan.py \
  --name <task-slug> \
...
**Location**: `.devflow/workspace/<developer>/.agents/registry.json`
  ...
  "agents": [
    {
      ...
      "worktree_path": "/abs/path/to/devflow-worktrees/feature/add-login",
      "pid": 12345,
      "started_at": "2026-01-31T10:30:00",
      "task_dir": ".devflow/tasks/01-31-add-login-taosu"
    }
  ]
}
```
...
# Configure
python3 .devflow/scripts/task.py init-context <task-dir> fullstack
python3 .devflow/scripts/task.py set-branch <task-dir> feature/add-login
...
```bash
python3 .devflow/scripts/multi_agent/status.py --watch add-login
```
...
# Cleanup
python3 .devflow/scripts/multi_agent/cleanup.py feature/add-login
```
...
```bash
# Session 1
python3 .devflow/scripts/multi_agent/start.py .devflow/tasks/01-31-add-login

# Session 2 (immediately)
python3 .devflow/scripts/multi_agent/start.py .devflow/tasks/01-31-fix-api

# Session 3
python3 .devflow/scripts/multi_agent/start.py .devflow/tasks/01-31-update-docs
...
# Resume
cd ../devflow-worktrees/feature/task-name
...
**State** (`.devflow/.ralph-state.json`):
```json
{
  "task": ".devflow/tasks/01-31-add-login",
  "iteration": 2,
  "started_at": "2026-01-31T10:30:00"
}
```
...
4. Check `.devflow/.ralph-state.json`
...
# Manual edit
vim .devflow/workspace/<dev>/.agents/registry.json
```
