# DevFlow Local Skill Template

Copy this template to create a project-specific `devflow-local` skill.

---

## How to Use

1. Create directory: `mkdir -p .claude/skills/devflow-local`
2. Copy the template below to `.claude/skills/devflow-local/SKILL.md`
...
name: devflow-local
description: |
  Project-specific DevFlow customizations for [PROJECT_NAME].
  This skill documents all modifications made to the vanilla DevFlow system.
  Inherits from devflow-meta for base architecture documentation.
  Use this skill to understand what's been customized in this project's DevFlow setup.
---

# DevFlow Local - [PROJECT_NAME]

## Overview

This skill documents all customizations made to DevFlow in this project. For vanilla DevFlow documentation, see the `devflow-meta` skill.
...
#### /devflow:command-name [ALL]
- **File**: `.claude/commands/devflow/command-name.md`
...
#### Category Name
- **Path**: `.devflow/spec/category-name/`
- **Files**: [list of files]
...
#### spec-name.md
- **Path**: `.devflow/spec/category/spec-name.md`
- **Change**: [what was changed]
- **Date**: YYYY-MM-DD
- **Reason**: [why it was changed]
-->
...
Document any special steps needed when upgrading DevFlow.

<!-- Template:
### Upgrade to DevFlow X.Y.Z
...
```bash
#!/bin/bash
# create-devflow-local.sh

PROJECT_NAME="${1:-$(basename $(pwd))}"
SKILL_DIR=".claude/skills/devflow-local"
...
name: devflow-local
description: |
  Project-specific DevFlow customizations for PROJECT_NAME_PLACEHOLDER.
  This skill documents all modifications made to the vanilla DevFlow system.
  Inherits from devflow-meta for base architecture documentation.
---

# DevFlow Local - PROJECT_NAME_PLACEHOLDER

## Base Information

| Field | Value |
|-------|-------|
| DevFlow Version | $(cat package.json 2>/dev/null | grep version | head -1 | cut -d'"' -f4 || echo "unknown") |
