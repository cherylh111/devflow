...
Detailed guide on DevFlow feature availability across different AI coding platforms.

---

## Overview

DevFlow is designed primarily for **Claude Code** but provides partial support for **Cursor**. Future support for **OpenCode** is under consideration.
...
|---------|----------|-------------|
| Workspace system | `.devflow/workspace/` | Journals, session history |
| Task system | `.devflow/tasks/` | Task tracking, requirements |
| Spec system | `.devflow/spec/` | Coding guidelines |
...
| Developer identity | `.devflow/.developer` | Who is working |
| Current task | `.devflow/.runtime/sessions/` | Session-scoped active task state |
...
For teams using Cursor, here's how to get partial DevFlow benefits:
...
```
1. Session Start
   - Read .devflow/workflow.md
   - Read relevant specs from .devflow/spec/
...
| DevFlow Version | Claude Code | Cursor | OpenCode |
|-----------------|-------------|--------|----------|
...
├── YES → Full DevFlow support (Claude Code)
