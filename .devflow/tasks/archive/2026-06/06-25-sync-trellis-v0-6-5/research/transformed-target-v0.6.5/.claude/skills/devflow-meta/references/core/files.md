# DevFlow File Reference

Complete reference of all files in the `.devflow/` directory.
...
├── .version                # Installed DevFlow version
...
```json
{
  "current_task": ".devflow/tasks/01-31-add-login-taosu",
  "current_run": null,
  "platform": "claude",
...
**Purpose**: Legacy ignored pointer from older DevFlow versions.

**Active-task behavior**: Not read or written as a fallback. Current DevFlow
...
**Purpose**: Track template file versions for `devflow update`.

**Created by**: `devflow init` or `devflow update`

**Format**: JSON object mapping file paths to SHA-256 hashes.

```json
{
  ".devflow/workflow.md": "028891d1fe839a266...",
...
**Used by**:
- `devflow update` - Detect which files have been modified
...
**Purpose**: Track installed DevFlow CLI version.
...
**Used by**:
- `devflow update` - Determine if update is needed
...
**Location**: `.devflow/workspace/{developer}/.agents/`
...
These files are managed by `devflow update`:

| File | Purpose |
|------|---------|
| `.devflow/workflow.md` | Workflow documentation |
| `.devflow/worktree.yaml` | Multi-session config |
| `.devflow/.gitignore` | Git ignore rules |
...
.devflow/tasks/{task}/ → .devflow/tasks/archive/YYYY-MM/
