...
|--------|---------|-------|
| Workspace | Session tracking, journals | `.devflow/workspace/` |
| Tasks | Work item tracking | `.devflow/tasks/` |
| Specs | Coding guidelines | `.devflow/spec/` |
| Commands | Slash command prompts | `.claude/commands/` |
| Scripts | Automation utilities | `.devflow/scripts/` (core subset) |
...
1. Read `.devflow/workflow.md`
2. Read relevant specs from `.devflow/spec/`
3. Run `python3 .devflow/scripts/task.py current --source` for active work
...
| Document | Content |
|----------|---------|
| `files.md` | All files in `.devflow/` with purposes |
