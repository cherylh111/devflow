...
Platform-independent Python scripts for DevFlow automation.
...
**Creates:**
- `.devflow/.developer`
- `.devflow/workspace/<name>/`
- `.devflow/workspace/<name>/index.md`
- `.devflow/workspace/<name>/journal-1.md`
...
```bash
python3 .devflow/scripts/get_developer.py
# Output: taosu
```
...
```bash
python3 .devflow/scripts/add_session.py "Session summary"
```
...
Sets the active task in `.devflow/.runtime/sessions/<session-key>.json`.
Without a session identity or `DEVFLOW_CONTEXT_ID`, this command fails and
does not create `.devflow/.current-task`.
...
```bash
python3 .devflow/scripts/task.py set-branch <task-dir> <branch-name>
```
...
Moves task to `.devflow/tasks/archive/YYYY-MM/`.
...
from common.paths import (
    DEVFLOW_DIR,      # .devflow/
    WORKSPACE_DIR,    # .devflow/workspace/
    TASKS_DIR,        # .devflow/tasks/
    SPEC_DIR,         # .devflow/spec/
)
```
...
```bash
cd /path/to/project
python3 .devflow/scripts/init_developer.py john-doe
```
...
```bash
# Create task
python3 .devflow/scripts/task.py create "Add user login" --slug add-login

# Initialize context for fullstack work
python3 .devflow/scripts/task.py init-context \
  .devflow/tasks/01-31-add-login-john-doe fullstack

# Start task
python3 .devflow/scripts/task.py start \
  .devflow/tasks/01-31-add-login-john-doe
```

### Record Session

```bash
python3 .devflow/scripts/add_session.py "Implemented login form, pending API integration"
```

### Archive Completed Task

```bash
python3 .devflow/scripts/task.py archive \
  .devflow/tasks/01-31-add-login-john-doe
```
