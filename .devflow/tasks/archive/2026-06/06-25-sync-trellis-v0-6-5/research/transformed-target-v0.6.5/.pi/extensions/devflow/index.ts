...
// ── Constants ─────────────────────────────────────────────────────────
const DEVFLOW_AGENT_JSONL: Record<string, string> = {
  "devflow-implement": "implement.jsonl",
  implement: "implement.jsonl",
  "devflow-check": "check.jsonl",
  check: "check.jsonl",
};
...
interface ProgressDetails {
  kind: "devflow-subagent-progress";
  agent: string;
  mode: "single" | "parallel" | "chain";
  startedAt: number;
  updatedAt: number;
...
function cmdHasDevFlowCtx(cmd: string) {
  const t = cmd.trimStart();
  return (
    /^export\s+DEVFLOW_CONTEXT_ID=/.test(t) ||
    /^DEVFLOW_CONTEXT_ID=/.test(t) ||
    /^env\s+.*DEVFLOW_CONTEXT_ID=/.test(t)
  );
}
...
function resolvePiCli(): { command: string; args: string[] } {
  const envCli = str(process.env.DEVFLOW_PI_CLI_JS);
  if (envCli) {
    const p = resolve(envCli);
    if (!exists(p)) throw new Error(`DEVFLOW_PI_CLI_JS missing: ${p}`);
...
function findRoot(start: string): string {
  let c = resolve(start);
  while (true) {
    if (existsSync(join(c, ".devflow")) || existsSync(join(c, ".pi"))) return c;
    const p = dirname(c);
...
function contextKey(input?: unknown, ctx?: PiExtensionContext): string | null {
  const ov = str(process.env.DEVFLOW_CONTEXT_ID);
...
function readTaskDir(root: string, key: string | null): string | null {
  ...
  try {
    const ctx = JSON.parse(
      readText(join(root, ".devflow", ".runtime", "sessions", `${key}.json`)),
    ) as JsonObject;
    let ref = str(ctx.current_task);
    if (!ref) return null;
    ref = ref;
    ref = ref.replace(/\\/g, "/").replace(/^\.\//, "");
    if (ref.startsWith("tasks/")) ref = `.devflow/${ref}`;
    return ref.startsWith(".devflow/")
      ? join(root, ref)
      : isAbsolute(ref)
        ? ref
        : join(root, ".devflow", "tasks", ref);
  } catch {
    return null;
  }
}
...
function adoptKey(root: string, key: string): string {
  ...
  try {
    const dir = join(root, ".devflow", ".runtime", "sessions");
...
function workflowBreadcrumb(root: string, key: string | null): string {
  const wf = readText(join(root, ".devflow", "workflow.md"));
  if (!wf) return "";
...
function sessionOverview(root: string, key: string | null): string {
  const script = join(root, ".devflow", "scripts", "get_context.py");
  if (!exists(script)) return "";
  try {
    const py = process.platform === "win32" ? "python" : "python3";
    const result = spawnSync(py, [script], {
      cwd: root,
      env: key ? { ...process.env, DEVFLOW_CONTEXT_ID: key } : process.env,
      encoding: "utf-8",
...
function normalizeAgent(agent: string | undefined): string {
  const name = agent ?? "devflow-implement";
  return name.startsWith("devflow-") ? name : `devflow-${name}`;
}

function isDevFlowAgent(root: string, agent: string): boolean {
...
function runPi(
  ...
  return new Promise((resolve) => {
    ...
    const childEnv = {
      ...process.env,
      DEVFLOW_SUBAGENT_CHILD: "1",
      ...(key ? { DEVFLOW_CONTEXT_ID: key } : {}),
    };
...
// ── Extension ──────────────────────────────────────────────────────────
export default function devflowExtension(pi: {
  ...
  if (process.env.DEVFLOW_SUBAGENT_CHILD === "1") return;
  ...
  // Tool registration
  pi.registerTool?.({
    name: "devflow_subagent",
    label: "DevFlow Subagent",
    description: "Run a DevFlow project sub-agent with active task context.",
    promptSnippet:
      'Sub-agent dispatch protocol (DevFlow): your dispatch prompt MUST start with one line "Active task: <task path from `task.py current`>" before any other instructions.',
    promptGuidelines: [
    ...
    parameters: {
      ...
      properties: {
        agent: {
          type: "string",
          description:
            "Agent name, such as devflow-implement or devflow-check.",
        },
        prompt: {
          type: "string",
    ...
    execute: async (
      ...
      activeSubagentToolCallId = id;
      const agentName = normalizeAgent(input.agent);
      if (!isDevFlowAgent(root, agentName)) {
        return {
          content: [
            {
              type: "text",
              text:
                "`devflow_subagent` is only for DevFlow workflow agents with a definition file in .pi/agents/.\n\n" +
                ...
                `- Create .pi/agents/${agentName}.md for your custom DevFlow agent\n` +
                "- Install a community subagent package: pi install -l npm:@tintinweb/pi-subagents",
            },
          ],
          details: { agent: agentName, error: "not a devflow workflow agent" },
        };
      }
      const mode = input.mode ?? "single";
    ...
    renderResult: (
      ...
      const isProgress =
        ...
        result.details.kind === "devflow-subagent-progress";
  ...
  pi.on?.("session_start", (event, ctx) => {
    getKey(event, ctx);
    ctx?.ui?.notify?.(
      "DevFlow project context is available. Use /devflow-continue to resume the current task.",
      "info",
    );
  });
  ...
  pi.on?.("tool_call", (event, ctx) => {
    ...
    if (
      ...
      !cmdHasDevFlowCtx(ev.input.command)
    )
      ev.input.command = `export DEVFLOW_CONTEXT_ID=${shellQuote(k)}; ${ev.input.command}`;
  });
  ...
  pi.on?.("tool_result", (event) => {
    ...
    if (
      ev.toolName === "devflow_subagent" &&
      isObj(ev.details) &&
      ev.details.kind === "devflow-subagent-progress" &&
  ...
  pi.on?.("before_agent_start", (event, ctx) => {
    ...
    const ctxText = buildContext(root, "devflow-implement", k);
