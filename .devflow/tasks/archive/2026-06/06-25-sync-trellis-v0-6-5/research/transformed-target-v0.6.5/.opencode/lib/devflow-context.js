...
// Debug logging
const DEBUG_LOG = "/tmp/devflow-plugin-debug.log"
...
// Matches `devflow-implement`, `devflow-check`, `devflow-research` exactly.
// Used by chat.message plugins to skip injection inside DevFlow sub-agent turns.
const DEVFLOW_SUBAGENT_RE = /^devflow-(implement|check|research)$/

/**
 * Return true when the OpenCode `chat.message` input represents a DevFlow
 ...
 */
export function isDevFlowSubagent(input) {
  if (!input || typeof input !== "object") return false
  const agent = typeof input.agent === "string" ? input.agent.trim() : ""
  return DEVFLOW_SUBAGENT_RE.test(agent)
}

/**
 * DevFlow Context Manager
 */
export class DevFlowContext {
  constructor(directory) {
    this.directory = directory
    debugLog("context", "DevFlowContext initialized", { directory })
  }
  ...
  isDevFlowProject() {
    return existsSync(join(this.directory, ".devflow"))
  }

  getContextKey(platformInput = null) {
    const override = stringValue(process.env.DEVFLOW_CONTEXT_ID)
    if (override) {
  ...
  readContext(contextKey) {
    try {
      const contextPath = join(this.directory, ".devflow", ".runtime", "sessions", `${contextKey}.json`)
  ...
  _resolveSingleSessionFallback() {
    const sessionsDir = join(this.directory, ".devflow", ".runtime", "sessions")
  ...
  resolveTaskDir(taskRef) {
    ...
    return join(this.directory, ".devflow", "tasks", normalized)
  }
