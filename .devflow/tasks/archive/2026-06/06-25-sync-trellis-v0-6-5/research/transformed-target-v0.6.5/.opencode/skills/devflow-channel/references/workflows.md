...
```bash
devflow channel create brainstorm-storage-layer --by main \
  --task .devflow/tasks/05-XX-storage-adapter

devflow channel spawn brainstorm-storage-layer \
  --agent architect --provider codex \
  --file .devflow/tasks/05-XX-storage-adapter/prd.md \
  --file .devflow/tasks/05-XX-storage-adapter/design.md \
  --as cx-arch --timeout 30m

devflow channel send brainstorm-storage-layer \
  --as main --to cx-arch --text-file /tmp/brainstorm-r1.md

devflow channel wait brainstorm-storage-layer \
...
```bash
TASK=.devflow/tasks/05-12-foo
devflow channel create cr-foo --task "$TASK" --by main

devflow channel spawn cr-foo \
  --agent check \
...
devflow channel send cr-foo --as main --to check --text-file /tmp/cr-brief.md
devflow channel wait cr-foo --as main --kind done --from check --timeout 15m
devflow channel messages cr-foo --kind message --from check --tag final_answer
```
...
```bash
devflow channel create cr-feature --by main --ephemeral
...
devflow channel send cr-feature --as main --to check --text-file /tmp/cr-brief.md
devflow channel send cr-feature --as main --to check-cx --text-file /tmp/cr-brief.md
...
```bash
devflow channel run --provider codex --message "say hi in 3 words" --timeout 1m
devflow channel run --agent plan --message-file /tmp/plan-question.md --timeout 10m
```
...
devflow channel thread <board> <thread> --scope global --raw
devflow channel context list <board> --scope global --thread <thread>
devflow channel messages <board> --scope global --raw --thread <thread>
```
