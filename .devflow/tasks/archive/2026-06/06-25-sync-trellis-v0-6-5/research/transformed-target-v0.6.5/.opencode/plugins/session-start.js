...
import { DevFlowContext, contextCollector, debugLog, isDevFlowSubagent } from "../lib/devflow-context.js"
import {
  buildSessionContext,
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      try {
        ...
        // Skip DevFlow sub-agent turns — sub-agent context is injected by
        ...
        if (isDevFlowSubagent(input)) {
          debugLog("session", "Skipping devflow subagent turn:", agent)
          return
        }

        if (process.env.DEVFLOW_HOOKS === "0" || process.env.DEVFLOW_DISABLE_HOOKS === "1") {
          debugLog("session", "Skipping - DEVFLOW_HOOKS disabled")
          return
        }
        ...
        if (await hasPersistedInjectedContext(client, ctx.directory, sessionID)) {
          ...
          debugLog("session", "Skipping - session already contains persisted DevFlow context")
          return
        }
