...
`devflow channel messages <channel>` renders a compact, human-readable view:
...
devflow channel messages <channel> --kind done --last 10
devflow channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
devflow channel messages <channel> --raw --kind progress --last 20
devflow channel messages <channel> --raw --last 50
```
...
```bash
devflow channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `devflow channel list` shows the worker as running, but no new
...
1. **Locate the channel files.** Use `list --all --all-projects` if you are
   ...
   ```bash
   devflow channel list --all --all-projects
   CHAN=~/.devflow/channels/<bucket>/<channel>
   ```
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `devflow channel kill <name> --as <worker> --force`.
...
```bash
devflow channel wait T --as main --from check --kind done --timeout 15m
devflow channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
devflow channel wait T --as worker --tag interrupt --timeout 1h
devflow channel wait T --as main --thread release-note --action status --timeout 10m
```
...
devflow channel forum list <channel>

# Inspect one thread end-to-end
devflow channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
devflow channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
devflow channel context <channel> --as <worker>
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `devflow: command not found` | CLI not installed globally | `npm install -g @enpd/devflow` |
...
| ghost worker in list | supervisor died without cleanup | `devflow channel kill <name> --as <worker> --force` |
