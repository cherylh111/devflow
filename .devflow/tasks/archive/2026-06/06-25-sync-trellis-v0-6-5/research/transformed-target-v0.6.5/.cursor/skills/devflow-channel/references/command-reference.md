...
Authoritative current command reference for `devflow channel` subcommands,
...
```bash
devflow channel create <name>
  [--scope project|global]                # default: project
  [--type chat|forum]                     # default: chat
  [--task <path>]                         # associated DevFlow task dir
  [--project <slug>]
  [--labels a,b,c]
...
```bash
devflow channel send <name> [text]
...
- `--kind` is the only event-type filter, and it is constrained to the
  devflow-emitted whitelist (`CHANNEL_EVENT_KINDS` in
...
- Use `devflow channel interrupt` (the command) only when you actually want
...
```bash
devflow channel send T --as A --stdin < /tmp/message.md
devflow channel send T --as A --text-file /tmp/message.md
```
...
```bash
devflow channel interrupt <name> [text]
...
devflow channel spawn <name>
  [--scope project|global]
  [--agent <agent-name>]                  # loads .devflow/agents/<name>.md
  ...
  [--jsonl <path>] ...                    # DevFlow manifest, repeatable
  [--by <agent>]                          # spawn-event author
                                          # default: DEVFLOW_CHANNEL_AS env or 'main'
...
- OOM-guard precedence: CLI flag → env var
  (`DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT`,
  `DEVFLOW_CHANNEL_MAX_LIVE_WORKERS`) →
  `.devflow/config.yaml#channel.worker_guard` → built-in defaults.

### `run [name]`

```bash
devflow channel run [name?]
  [--agent <name>]
...
```bash
devflow channel post <name> <action>
  --as <agent>                            # REQUIRED
  [--scope project|global]
...
```bash
devflow channel thread <name> <thread-key>
  [--scope project|global]
  [--raw]

devflow channel thread rename <name> <old-thread> <new-thread>
  --as <agent>                            # REQUIRED
  [--scope project|global]
```
...
```bash
devflow channel context add <name>
...
devflow channel context delete <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
  [--thread <key>]
  [--file <abs-path>] ...
...
devflow channel context list <name>
  [--scope project|global]
  [--thread <key>]
...
```bash
devflow channel title set <name>
  --title <text>                          # REQUIRED
  [--as <agent>]                          # default: main
  [--scope project|global]

devflow channel title clear <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
```
