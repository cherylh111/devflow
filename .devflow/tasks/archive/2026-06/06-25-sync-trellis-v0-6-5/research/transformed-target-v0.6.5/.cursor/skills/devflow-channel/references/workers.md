...
```bash
devflow channel create impl-task --by dispatcher --cwd /path/to/repo
devflow channel spawn impl-task --provider codex --as codex-impl --timeout 30m

echo "Implement the schema for table X per .devflow/.../prd.md" \
  | devflow channel send impl-task --as dispatcher --to codex-impl --stdin

devflow channel wait impl-task --as dispatcher --from codex-impl --kind done --timeout 30m
```
...
- `--agent <name>` — load `.devflow/agents/<name>.md` (provider/model/as/system prompt defaults).
...
- `--jsonl <path>` (repeatable) — DevFlow jsonl manifest (`{file, reason}` per line).
- `--by <agent>` — author of the `spawned` event (defaults to `$DEVFLOW_CHANNEL_AS` or `main`).
...
`--agent <name>` resolves to `.devflow/agents/<name>.md`. The card name must
match `[A-Za-z0-9._-]+`. The default DevFlow install ships two cards:

- `.devflow/agents/check.md` — code-quality reviewer.
- `.devflow/agents/implement.md` — coding worker for implementation runs.
...
```bash
ls .devflow/agents
sed -n '1,100p' .devflow/agents/check.md
```
...
- `--jsonl <path>` — repeatable DevFlow manifest where every line is
...
```bash
TASK=.devflow/tasks/05-13-example
devflow channel spawn cr-example --agent check --provider codex --as check-cx \
  --file "$TASK/prd.md" \
  --file "$TASK/design.md" \
...
```bash
devflow channel spawn cr-feature --agent check --as check-claude
devflow channel spawn cr-feature --agent check --provider codex --as check-cx

devflow channel wait cr-feature --as main \
...
echo "Stop refactoring the parser — switch to fixing the failing test in src/foo.ts" \
  | devflow channel interrupt impl-task --as dispatcher --to codex-impl --stdin
```
...
echo "Check this when you reach the next turn." \
  | devflow channel send impl-task --as dispatcher --to codex-impl \
      --stdin --tag question
```
...
```bash
devflow channel kill impl-task --as codex-impl
devflow channel spawn impl-task --as codex-impl --provider codex \
  --resume "$(cat ~/.devflow/channels/<bucket>/impl-task/worker.session-id)"
...
2. Environment variables: `DEVFLOW_CHANNEL_WORKER_IDLE_TIMEOUT`,
   `DEVFLOW_CHANNEL_MAX_LIVE_WORKERS`.
3. `.devflow/config.yaml` under `channel.worker_guard`.
...
`~/.devflow/channels/<bucket>/<channel>/`.
...
devflow channel wait impl-task --as dispatcher \
  --from codex-impl --kind done,error --timeout 30m

# 3. Read the final answer.
devflow channel messages impl-task --from codex-impl --last 1 --raw
```
