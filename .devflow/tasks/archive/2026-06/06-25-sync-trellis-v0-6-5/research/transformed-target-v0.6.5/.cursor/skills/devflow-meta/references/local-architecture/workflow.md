...
`.devflow/workflow.md` is the DevFlow workflow source of truth inside the user project. An AI does not need DevFlow source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.devflow/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch `devflow-implement` by default for implementation and `devflow-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `devflow-before-dev`, then executes directly.
...
After editing, make the AI reread `.devflow/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `devflow-brainstorm` and `devflow-check`.
