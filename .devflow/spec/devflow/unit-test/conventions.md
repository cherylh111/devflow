...
Several DevFlow modules (e.g. `OpenCodeContext.getContextKey`, `DevFlowContext.getActiveTask`) consult `process.env.DEVFLOW_CONTEXT_ID` and `process.env.OPENCODE_RUN_ID` as **highest-priority overrides** — production behavior, by design.
...
```ts
// test/setup.ts
delete process.env.DEVFLOW_CONTEXT_ID;
...
it("safe-file-delete respects update.skip", () => {
  ...
  config.update.skip = [".claude/commands/devflow/"];
...
**Related**: `.devflow/spec/devflow/backend/quality-guidelines.md` → "Routing Fixes: Audit ALL Entry Paths" — the structural side of the same lesson. Every convenience flag in tests typically corresponds to an unfixed entry path in production code.
...
```typescript
/**
 * Helper: simulate a fresh clone of an existing DevFlow project...
 *
