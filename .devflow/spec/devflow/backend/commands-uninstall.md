# `devflow uninstall` Command

Source: `packages/cli/src/commands/uninstall.ts`

How the uninstall command removes every DevFlow-written file from a project, scrubs structured config files in place, and prunes empty managed directories — without ever touching user-authored neighbors.

---

## Overview

`devflow uninstall` is the inverse of `devflow init` / `devflow update`: it removes everything DevFlow wrote and leaves everything DevFlow did not.

- **Manifest is authoritative.** The single source of truth for "what devflow wrote" is `.devflow/.template-hashes.json`. Files outside that manifest are never touched, regardless of where they live (e.g. user-added scripts under `.claude/hooks/`, custom commands under `.cursor/commands/`).
- **No user-modification gate.** Whether the user has edited a manifest-listed file or not, it is removed. `update` semantics (warn / preserve modified files) do not apply here — the user's intent is to remove DevFlow entirely.
- **Two file classes.** Manifest entries fall into:
  1. *Opaque content files* (`.py`, `.md`, `.toml`, `.json` agents, etc.) — unlinked outright.
  2. *Structured config files* (`settings.json`, `hooks.json`, `package.json`, `config.toml`) — passed through a scrubber that removes only the devflow-owned fields and writes the trimmed result back. If nothing meaningful remains, the scrubber returns `fullyEmpty: true` and the file is deleted instead of rewritten.
- **`.devflow/` is removed unconditionally.** Tasks, runtime state, workspace journal, config — all of it. Users who want to keep historical task records must back up `.devflow/tasks/` themselves before running `uninstall`.
- **Idempotent.** Re-running on a project that has no `.devflow/` is a friendly no-op. Re-running after a partial failure picks up whatever is still on disk and converges.
...
For the *content* of each scrubber (which fields are stripped from `.claude/settings.json`, what counts as a devflow comment in `.codex/config.toml`, etc.), see `uninstall-scrubbers.md` for per-file scrubbing rules.
...
```
devflow uninstall [-y|--yes] [--dry-run]
```
...
There are no `--platform <name>` or `--keep-config` flags. The design is intentionally all-or-nothing: partial uninstall (e.g. "remove DevFlow from Cursor only, leave Claude Code") is **out of scope** because the manifest does not partition by platform — see *Common Pitfalls* below.
...
1. **`.devflow/` must exist.** If missing, print a gray "not installed" message and return cleanly (exit 0). This is the idempotent re-run path.
2. **Manifest must exist and be non-empty.** `loadHashes(cwd)` returns `{}` when `.devflow/.template-hashes.json` is missing or unreadable. Without the manifest there is no way to distinguish devflow-owned platform files from user-owned ones, so the command refuses to proceed and exits with a red error message + `process.exit(1)`. Users in this state are told they may delete `.devflow/` manually.
...
`removeDevFlowDir` is set to `true` unconditionally — by the time `buildPlan` runs, we have already verified `.devflow/` exists.
...
The `StructuredFileSpec.scrub` callback receives `(content, deletedPaths)`. `deletedPaths` is the full set of manifest-listed POSIX paths for *this uninstall*, used by hooks-JSON scrubbers to identify devflow-managed `command` strings without false-matching on user-added hooks that merely mention the path in an `echo` or comment.

### Plan rendering — `commands/uninstall.ts:renderPlan`

Two-column output:

- **Will be deleted (N entries)** — the un-missing deletions plus a synthetic `WORKFLOW/` line representing the `.devflow/` directory itself (only printed if the directory still exists, which it always should after the pre-check).
...
If `process.stdin.isTTY` is false **and** neither `--yes` nor `--dry-run` is set, the command refuses to prompt and exits non-zero with a red message instructing the user to pass `--yes` or `--dry-run`. This is a deliberate fail-closed UX choice that mirrors `devflow update` in scripted environments. The brief `readline.createInterface(...).close()` call before exit is a defensive ref-release in case anything else opened stdin (mostly defensive — the process is about to exit anyway).
...
Write each `PlannedModification.result.content` to its `absPath` via `fs.writeFileSync`. Doing this **before** deletions means that if a later step crashes, structured config files have at least had their devflow fragments stripped. User data inside those files (other deps in `package.json`, other hooks in `settings.json`, custom keys) is preserved.
...
### Phase 3 — Drop `.devflow/` recursively

`fs.rmSync(devflowDir, { recursive: true, force: true })`. Whole directory tree gone in one call. This is unconditional within `executePlan`; the only gate is the pre-check at the top of `uninstall()` which establishes that the directory exists and a manifest is present.
...
`.devflow/` is removed in its entirety — there is no `--keep-config` or `--keep-tasks` flag. This includes:

| Subdirectory | Status |
|---|---|
| `.devflow/scripts/` | Removed (template-managed). |
| `.devflow/spec/` | Removed (managed via `update.skip` semantics during `update`, but uninstall removes everything). |
| `.devflow/tasks/` | Removed (user data). |
| `.devflow/workspace/` | Removed (user journal). |
| `.devflow/runtime/` | Removed (session state). |
| `.devflow/config.yaml` | Removed (user config). |
| `.devflow/.developer` | Removed. |
| `.devflow/.current-task` | Removed. |
| `.devflow/.template-hashes.json` | Removed. |

This is **deliberately destructive** for user data inside `.devflow/`. Users are responsible for backing up `tasks/` or `workspace/` before running `uninstall` if they want history preserved. The plan output prints `WORKFLOW/  (entire directory, including tasks/runtime/config)` so this is visible before the confirmation prompt.

> Rationale: a "soft uninstall" that leaves orphan `.devflow/` content behind is a worse state than either fully-installed or fully-uninstalled — the leftover files reference removed scripts (`.devflow/scripts/`) and broken sub-agent configs (`.devflow/tasks/<id>/implement.jsonl` pointing at deleted spec files). Either keep DevFlow or remove it cleanly. There is no half-DevFlow mode.
...
- **Mutate user-authored sections of structured config.** Scrubbers strip *only* devflow-emitted entries. Other deps in `package.json`, other event hooks in `settings.json`, custom `[features]` table entries in `config.toml` — all preserved. Test `#8` covers this for `.claude/settings.json`.
...
1. Did the file exist in `.devflow/.template-hashes.json` before the uninstall? (If not, it was never devflow-owned.)
...
If a user wants to remove just one platform's files, the path is `devflow update` after editing `config.yaml`'s platform list — that flow knows how to deconfigure platforms cleanly. `uninstall` is a single-shot full removal.
...
**Symptom**: User manually deleted some DevFlow files, then runs `uninstall`. Plan shows "skipped N entries" for those files (they were missing on disk), but the unrelated structured-config phase still works correctly.
...
**Symptom**: User uninstalls DevFlow but `~/.codex/config.toml` still has `[features]\nhooks = true`.

**Cause**: That flag is in the **user-level** Codex config, not project-local. DevFlow never wrote to it (the README+the project `.codex/config.toml` comment block instruct the user to add it manually). `uninstall` therefore does not remove it.
...
Tests live in `packages/cli/test/commands/uninstall.integration.test.ts`. The file pattern: each test runs `init({ ..., force: true })` in a fresh tmpdir to set up a real DevFlow install, then exercises one path through `uninstall()`.

Reference cases (number = test ID in the file):

| # | Scenario | What it pins down |
|---|---|---|
| 1 | `.devflow/` missing | Friendly no-op exit, no error. |
| 2 | `.devflow/` present, manifest missing | Error exit (manual cleanup hint). |
...
| 8 | `.claude/settings.json` with extra user fields | Scrubber preserves user fields, strips devflow hooks. |
...
3. Add an integration test in this file mirroring `#8` — init that platform, write some user-owned fields into the structured config, uninstall, assert the user fields survive and the devflow fields are gone.
