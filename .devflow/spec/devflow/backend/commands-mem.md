# `devflow mem` — Cross-Platform AI Session Memory

How DevFlow indexes, searches, and extracts dialogue from on-disk session files
written by Claude Code, Codex, and OpenCode.

The retrieval engine lives in `@enpd/devflow-core/mem` (`packages/core/src/mem/`);
...
`devflow mem` is an offline reader over **local AI session stores**. It does not
...
strips DevFlow / platform injection tags so search hits aren't dominated by
session-start preamble.

The retrieval domain does **not** depend on the rest of the DevFlow runtime (no
...
invoked from the `devflow` Commander wire.
...
`mem` is split between `@enpd/devflow-core` and the CLI. See
`devflow-core-sdk.md` for the general rule; the `mem`-specific split:

**Core owns** (`packages/core/src/mem/`, public surface at the
`@enpd/devflow-core/mem` subpath — **not** the root barrel):
...
`commands/mem.ts:parseArgv`, then calls the matching core `@enpd/devflow-core/mem`
...
`node-gyp` rebuild required VS2017+ build tools, and `devflow` failed to
...
DevFlow / platform injection tags would otherwise dominate every match.

If you need to add a new injection tag (e.g. a new DevFlow hook adds
...
- **No transitive dependency on DevFlow runtime**: `core/mem/` does not import
  from `configurators/`, `migrations/`, `templates/`, or `.devflow/scripts`,
...
tool heavily. For example, `devflow mem search "Skill"` against a session that
...
grep -oE '"name":"Skill","input":\{[^}]+\}' \
  ~/.claude/projects/-Users-…-DevFlow/<session-id>.jsonl

# Cross-session skill usage in a project
grep -hoE '"skill":"[a-z0-9-]+"' \
  ~/.claude/projects/-Users-…-DevFlow/*.jsonl | sort | uniq -c
```

**Decision rule** for choosing between `devflow mem` and raw `grep`:

| Searching for | Tool |
|---|---|
| User/assistant said something / discussed a topic / made a decision | `devflow mem search` |
| What tool / skill / agent / sub-agent was used | `grep` over JSONL |
| Tool call frequency / parameters | `grep` + `jq` over JSONL |
| Cross-session topic recall (concepts in dialogue) | `devflow mem search` |
...
`devflow mem extract <id> --phase <brainstorm|implement|all>` slices the cleaned
dialogue by DevFlow brainstorm windows, allowing the high-density discussion
...
cover every shape DevFlow users actually write:
...
- `python ./.devflow/scripts/task.py create "title"`
- `python3 ./.devflow/scripts/task.py create my-task`
- `py -3 .devflow/scripts/task.py create ...` (Windows launcher)
- `python3 .devflow\\scripts\\task.py start ...` (JSONL-double-escaped backslash)
- `python3 .devflow\scripts\task.py start ...` (single backslash)
...
| `python3 .devflow/scripts/task.py start .devflow/tasks/05-08-foo` | task-dir has `MM-DD-` prefix from `task.py create` | `slugFromTaskDir` strips a leading `MM-DD-` so a `create --slug foo` pairs with this `start` via slug match |
...
- New surface forms (e.g., `devflow task create` if DevFlow ever ships a wrapper)
...
inflates hit counts on every DevFlow-using session. Always run text through
...
Measured impact on a 36 MB Claude session (DevFlow dogfood):

| Operation | Before (full read + split) | After (chunked + 0x7b skip) |
|---|---|---|
| `devflow mem list` | ~3.5s | ~0.67s |
| `devflow mem extract --phase brainstorm` | ~5.8s | ~0.73s |
...
`core/mem/` does **not** use `zod` — `@enpd/devflow-core` keeps a
zero-dependency surface (see `devflow-core-sdk.md`). External platform shapes
...
- A new shell-arg / Codex-argument form picked up by the phase parsers →
  ...
  studies live under `.devflow/tasks/05-08-mem-phase-slice/` and
  `.devflow/tasks/05-09-mem-phase-multi/`.
...
| Export | Use |
|--------|-----|
| `runMem(args)` | Entry point — `devflow mem ...` calls into this |
...
- `packages/core/src/mem/index.ts` — `@enpd/devflow-core/mem` public surface
...
- `.devflow/tasks/05-14-mem-core-channel-reuse/` — the mem-core extraction task
- `.devflow/tasks/05-08-mem-since-cross-day-filter/` — historical context for
  the `inRangeOverlap` switch
- `.devflow/tasks/05-08-mem-phase-slice/` — historical context for the
