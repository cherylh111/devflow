...
```json
{
  ...
  "migrations": [
    ...
    {
      "type": "delete",
      "from": ".devflow/scripts/deprecated.py",
      "description": "删除原因"
    }
  ]
}
...
**字段语义**：
- `migrationGuide` —— 面向最终用户的 narrative，被模板化塞进 `.devflow/tasks/MM-DD-migrate-to-<version>/prd.md` 的 PRD section
...
```json
{
  ...
  "from": ".claude/commands/devflow/before-backend-dev.md",
...
**工作机制**：
- **无需 `--migrate`** — 在每次 `devflow update` 时自动执行
...
可选 manifest 字段，声明本版本在 `.devflow/config.yaml` 引入的新顶层 section。`devflow update` 走完文件写入循环后会再扫一遍 `getConfigSectionsAddedBetween(fromVersion, toVersion)`，每条 entry 命中"目标文件存在 + sentinel 不在文件里"才会从打包模板里抽对应 section 追加到末尾。仅追加，幂等（重跑时 sentinel 已在）。
...
- `file` —— 目标文件相对仓库根的路径（当前只用过 `.devflow/config.yaml`，机制本身不限定）
...
**未来加新 section**：在对应版本 manifest 加一条 entry 即可，`update.ts` 不需要改。`packages/cli/src/templates/devflow/config.yaml` 模板里加对应 `#---` 分隔块和 section 内容，`devflow init` 走默认写入路径自然会拿到新 section。

## 受保护路径

以下路径不会被任何迁移操作修改或删除（用户数据）：

- `.devflow/workspace` — 开发者工作记录
- `.devflow/tasks` — 任务追踪
- `.devflow/spec` — 开发指南（用户自定义）
- `.devflow/.developer` — 开发者身份
- `.devflow/.current-task` — 当前任务指针
...
- 存储位置：`.devflow/.template-hashes.json`
- 用途：检测用户是否修改过模板文件
- 原理：比较当前文件 SHA256 与存储的哈希值
- 初始化：`devflow init` 时自动创建
- 更新：`devflow update` 后自动更新被覆盖文件的哈希

### Manifest ownership contract (CRITICAL — data loss prevention)

`.template-hashes.json` is the **single source of truth** for `devflow uninstall`. Every key listed there gets `fs.unlinkSync`'d at uninstall time. Therefore the manifest must contain **only files devflow actually wrote during init / update — never files that merely happen to exist under a managed directory**.
...
**`initializeHashes(cwd, opts)`** must derive the manifest set from `opts.trackedPaths` (a `Set<string>` of paths devflow **actually wrote this run**), NOT from `fs.readdirSync` walks of platform dirs.
...
#### `.devflow/` walk exemption

`.devflow/` files are still hashed via recursive walk (existing `collectFiles` behavior + `EXCLUDE_FROM_HASH` filters). Rationale: `devflow uninstall` step 3 does `fs.rmSync('.devflow/', { recursive: true, force: true })` regardless of manifest content, so the walk's blast radius is contained. Over-hashing inside `.devflow/` only affects `devflow update` 3-way-merge accuracy, not uninstall safety.

#### Self-heal contract: `pruneOrphanManifestKeys`

Existing users may have poisoned manifests from older devflow versions. `pruneOrphanManifestKeys(cwd, configuredPlatforms, hashes, opts)` runs at the top of both `devflow update` AND `devflow uninstall` (before plan classification) and prunes orphan keys.

**Preserve set** (a key is kept iff one of):

1. Starts with `.devflow/` (walk-managed)
...
4. Path is `AGENTS.md` AND the file on disk contains `DEVFLOW_BLOCK_START` + `DEVFLOW_BLOCK_END` markers, OR the file is missing. Files lacking markers are treated as user-owned and pruned.
...
// Correct — manifest reflects exactly what devflow wrote this run
...
Even with the manifest contract above, `devflow init` and `devflow uninstall` refuse to run when `process.cwd()` is exactly the user's home directory. Use `isCwdHomedir()` from `utils/cwd-guard.ts` — it compares via `fs.realpathSync.native()` on both sides, Windows-lowercases for case-insensitivity, try/catch defaults permissive on lookup failure. Override via `DEVFLOW_ALLOW_HOMEDIR=1` only; `--force` does NOT bypass.
...
`.devflow/workflow.md` is not only documentation. It is runtime input for
`get_context.py`, `workflow_phase.py`, SessionStart strippers, and per-turn
workflow-state hooks.

`devflow update` must therefore keep `workflow.md` on the normal whole-file
template path:
...
not only fresh-init template tests: write the older `.devflow/.version`, stage
older hash-tracked template files, run `devflow update`, then assert the
...
```bash
devflow update              # 显示迁移提示
devflow update --migrate    # 执行迁移（修改过的文件会提示确认）
devflow update --migrate -f # 强制迁移（备份后执行）
devflow update --migrate -s # 跳过修改过的文件
```
