...
| `--create-new` | Write `.devflow/workflow.md.new`; do not change active workflow or hash file |
...
- Explicit `--template` with modified workflow fails even when `stdin.isTTY` is
  true.
- `--create-new` writes `.devflow/workflow.md.new` and does not touch the active
  workflow or hash.
