...
The breadcrumb is the **only** per-turn channel that fires while a DevFlow task
...
breadcrumb body lives in `.devflow/workflow.md`; this spec covers everything
...
Each breadcrumb body lives in a managed block of `.devflow/workflow.md`:
...
2. It walks up from `cwd` to find `.devflow/`. If none, exit 0.
...
5. It opens `.devflow/workflow.md` and parses every `[workflow-state:STATUS]`
   block.
...
`.devflow/workflow.md`. No script change required.
...
For that reason, `devflow update` must update `workflow.md` as one managed
...
|---|--------|-----------|-------|---------|
| 1 | `cmd_create` | `packages/cli/src/templates/devflow/scripts/common/task_store.py:206` | `"planning"` | `task.py create "<title>"` (also auto-sets the session active-task pointer when session identity is available — see R7 in 04-30-workflow-state-commit-gap PRD) |
| 2 | `cmd_start` | `packages/cli/src/templates/devflow/scripts/task.py:114-115, 128-129` | `"in_progress"` (gated on prior `"planning"`; both branches in `cmd_start`) | `task.py start <dir>` |
| 3 | `cmd_archive` | `packages/cli/src/templates/devflow/scripts/common/task_store.py:337` | `"completed"` (unconditional flip + archive `mv`) | `task.py archive <dir>` |
| 4 | `emptyTaskJson` factory | `packages/cli/src/utils/task-json.ts:54` | `"planning"` (default) | TS callers (init, update) |
| 5 | `getBootstrapTaskJson` | `packages/cli/src/commands/init.ts:535` | `"in_progress"` (override) | `devflow init` (creator path) |
| 6 | `getJoinerTaskJson` | `packages/cli/src/commands/init.ts:587` | `"in_progress"` (override) | `devflow init` (joiner path) |
| 7 | migration-task via `emptyTaskJson` | `packages/cli/src/commands/update.ts:2483-2494` | `"planning"` (override on factory) | `devflow update --migrate` for breaking-change manifest |

**No other writer exists.** No hook script writes `task.json.status` — verified
by `grep -rn '"status"' .devflow/scripts/`. Linear-sync hook (`linear_sync.py`)
...
1. Add a `[workflow-state:my-status]...[/workflow-state:my-status]` block to
   `.devflow/workflow.md` (STATUS charset: `[A-Za-z0-9_-]+`).
...
3. (Optional) Add the status to `.devflow/spec/devflow/backend/workflow-state-contract.md`'s
...
hooks for child turns or inherit main-session per-turn context. DevFlow must not
...
main-session dispatch guidance must self-exempt `devflow-implement` /
`devflow-check` readers so they implement or check directly instead of spawning
nested DevFlow sub-agents.

---

## DO

- Edit `.devflow/workflow.md` `[workflow-state:STATUS]` blocks for breadcrumb
  body changes; never touch the parser scripts.
- Keep `devflow update` whole-file behavior for hash-tracked `workflow.md`.
...
- Hook script structural change (parser, output envelope, what reads
  `task.json.status`)
- `workflow.md` update semantics in `devflow update`
...
Cross-reference: `.devflow/spec/devflow/backend/quality-guidelines.md` "Routing Fixes: Audit
