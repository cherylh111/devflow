...
- If two platforms render `{{PYTHON_CMD}}` differently, `devflow update`'s template-hash compare reports a phantom diff after every install.
...
- If `configure*()` writes through a helper but `collectTemplates()` byte-renders the raw template, hash tracking churns on every `devflow update`.
...
`configurators/shared.ts:resolveCommands` — returns command templates as plain commands (no frontmatter). Used by platforms that have a native command surface (Cursor, Claude, Gemini, OpenCode, etc.). Filters out `start.md` only when the platform is both `agentCapable` and `hasHooks` — hook-backed platforms inject the workflow overview automatically, so a user-facing `/start` would be redundant. Agent-capable platforms without SessionStart-style hooks (for example Codex and other pull-based or plugin-backed surfaces) keep `start.md` as a user-invocable fallback.
...
`configurators/shared.ts:resolveAllAsSkills` — folds command templates into skill format (with `devflow-` prefix and skill frontmatter). Used by skill-only platforms (Codex, Kiro, Qoder when emitting workflow skills). `start` is filtered out only for platforms whose `TemplateContext` is both `agentCapable` and `hasHooks`.

`configurators/shared.ts:resolveAllAsSkillsNeutral` — same, but neutral. Used by Codex for the command-as-skill files in `.agents/skills/` (`devflow-continue/SKILL.md`, `devflow-finish-work/SKILL.md`). Codex is the only writer of these specific files, so byte-identity isn't strictly required, but they go through the neutral helper to keep `{{CMD_REF}}` rendering consistent with the surrounding shared workflow skills.

`configurators/shared.ts:resolveCodexDevFlowStartSkill` — special-case singleton. Builds the `devflow-start` skill from the `start` command template + neutral renderer + skill frontmatter. Codex needs this in `.agents/skills/devflow-start/SKILL.md` so the `<devflow-bootstrap>` notice from `inject-workflow-state.py` resolves to a real file (the bootstrap notice tells the AI to invoke `$devflow-start` once on the first `no_task` turn). Returns `null` if the template is missing — defensive; should never happen in production. **Both** `configureCodex()` (init write) and `collectPlatformTemplates.codex` (update manifest) must call this; if only one calls it, upgraded users get the file written but never hash-tracked, or hash-tracked but never written.

`configurators/shared.ts:resolveBundledSkills` — resolves multi-file built-in skills discovered under `templates/common/bundled-skills/<skill-name>/` into `ResolvedSkillFile[]`. Each entry has a POSIX-relative path under the skill name (e.g. `devflow-meta/references/core/template-pipeline.md` or `devflow-use/SKILL.md`). Bundled skill names are the directory names; DevFlow-owned bundled skills should keep the `devflow-` prefix for consistency. Bundled `SKILL.md` already owns its frontmatter — this helper does **not** wrap it. Configurators must pass these to both `writeSkills()` (init) and `collectSkillTemplates()` (update) to keep hash tracking aligned.
...
`configurators/shared.ts:buildPullBasedPrelude` — returns the standard "Required: Load DevFlow Context First" block. Used by class-2 platforms whose hook can't inject the sub-agent prompt (Gemini, Qoder, Codex, Copilot). The prelude tells the sub-agent to: (1) read `Active task: <path>` from the dispatch prompt; (2) fall back to `task.py current --source`; (3) ask the user. See `platform-integration.md` "Active task discovery on class-2 platforms (issue #225)" for why all three layers are needed.

`configurators/shared.ts:detectSubAgentType` — returns `"implement"` / `"check"` / `null` from a filename like `devflow-implement.md`. Strips `.md`, `.toml`, `.prompt.md`. Returns `null` for `devflow-research` and unknown names — they skip the prelude.
...
Resolution happens **at template-write time** (`devflow init`, `devflow update`). There are no runtime placeholders — by the time a hook script or agent definition is written to disk, every `{{…}}` is gone.
...
| `{{CMD_REF:name}}` | `ctx.cmdRefPrefix` | `resolvePlaceholders` (per-platform) / `resolvePlaceholdersNeutral` (`` `name` (DevFlow command) ``) | Use neutral form for any `.agents/skills/` write |
...
- **Localized template overlays mirror English template shape.** When changing the line structure, fences, placeholders, or injectable blocks in an English template under `packages/cli/src/templates/`, check whether `packages/cli/src/templates/zh/` has a matching overlay and update it in the same change. `test/templates/language-parity.test.ts` enforces equal line count, fence count, and placeholder sets for localized overlays.
...
- **Class-2 agent definitions carry the pull-based prelude.** `applyPullBasedPreludeMarkdown` / `applyPullBasedPreludeToml` must run on every class-2 platform's `devflow-implement` and `devflow-check` definitions (research is intentionally exempt).
...
- **`start.md` is filtered for hook-backed agent platforms.** `filterCommands` is private; `resolveCommands` / `resolveAllAsSkills` / `resolveAllAsSkillsNeutral` apply it when `ctx.agentCapable && ctx.hasHooks`. Configurators must not bypass these and call `getCommandTemplates()` directly — that re-introduces `start` on platforms that don't need it or removes it from hookless platforms that do.
...
- **Bundled skills already own frontmatter.** `wrapWithSkillFrontmatter` must not be applied to `resolveBundledSkills` output. `writeSkills` and `collectSkillTemplates` accept bundled files separately for this reason. Use the discovered directory name as the install directory; for DevFlow-owned bundled skills, keep that directory name prefixed with `devflow-`.
...
- **Touch user-owned spec content.** `.devflow/spec/`, `.devflow/.developer`, `.devflow/tasks/`, `.devflow/workspace/`, `.devflow/.current-task` are protected paths owned by `commands/update.ts` migration logic, not by configurators.
...
Correct: feed the same agent list through `applyPullBasedPreludeMarkdown` in both paths, then pass the result to `writeAgents` and `collectTemplates` respectively. After every `devflow update` on a stable installation the hash-tracker must report zero changes.
...
Correct: edit `SKILL_DESCRIPTIONS` in `configurators/shared.ts` to add the new entry, then add a regression test asserting `wrapWithSkillFrontmatter("devflow-foo", "...")` does not throw. The throw at init time is the safety net that prevents shipping a skill the AI matcher can never trigger.
...
This applies the prelude even to `devflow-research`, which doesn't have an active task to load. Correct: use `applyPullBasedPreludeMarkdown(agents)` — `detectSubAgentType` returns `null` for research, so the helper passes it through unchanged.
...
- `test/regression.test.ts` — historical issue gates: pull-based prelude alignment between write/collect (issue #225); `.agents/skills/` neutral rendering byte-identity; Codex `devflow-start` skill present after both init and update.
