...
**Codex-only files under `.agents/skills/`** (currently `devflow-continue/SKILL.md` and `devflow-finish-work/SKILL.md`, written via `resolveAllAsSkillsNeutral()`) are an explicit exception: only Codex writes them, so byte-identity across platforms is not required and they may use `{{CLI_FLAG}}` / `{{PYTHON_CMD}}`. They still go through the neutral helper to keep `{{CMD_REF}}` neutralized for consistency with the surrounding shared skills.
...
|---------|------------------|
| Python hooks/scripts | `.devflow/scripts/common/active_task.py` |
...
Do not add direct `.devflow/.current-task` reads in hooks, sub-agent context
...
| Per-agent-invocation context | `before_agent_start` extension event — appends `buildDevFlowContext()` (PRD + jsonl) **and** the same per-turn breadcrumb to `systemPrompt` so sub-agent first turns see workflow state |
...
| Windsurf | `/devflow-xxx` | Markdown (`.md`) + `SKILL.md` | `/devflow-finish-work` |
...
|---------|------------------------------|--------------------------|
| `start` | ❌ not emitted by the common command resolver (Codex installs `devflow-start` as a skill because it has no active SessionStart hook) | ✅ emitted — manual equivalent of session-start hook |
...
| **Class-2** — Pull-based | Platform's hook can't reliably mutate sub-agent prompts; DevFlow injects a "Required: Load DevFlow Context First" prelude into each sub-agent definition file so the sub-agent reads context itself at startup | Codex, Gemini CLI, Qoder, Copilot |
...
| `before_agent_start` | `pi.on?.("before_agent_start", …)` | every agent invocation (main + sub-agents) | full DevFlow context via `buildDevFlowContext()` (PRD + jsonl-referenced specs + agent definition) **appended to** the existing systemPrompt, plus the same per-turn breadcrumb so a sub-agent's first turn still sees workflow state |
...
- **`<session-overview>` parity**: Pi shells out to `python3 .devflow/scripts/get_context.py --mode session-overview` rather than re-implementing context generation in TS, so output stays canonical. Don't replace this with an inline TS implementation — that's a parity drift waiting to happen.
...
function onInput(event, ctx) {
  const overview = spawnSync("python3", [".devflow/scripts/get_context.py", "--mode", "session-overview"]);
...
`.devflow/tasks/04-17-subagent-hook-reliability-audit/research/platform-hook-audit.md`
