...
> Package boundary and coding rules for `@enpd/devflow-core` and the CLI.

---

## Overview

DevFlow is split into two version-locked packages:

| Package | Responsibility |
|---|---|
| `@enpd/devflow-core` | Reusable domain logic, storage primitives, reducers, task APIs, channel APIs, and typed contracts. |
| `@enpd/devflow` | CLI argument parsing, terminal rendering, command wiring, process exit behavior, template installation, migrations, and release scripts. |
...
- command definitions and option parsing (including `devflow mem` argv parsing)
- help text and terminal output (including `devflow mem` row formatting and `--json` shaping)
- prompts, confirmations, exit codes, and `process.exit`
- the OpenCode-unavailable stderr notice for `devflow mem` (a presentation concern, not a core one)
...
```ts
import { createChannelStore } from "@enpd/devflow-core/channel";
```
