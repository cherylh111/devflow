...
print("Run: {{PYTHON_CMD}} ./.devflow/scripts/task.py <args>")
```
...
command = `export DEVFLOW_CONTEXT_ID=${shellQuote(contextKey)}; ${command}`;

// GOOD - shell-dialect-aware command prefix
const prefix = process.platform === "win32" && !isWindowsPosixShell(process.env)
  ? `$env:DEVFLOW_CONTEXT_ID = ${powershellQuote(contextKey)}; `
  : `export DEVFLOW_CONTEXT_ID=${shellQuote(contextKey)}; `;
...
timeout and captured output. For example, `devflow --version` already runs the
...
- [ ] `.devflow/scripts/` - Project's own scripts (if self-hosting)
...
.devflow/scripts/script.py  ← Forgot to sync!
```
...
# DevFlow docs/config should say the rule, not guess one alias everywhere
...
included with DevFlow, verify the whole distribution path:
...
- [ ] `.devflow/.template-hashes.json` tracks the generated asset path.
- [ ] `devflow update --dry-run` reports `Already up to date!` in that temp
  repository.
...
tmpdir=$(mktemp -d /tmp/devflow-built-bin-smoke-XXXXXX)
